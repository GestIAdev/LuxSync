...
class MetaEngineAdapter extends BaseMetaEngineImpl {
     ...
     */
    setOrchestrationContext(context) {
    ...
    async executeEngineLogic(context) {
        ...
        switch (engineId) {
            ...
            case 'dream-forge':
                ...
                return this.engine.forgeDreams(this.orchestrationContext.currentState || {}, this.orchestrationContext.desiredOutcome || {});
            case 'ethical-core':
    ...
    async onCleanup() {
        ...
        if (this.engine && typeof this.engine.cleanup === 'function') {
            try {
                ...
                console.error(`💥 Failed to cleanup wrapped engine ${this.config.id}:`, error);
            }
        }
...
export class ConcreteMetaOrchestrator extends MetaOrchestratorImpl {
        ...
        try {
            ...
            if (this.selfAnalysisEngine && result.engineResults) {
                for (const engineResult of result.engineResults) {
                    ...
                    this.selfAnalysisEngine.recordDecision(decisionType, success, processingTime, decisionContext);
                }
            }
            return result;
        }
        catch (error) {
            console.error('🧠 [META-ORCHESTRATOR] Orchestration failed with exception:', error);
            return {
                success: false,
     ...
     */
    mapEngineToDecisionType(engineId) {
        switch (engineId) {
            case 'self-analysis':
                return 'decision';
            case 'pattern-emergence':
...
//# sourceMappingURL=ConcreteMetaOrchestrator.js.map