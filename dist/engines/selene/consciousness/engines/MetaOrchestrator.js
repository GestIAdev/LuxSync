...
export class MetaOrchestratorImpl {
    config;
    engineRegistry;
    safetyOrchestration;
    // Engine instances
    ...
    async getHealthSummary() {
        ...
        const scores = Object.values(engineHealths).map((h) => h.score);
        const averageScore = scores.length > 0 ? scores.reduce((a, b) => a + b) / scores.length : 0;
        const criticalCount = Object.values(engineHealths).filter((h) => h.issues.some((i) => i.severity === 'critical')).length;
    ...
    async initializeSafetyOrchestration() {
    ...
    async executeOrchestrationPhases(context, correlationId) {
        const engineResults = [];
        ...
        try {
            ...
            if (!finalResult.success) {
                finalResult.error = new Error(`ORCHESTRATION_FAILED: ${failedEngines} out of ${this.engines.size} engines failed. ` +
                    `Executed: ${executedEngines}, Failed: ${failedEngines}`);
            }
            else if (finalResult.degradedMode) {
    ...
    async executeEnginePhase(engineId, context, correlationId) {
        ...
        try {
            ...
            if (engineId === 'dream-forge' && typeof engine.setOrchestrationContext === 'function') {
                engine.setOrchestrationContext(context);
            }
...
//# sourceMappingURL=MetaOrchestrator.js.map