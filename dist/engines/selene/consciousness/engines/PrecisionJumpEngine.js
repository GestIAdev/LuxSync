...
export class PrecisionJumpEngine {
    ...
    calculateVolatility(recentPatterns) {
        ...
        const beautyVariance = beautyScores.reduce((sum, b) => sum + Math.pow(b - avgBeauty, 2), 0) / beautyScores.length;
        // 2. Varianza de convergence times
        const times = recentPatterns.map(p => p.convergenceTime);
        const avgTime = times.reduce((sum, t) => sum + t, 0) / times.length;
        const convergenceVariance = times.reduce((sum, t) => sum + Math.pow(t - avgTime, 2), 0) / times.length;
        ...
        if (volatileFactors === 0)
            overallVolatility = 'low';
        else if (volatileFactors === 1)
            overallVolatility = 'medium';
        else
            overallVolatility = 'high';
        return {
            beautyVariance,
            convergenceVariance,
            patternSwitchRate,
...
//# sourceMappingURL=PrecisionJumpEngine.js.map