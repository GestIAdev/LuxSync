...
export class VerificationEngine {
    db;
    rules; // Cache: "EntityType:fieldName" -> Rules[]
    rulesByType; // Cache: "check_type" -> Rules[]
    isInitialized = false;
    lastRuleLoadTime = 0;
    ruleLoadIntervalMs = 3600000; // Reload rules every hour
    verificationCache; // Simple cache
    ...
    async loadRules() {
        try {
            ...
            const result = await this.db.query(`SELECT id, entity_type, field_name, check_type, check_name, 
         ...
         ORDER BY entity_type, field_name, severity DESC`);
            ...
            console.log(`✅ Guardian loaded ${result.rows.length} integrity rules in ${loadTimeMs}ms`);
     ...
     */
    async verify(entityType, fieldName, value, context) {
        const startTime = Date.now();
        // Check cache first
     ...
     */
    async verifyBatch(entityType, input, context) {
        const startTime = Date.now();
        if (!this.isInitialized) {
            await this.loadRules();
        }
        ...
        const verifications = Object.entries(input).map(([fieldName, value]) => this.verify(entityType, fieldName, value, context)
            .then(result => {
     ...
     */
    async verifyForeignKey(referencedTable, referencedId, checkFields) {
        const startTime = Date.now();
        try {
            let query = `SELECT 1 FROM ${referencedTable} WHERE id = $1`;
            const params = [referencedId];
     ...
     */
    async verifyStateTransition(currentState, requestedState, transitionRules) {
     ...
     */
    async checkDependencies(entityType, entityId, dependencyMap) {
        const dependents = [];
        const affectedIds = [];
        const dependencies = dependencyMap[entityType] || [];
        for (const dep of dependencies) {
            try {
                const result = await this.db.query(`SELECT COUNT(*) as count, ARRAY_AGG(id) as ids FROM ${dep.table} WHERE ${dep.field} = $1`, [entityId]);
     ...
     */
    async verifyImmutable(oldValue, newValue, fieldName) {
     ...
     */
    async verifyHashIntegrity(fileContent, expectedHash, algorithm = 'sha256') {
        try {
     ...
     */
    async verifyDateRange(date, rules) {
        const errors = [];
     ...
     */
    async verifyEnum(value, allowedValues) {
     ...
     */
    async verifyUnique(table, field, value, excludeId) {
        try {
    ...
    async executeRule(rule, value, context) {
        try {
            switch (rule.check_type) {
                ...
                case 'FOREIGN_KEY':
                    const fkResult = await this.verifyForeignKey(rule.check_rule.referencedTable, value, rule.check_rule.checkFields);
                    return fkResult.valid;
                case 'UNIQUE':
                    const uniqueResult = await this.verifyUnique(rule.check_rule.table, rule.check_rule.field, value, context?.entityId);
                ...
                case 'IMMUTABLE':
                    ...
                    if (!context?.oldValue)
                        return true; // Can't check without old value
                ...
                case 'STRING':
                    ...
                    if (rule.check_rule.forbiddenChars) {
                        const forbidden = rule.check_rule.forbiddenChars;
                ...
                case 'STATE_MACHINE':
                    ...
                    if (!context?.currentState)
                        return true;
                    const stateResult = await this.verifyStateTransition(context.currentState, value, rule.check_rule.stateTransitions || {});
 ...
 */
export function createVerificationEngine(pool) {
