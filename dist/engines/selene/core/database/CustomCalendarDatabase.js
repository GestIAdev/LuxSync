...
export class CustomCalendarDatabase extends BaseDatabase {
    ...
    async createCustomCalendarViewV3(input) {
        const { userId, name, description, isPublic, isDefault, viewType, dateRangeStart, dateRangeEnd, filters, colorScheme } = input;
        const query = `
    ...
    async createCalendarEventV3(input) {
        const { userId, viewId, title, description, eventType, startDate, endDate, allDay, location, attendees, reminders, color, isRecurring, recurrenceRule, status } = input;
        const query = `
    ...
    async createCalendarFilterV3(input) {
        const { userId, viewId, name, description, filterType, filterCriteria, isActive, sortOrder } = input;
        const query = `
    ...
    async updateCalendarSettingsV3(userId, input) {
    ...
    // ============================================================================
    async getUserDefaultCalendarViewV3(userId) {
        const query = `
    ...
    async getCalendarEventsWithRemindersV3(userId) {
        const query = `
      SELECT
        ce.*,
        ccv.name as view_name
    ...
    async duplicateCalendarViewV3(viewId, newName, userId) {
        // Get original view
    ...
    async applyCalendarFilterV3(viewId, filterId) {
        // Get filter criteria
    ...
    async getCalendarViewStatsV3(viewId) {
        const query = `
      SELECT
    ...
    async bulkUpdateCalendarEventsV3(eventIds, updates) {
        const updatedEvents = [];
    ...
    async setDefaultCalendarViewV3(userId, viewId) {
        ...
        await this.runQuery(`UPDATE custom_calendar_views SET is_default = false WHERE user_id = $1`, [userId]);
        // Set the new default
        await this.runQuery(`UPDATE custom_calendar_views SET is_default = true WHERE id = $1 AND user_id = $2`, [viewId, userId]);
    }
    ...
    async toggleCalendarFilterV3(filterId) {
        ...
        const result = await this.runQuery(`UPDATE calendar_filters SET is_enabled = $1, updated_at = NOW() WHERE id = $2 RETURNING *`, [newEnabled, filterId]);
        return result.rows[0];
    }
}
//# sourceMappingURL=CustomCalendarDatabase.js.map