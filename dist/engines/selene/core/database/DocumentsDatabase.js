...
export class DocumentsDatabase extends BaseDatabase {
    ...
    async updateDocument(id, data) {
        ...
        try {
            ...
            values.push(id);
            const result = await client.query(`UPDATE medical_documents SET ${fields.join(", ")} WHERE id = $${paramIndex} AND deleted_at IS NULL RETURNING *`, values);
            await client.query("COMMIT");
    ...
    async deleteDocument(id) {
        ...
        try {
            await client.query("BEGIN");
            const result = await client.query(`UPDATE medical_documents SET deleted_at = NOW(), last_modified = NOW() WHERE id = $1 AND deleted_at IS NULL RETURNING id`, [id]);
            await client.query("COMMIT");
...
//# sourceMappingURL=DocumentsDatabase.js.map