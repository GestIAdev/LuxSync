...
export class InventoryDatabase extends BaseDatabase {
    ...
    async createInventoryV3(input) {
        const { name, category, currentStock, minimumStock, unit, location, supplierId, expiryDate } = input;
        const query = `
    ...
    async adjustInventoryStockV3(id, adjustment, reason) {
        ...
        const current = await this.getOne(`SELECT current_stock FROM inventory WHERE id = $1`, [id]);
        if (!current) {
        ...
        await this.runQuery(`UPDATE inventory SET current_stock = $1, last_restocked = NOW(), updated_at = NOW() WHERE id = $2`, [newStock, id]);
        // Record the adjustment (assuming there's an inventory_adjustments table)
        await this.runQuery(`INSERT INTO inventory_adjustments (inventory_id, adjustment, reason, created_at)
       VALUES ($1, $2, $3, NOW())`, [id, adjustment, reason]);
    ...
    async createMaterialV3(input) {
        const { name, category, unitCost, unit, currentStock, minimumStock, supplier, expiryDate, batchNumber } = input;
        const query = `
    ...
    async createEquipmentV3(input) {
        const { name, model, serialNumber, manufacturer, category, status, purchaseDate, warrantyExpiry, location, purchaseCost, depreciationRate } = input;
        const query = `
    ...
    async createMaintenanceV3(input) {
        const { equipmentId, maintenanceType, description, scheduledDate, status, cost, performedBy } = input;
        const query = `
    ...
    async createSupplier(input) {
        const { name, contactPerson, email, phone, address, paymentTerms, deliveryTimeDays, minimumOrderValue, rating, notes } = input;
        const query = `
    ...
    async createPurchaseOrder(input) {
        const { supplierId, orderNumber, orderDate, expectedDeliveryDate, totalAmount, taxAmount, discountAmount, notes, approvedBy } = input;
        const query = `
    ...
    async receivePurchaseOrder(id, receivedBy) {
        const updateQuery = `
      UPDATE purchase_orders
    ...
    async getPurchaseOrderItemById(id) {
        const query = `
      SELECT poi.*, i.name as material_name, i.unit
      FROM purchase_order_items poi
    ...
    async addPurchaseOrderItem(purchaseOrderId, input) {
    ...
    async updatePurchaseOrderItem(id, input) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
    ...
    async updatePurchaseOrderTotal(purchaseOrderId) {
        const query = `
      UPDATE purchase_orders
      SET total_amount = (
    ...
    // ============================================================================
    async getSupplierPurchaseOrders(args) {
    ...
    async getSupplierTotalOrders(supplierId) {
    ...
    async getSupplierTotalSpent(supplierId) {
        const query = `
...
//# sourceMappingURL=InventoryDatabase.js.map