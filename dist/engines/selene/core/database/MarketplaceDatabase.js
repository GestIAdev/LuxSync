...
export class MarketplaceDatabase extends BaseDatabase {
    ...
    async createSupplierV3(input) {
        const { name, contactPerson, email, phone, address, isActive = true, paymentTerms, deliveryTimeDays, minimumOrderValue, rating, notes } = input;
        const query = `
    ...
    async createPurchaseOrderV3(input) {
        const { supplierId, orderDate, expectedDeliveryDate, notes, items } = input;
    ...
    async receivePurchaseOrderV3(id, receivedItems) {
        // Update received quantities for items
        for (const item of receivedItems) {
            await this.runQuery(`UPDATE purchase_order_items
         SET received_quantity = received_quantity + $1, updated_at = NOW()
         WHERE id = $2`, [item.receivedQuantity, item.id]);
        }
        // Check if all items are fully received
        const itemsResult = await this.runQuery(`SELECT COUNT(*) as total, COUNT(CASE WHEN quantity <= received_quantity THEN 1 END) as received
       FROM purchase_order_items
       WHERE purchase_order_id = $1`, [id]);
        const { total, received } = itemsResult.rows[0];
        const status = total === received ? 'RECEIVED' : 'PARTIALLY_RECEIVED';
        const result = await this.runQuery(`UPDATE purchase_orders
    ...
    async updatePurchaseOrderItemV3(id, input) {
        ...
        if (input.quantity !== undefined) {
            ...
            if (input.unitPrice !== undefined) {
                ...
                const current = await this.runQuery(`SELECT unit_price FROM purchase_order_items WHERE id = $1`, [id]);
                if (current.rows[0]) {
                    updates.push(`total_price = $${paramIndex++}`);
        ...
        if (input.unitPrice !== undefined) {
            ...
            // Recalculate total price
            const current = await this.runQuery(`SELECT quantity FROM purchase_order_items WHERE id = $1`, [id]);
            if (current.rows[0]) {
                updates.push(`total_price = $${paramIndex++}`);
    ...
    async removePurchaseOrderItemV3(id) {
        ...
        const itemResult = await this.runQuery(`SELECT purchase_order_id FROM purchase_order_items WHERE id = $1`, [id]);
        if (itemResult.rows[0]) {
            const purchaseOrderId = itemResult.rows[0].purchase_order_id;
            await this.runQuery(`DELETE FROM purchase_order_items WHERE id = $1`, [id]);
            // Update purchase order total
            await this.updatePurchaseOrderTotalV3(purchaseOrderId);
        }
    }
    ...
    async updatePurchaseOrderTotalV3(purchaseOrderId) {
        const result = await this.runQuery(`SELECT COALESCE(SUM(total_price), 0) as total FROM purchase_order_items
       WHERE purchase_order_id = $1`, [purchaseOrderId]);
        const total = result.rows[0].total;
        await this.runQuery(`UPDATE purchase_orders SET total_amount = $1, updated_at = NOW() WHERE id = $2`, [total, purchaseOrderId]);
    }
    // ============================================================================
    ...
    async getSupplierTotalOrdersV3(supplierId) {
        const result = await this.runQuery(`SELECT COUNT(*) as total FROM purchase_orders WHERE supplier_id = $1`, [supplierId]);
        return parseInt(result.rows[0].total) || 0;
    }
    async getSupplierTotalSpentV3(supplierId) {
        const result = await this.runQuery(`SELECT COALESCE(SUM(total_amount), 0) as total FROM purchase_orders
       WHERE supplier_id = $1 AND status IN ('RECEIVED', 'PARTIALLY_RECEIVED')`, [supplierId]);
    ...
    async addToCartV3(input) {
        const { marketplaceProductId, quantity, unitPrice } = input;
        // Check if item already exists in cart
        const existing = await this.runQuery(`SELECT id, quantity FROM cart_items WHERE marketplace_product_id = $1`, [marketplaceProductId]);
        if (existing.rows.length > 0) {
            // Update existing item
            const newQuantity = existing.rows[0].quantity + quantity;
            const totalPrice = newQuantity * unitPrice;
            const result = await this.runQuery(`UPDATE cart_items
         SET quantity = $1, total_price = $2, updated_at = NOW()
         WHERE id = $3
         RETURNING *`, [newQuantity, totalPrice, existing.rows[0].id]);
            return result.rows[0];
        }
        else {
            // Add new item
            const totalPrice = quantity * unitPrice;
            const result = await this.runQuery(`INSERT INTO cart_items (
          marketplace_product_id, quantity, unit_price, total_price, added_at
        ) VALUES ($1, $2, $3, $4, NOW())
        RETURNING *`, [marketplaceProductId, quantity, unitPrice, totalPrice]);
            return result.rows[0];
        }
    }
    async updateCartItemV3(id, input) {
        ...
        if (input.quantity !== undefined) {
            ...
            const current = await this.runQuery(`SELECT unit_price FROM cart_items WHERE id = $1`, [id]);
            if (current.rows[0]) {
                updates.push(`total_price = $${paramIndex++}`);
                values.push(input.quantity * current.rows[0].unit_price);
            }
        ...
        if (input.unitPrice !== undefined) {
            ...
            const current = await this.runQuery(`SELECT quantity FROM cart_items WHERE id = $1`, [id]);
            if (current.rows[0]) {
                updates.push(`total_price = $${paramIndex++}`);
                values.push(current.rows[0].quantity * input.unitPrice);
            }
...
//# sourceMappingURL=MarketplaceDatabase.js.map