...
export class NotificationsDatabase extends BaseDatabase {
     ...
     */
    async updatePreferences(patientId, input, auditContext) {
        try {
            const { smsEnabled, emailEnabled, pushEnabled, appointmentReminders, billingAlerts, treatmentUpdates, marketingEmails, } = input;
     ...
     */
    async createAuditTrail(args) {
        try {
            const { patientId, action, oldValues, newValues, userId, ipAddress, userAgent, } = args;
            const query = `
    ...
    async createNotification(input) {
        try {
            const { patientId, type, channel, title, message, priority, metadata, } = input;
            const query = `
...
//# sourceMappingURL=NotificationsDatabase.js.map