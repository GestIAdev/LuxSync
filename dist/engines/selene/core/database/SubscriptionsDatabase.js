...
export class SubscriptionsDatabase extends BaseDatabase {
    ...
    async createSubscriptionPlanV3(input) {
        ...
        const { clinic_id, // REQUIRED from user context
        name, description, price, currency = 'EUR', code, consultas_incluidas = 0, // 0 = unlimited
        max_services_per_month, max_services_per_year, features = [], billing_cycle = 'monthly', is_active = true } = input;
        if (!clinic_id) {
    ...
    async deleteSubscriptionPlanV3(id) {
    ...
    async cancelSubscriptionV3(id, immediate = false) {
        if (immediate) {
            const query = `
        UPDATE subscriptions
    ...
    async reactivateSubscriptionV3(id) {
        const query = `
      UPDATE subscriptions
    ...
    async getBillingCycleV3ById(id) {
        const query = `SELECT * FROM billing_cycles WHERE id = $1`;
        return await this.getOne(query, [id]);
    }
    async createBillingCycleV3(input) {
        const { subscriptionId, amount, currency, periodStart, periodEnd, status } = input;
        const query = `
    ...
    async updateBillingCycleV3(id, input) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
        if (input.status !== undefined) {
    ...
    async processBillingCycleV3(id) {
        ...
        const result = await this.runQuery(`UPDATE billing_cycles
    ...
    async trackServiceUsageV3(input) {
        const { subscriptionId, metric, value, recordedAt } = input;
        const query = `
    ...
    async getUsageSummaryV3(subscriptionId, startDate, endDate) {
        const query = `
      SELECT
        metric,
    ...
    // ============================================================================
    async getUserActiveSubscriptionV3(userId) {
        const query = `
      SELECT
        s.*,
        sp.name as plan_name, sp.price_monthly, sp.price_yearly,
    ...
    async checkSubscriptionLimitsV3(subscriptionId) {
        ...
        const userCount = await this.runQuery(`SELECT COUNT(*) as count FROM users WHERE subscription_id = $1`, [subscriptionId]);
        const patientCount = await this.runQuery(`SELECT COUNT(*) as count FROM patients WHERE subscription_id = $1`, [subscriptionId]);
        const storageUsage = await this.runQuery(`SELECT COALESCE(SUM(file_size), 0) as total FROM documents WHERE subscription_id = $1`, [subscriptionId]);
    ...
    async getTrialExpiringSubscriptionsV3(daysAhead = 7) {
    ...
    async convertTrialToPaidV3(subscriptionId) {
        ...
        const result = await this.runQuery(`UPDATE subscriptions
       SET status = 'ACTIVE', trial_end = NULL, updated_at = NOW()
       WHERE id = $1
       RETURNING *`, [subscriptionId]);
        // Update first billing cycle from TRIAL to PENDING
        await this.runQuery(`UPDATE billing_cycles
       SET status = 'PENDING', updated_at = NOW()
       WHERE subscription_id = $1 AND status = 'TRIAL'`, [subscriptionId]);
        return result.rows[0];
    }
...
//# sourceMappingURL=SubscriptionsDatabase.js.map