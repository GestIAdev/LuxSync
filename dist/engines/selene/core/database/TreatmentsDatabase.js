...
export class TreatmentsDatabase extends BaseDatabase {
     ...
     */
    async persistTreatmentMaterials(client, treatmentId, materials, createdBy) {
        ...
        for (const material of materials) {
            try {
                ...
                const materialInfo = await client.query(`SELECT id, unit_cost FROM dental_materials WHERE id = $1`, [material.inventoryItemId]);
                ...
                await client.query(`INSERT INTO treatment_materials (
            treatment_id, material_id, quantity, cost_snapshot, created_by, created_at
          ) VALUES ($1, $2, $3, $4, $5, NOW())`, [treatmentId, materialId, material.quantity, unitCost || 0, createdBy]);
    ...
    async createTreatment(data) {
      ...
      `, [
            ...
            if (data.materialsUsed && data.materialsUsed.length > 0) {
                ...
                await this.persistTreatmentMaterials(client, dbRow.id, data.materialsUsed, data.practitionerId);
            }
    ...
    async updateTreatment(id, data) {
        ...
        try {
            ...
            values.push(id);
            const result = await client.query(`UPDATE medical_records SET ${fields.join(", ")} WHERE id = $${paramIndex} AND deleted_at IS NULL RETURNING *`, values);
            await client.query("COMMIT");
    ...
    async deleteTreatment(id) {
        ...
        try {
            await client.query("BEGIN");
            const result = await client.query(`UPDATE medical_records SET deleted_at = NOW(), updated_at = NOW() WHERE id = $1 AND deleted_at IS NULL RETURNING id`, [id]);
            await client.query("COMMIT");
     ...
     */
    async getOdontogramData(patientId) {
        const query = `
     ...
     */
    async updateToothStatus(patientId, toothNumber, status, condition, notes) {
        const odontogram = await this.getOdontogramData(patientId);
        const teeth = odontogram.teeth.map((tooth) => {
     ...
     */
    calculateToothPosition(toothNumber) {
...
//# sourceMappingURL=TreatmentsDatabase.js.map