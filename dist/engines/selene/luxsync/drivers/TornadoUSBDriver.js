 ...
 */
export class TornadoUSBDriver {
    device = null; // USBDevice (Web USB API)
    interface = null;
    endpoint = null;
    connected = false;
    universe1; // 512 channels for OUT1
    universe2; // 512 channels for OUT2
    frameCount = 0;
    lastFrameTime = 0;
    targetFPS = 30; // DMX refresh rate (23-44 FPS standard)
    // Tornado USB device identifiers (common for FTDI-based DMX interfaces)
    static VENDOR_ID = 0x0403; // FTDI
    static PRODUCT_ID = 0x6001; // FT232 USB-Serial (common in DMX interfaces)
    constructor(fps = 30) {
    ...
    async initialize() {
        ...
        try {
            ...
            const outEndpoint = this.interface.alternate.endpoints.find((ep) => ep.direction === 'out');
            if (!outEndpoint) {
     ...
     */
    mapSceneToChannels(scene) {
        // Clear all channels
        this.universe1.fill(0);
        // Map each fixture from the fixtureStates Map
        let index = 0;
        scene.fixtureStates.forEach((state, fixtureId) => {
...
//# sourceMappingURL=TornadoUSBDriver.js.map