...
export class VisualEffects {
    currentEffect = { mode: 'none', speed: 0.5, intensity: 0.5 };
    frameCounter = 0;
    lastEffectTime = 0;
    constructor() {
     ...
     */
    applyEffect(fixtureCount, audioMetrics) {
        this.frameCounter++;
    ...
    chasePattern(fixtureCount) {
        ...
        return Array(fixtureCount).fill(null).map((_, i) => {
            ...
            if (isActive)
                dimmer = 1.0 + this.currentEffect.intensity; // Active bright
            else if (isPrevious)
                dimmer = 0.5 + this.currentEffect.intensity * 0.5; // Trail
            else if (isPrevious2)
                dimmer = 0.2 + this.currentEffect.intensity * 0.2; // Faint trail
            return {
                fixtureIndex: i,
     ...
     */
    strobePattern(fixtureCount, audioMetrics) {
     ...
     */
    pulsePattern(fixtureCount, audioMetrics) {
        const bass = audioMetrics?.bass || 0.5;
