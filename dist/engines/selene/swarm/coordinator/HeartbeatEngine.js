...
export class HeartbeatEngine extends EventEmitter {
    ...
    _interval = null;
    _sequence = 0;
    _pattern = RHYTHM_PATTERNS.STEADY;
    _redis;
    _redisOptimizer;
    _nodeId;
    _lastPulse = null;
    _systemVitals;
    _digitalSoul;
    constructor(nodeId, rhythm = GENESIS_CONSTANTS.HEARTBEAT_RHYTHM, redis, redisOptimizer, systemVitals, digitalSoul) {
        super();
        this._nodeId = nodeId;
        this._rhythm = rhythm;
        this._redis = redis;
    ...
    async getVitals() {
        ...
        let health;
        if (vitalSigns.health >= 0.9)
            health = "optimal";
        else if (vitalSigns.health >= 0.7)
            health = "healthy";
        else if (vitalSigns.health >= 0.5)
            health = "warning";
    ...
    async pulse() {
        ...
        if (process.env.DEBUG_HEARTBEAT === "true") {
            console.log(`💗 [${this._nodeId.id}] Pulse ${this._beats} - Sequence: ${this._sequence} - Rhythm: ${this._rhythm}ms`);
        }
        try {
            ...
            console.log(`💔 [${this._nodeId.id}] Pulse error: ${error instanceof Error ? error.message : "Unknown error"}`);
    ...
    async getConnectionCount() {
        try {
            ...
            const connections = await this._redis.scard(`${GENESIS_CONSTANTS.REDIS_SWARM_KEY}:connections:${this._nodeId.id}`);
    ...
    async publishHeartbeat(heartbeat) {
        try {
            ...
            await this._redisOptimizer.batchPublish(`${GENESIS_CONSTANTS.REDIS_SWARM_KEY}:heartbeat:${this._nodeId.id}`, heartbeatData);
            // Actualizar último heartbeat en Redis (BATCHED)
            await this._redisOptimizer.batchSet(`${GENESIS_CONSTANTS.REDIS_SWARM_KEY}:${this._nodeId.id}`, heartbeatData, 30);
            ...
            await this._redis.hset(GENESIS_CONSTANTS.REDIS_SWARM_KEY, // "dentiagest:swarm:nodes" (hash map)
            this._nodeId.id, // field del hash
            heartbeatData);
            // Note: expire() operation integrated into batchSet() TTL parameter
        }
        catch (error) {
            console.log(`🔴 [${this._nodeId.id}] Redis heartbeat publish error: ${error instanceof Error ? error.message : "Unknown error"}`);
...
export class EternalPulse extends EventEmitter {
    ...
    _started = null;
    _totalBeats = 0;
    constructor(nodeId, rhythm, redis, redisOptimizer, systemVitals, digitalSoul) {
        super();
        this._rhythm = rhythm;
        this._heartbeatEngine = new HeartbeatEngine(nodeId, rhythm, redis, redisOptimizer, systemVitals, digitalSoul);
...
//# sourceMappingURL=HeartbeatEngine.js.map