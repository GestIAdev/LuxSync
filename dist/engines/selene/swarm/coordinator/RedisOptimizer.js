...
export class RedisOptimizer {
    ...
    maxBatchSize = 100;
    flushTimeout = null;
    flushIntervalMs = 100; // Auto-flush every 100ms
    // Stats
    _stats = {
        totalOperations: 0,
        totalBatches: 0,
     ...
     */
    async batchPublish(channel, message) {
        if (!this.pipeline) {
    ...
    async flush() {
        ...
        try {
            ...
            this._stats.averageBatchSize = Math.round(this._stats.totalOperations / this._stats.totalBatches);
            this._stats.averageFlushTime = Math.round(this._stats.totalFlushTime / this._stats.totalBatches);
            // 🎯 PUNK RULE: Solo log operaciones SIGNIFICATIVAS (> 30 ops O > 5ms)
            if (opsCount > 30 || duration > 5) {
                console.log(`⚡ Redis batch flushed: ${opsCount} ops in ${duration}ms ` +
                    `(avg: ${this._stats.averageBatchSize} ops, ${this._stats.averageFlushTime}ms)`);
            }
        }
        catch (error) {
            console.error('💥 Redis batch flush failed:', error);
    ...
    async destroy() {
        ...
        console.log(`✅ RedisOptimizer destroyed - Total: ${this._stats.totalOperations} ops, ` +
 ...
 */
//# sourceMappingURL=RedisOptimizer.js.map