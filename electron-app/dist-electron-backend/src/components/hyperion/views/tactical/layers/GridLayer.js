 ...
 */
export function renderGridLayer(ctx, width, height, options) {
    const { showReferenceLines = true, showStereoDivision = true, opacity = 1.0, viewport, } = options ?? {};
    ...
    if (showStereoDivision) {
        ...
        ctx.beginPath();
        ctx.moveTo(centerX, drawTop);
        ctx.lineTo(centerX, drawTop + (drawBottom - drawTop) * 0.3);
        ctx.stroke();
        // Middle solid (30% → 70%)
        ctx.strokeStyle = `rgba(176, 38, 255, ${a})`;
        ctx.beginPath();
        ctx.moveTo(centerX, drawTop + (drawBottom - drawTop) * 0.3);
        ctx.lineTo(centerX, drawTop + (drawBottom - drawTop) * 0.7);
        ctx.stroke();
        // Bottom fade (70% → 100%)
        ctx.strokeStyle = `rgba(176, 38, 255, ${a * 0.5})`;
        ctx.beginPath();
        ctx.moveTo(centerX, drawTop + (drawBottom - drawTop) * 0.7);
        ctx.lineTo(centerX, drawBottom);
        ctx.stroke();
    }
}
// ═══════════════════════════════════════════════════════════════════════════
