...
const STRICT_PRIORITY_CHANNELS = new Set(['strobe', 'strobeRate', 'shutter']);
...
export class NodeArbiter {
    constructor() {
        ...
        this._calibrationLastInjectMs = 0;
        /**
        ...
        this._spatialSuppressedNodes = new Set();
        /**
         ...
         */
        this._manualPatternLocks = new Set();
        /**
         ...
         */
        this._spatialCoupledLock = new Set();
        /**
    ...
    setManualOverride(nodeId, channels) {
        ...
        if (this._spatialCoupledLock.has(nodeId)) {
            const hasAbsoluteClassicAxis = incomingKeys.some(k => (k === 'pan' || k === 'tilt'));
            if (hasAbsoluteClassicAxis) {
                console.warn(`[WAVE-7734] setManualOverride REJECTED on ${nodeId}: node is in ` +
    ...
    clearManualOverride(nodeId, releaseMs) {
     ...
     */
    setManualPatternLock(nodeIds) {
        for (let i = 0; i < nodeIds.length; i++) {
            this._manualPatternLocks.add(nodeIds[i]);
        }
    }
    clearManualPatternLock(nodeId) {
    ...
    _applyRelativeOffsetFusion() {
        ...
        for (const [nodeId, record] of this._result) {
            ...
            if (motor) {
                ...
                if (isFiniteChannelValue(mtx))
                    record['targetX'] = mtx;
                if (isFiniteChannelValue(mty))
                    record['targetY'] = mty;
                if (isFiniteChannelValue(mtz))
                    record['targetZ'] = mtz;
                ...
                if (isFiniteChannelValue(mpo))
                    record['pan_offset'] = mpo;
            ...
            const basePan = hasMotorPan ? motorPan : (hasManualPan ? manualPan : 0.5);
            const baseTilt = hasMotorTilt ? motorTilt : (hasManualTilt ? manualTilt : 0.5);
            ...
            if (hasBaseTilt || hasTiltOffset) {
                ...
                if (final < TILT_ARBITER_MIN)
                    final = TILT_ARBITER_MIN;
    ...
    _applyIntent(intent, layer) {
        ...
        for (const channel in values) {
            ...
            if (layer === 'effect' || layer === 'hephaestus' || layer === 'calibration') {
                ...
                if ((layer === 'hephaestus' || layer === 'calibration') && (channel === 'r' || channel === 'g' || channel === 'b' ||
        ...
        for (const [nodeId, rel] of this._releaseStates) {
            ...
            for (const key in rel.channels) {
                ...
                if (l0Value !== undefined && Number.isFinite(l0Value)) {
                    ...
                    if (key === 'tilt')
                        blended = Math.max(TILT_ARBITER_MIN, Math.min(blended, TILT_ARBITER_MAX));
                    record[key] = blended;
                }
            }
            if (fadeCompleted) {
