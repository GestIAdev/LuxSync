 ...
 * WAVE 3516.4: THE OPTIC BRIDGE
 */
export class BeamAdapter extends BaseSystem {
    constructor() {
        super();
        this.name = 'BeamAdapter';
        this.family = NodeFamily.BEAM;
        this.source = BEAM_SOURCE;
        /**
    ...
    process(view, context, bus) {
        ...
        view.forEach((node) => {
            ...
            if (node.hasZoom)
                this._valuesDict['zoom'] = zoom;
            if (node.hasFocus)
                this._valuesDict['focus'] = focus;
            if (node.hasGobo)
                this._valuesDict['gobo'] = goboValue;
            if (node.hasGoboRotation)
                this._valuesDict['gobo_rotation'] = goboRotation;
            if (node.hasPrism)
                this._valuesDict['prism'] = prismValue;
            if (node.hasPrismRotation)
                this._valuesDict['prism_rotation'] = prismRotation;
