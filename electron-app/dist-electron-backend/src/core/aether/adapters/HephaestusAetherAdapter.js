...
export class HephaestusAetherAdapter {
    ...
    ingest(outputs, arbiter) {
            ...
            if (familyNodeIds.length === 1) {
                ...
                if (output.cell !== undefined) {
                    for (let j = 0; j < familyNodeIds.length; j++) {
                        ...
                        if (!_nodeCellMatches(nodeId, output.cell))
                            continue; // O(1), zero-alloc
                        let intent = this._frameIntentMap.get(nodeId);
                        if (!intent) {
                            intent = this._acquireIntent(nodeId);
                            this._frameIntentMap.set(nodeId, intent);
        ...
        // ⚒️ WAVE 7751: TRUTH ENGINE — _ensureStrobeDimmer ELIMINADO.
        // El forzado de dimmer=1.0 cuando hay strobe activo rompía el diseño
        // por capas: la curva de Bezier es la única autoridad sobre el dimmer.
        // Los Gobernadores DMX del usuario protegen la mecánica. Si un clip
        // de strobe no tiene track de dimmer, el dimmer queda en su estado
        // anterior (L0/L1 o manual) — que es lo correcto arquitectónicamente.
    ...
    switch (param) {
        ...
        case 'strobe':
            ...
            if (output.normalizedValue > 0) {
                ...
                // ⚒️ WAVE 7751: TRUTH ENGINE — Eliminado el forzado de dimmer=1.0.
                // La curva de Bezier es la única autoridad. Los Gobernadores DMX
                // del usuario controlan la protección de la mecánica. Forzar
                // dimmer rompía el diseño por capas (WAVE 7749.49 revertido).
                // El shutter=1.0 se CONSERVA para garantizar que máquinas antiguas
                // con obturador mecánico estén encendidas por defecto.
            }
            break;
        case 'color': {
 ...
 */
function _nodeCellMatches(nodeId, cell) {
