...
const HARD_SAFETY_CHANNELS = new Set([
    'emission_gate',
    'fire_valve',
    'fire_ignite',
]);
...
export class NodeResolver {
    ...
    updateLiveCalibration(nodeId, calibration) {
        ...
        if (device && device.calibration) {
            ...
            device.calibration.panOffset = Math.round(calibration.panOffset / panRange * 255);
            device.calibration.tiltOffset = Math.round(calibration.tiltOffset / tiltRange * 255);
            device.calibration.invertPan = calibration.panInvert;
            device.calibration.invertTilt = calibration.tiltInvert;
        }
    }
    /**
        ...
        for (let fi = 0; fi < families.length; fi++) {
            ...
            view.forEach((node) => {
                ...
                for (let ci = 0; ci < node.channels.length; ci++) {
                    ...
                    if (!SOFT_BLACKOUT_INTENSITY_CHANNELS.has(chDef.type) && !HARD_SAFETY_CHANNELS.has(chDef.type))
                        continue;
                    const idx = baseAddr + chDef.dmxOffset;
                    if (idx >= 0 && idx < DMX_UNIVERSE_SIZE) {
                        mask[idx] = 1;
