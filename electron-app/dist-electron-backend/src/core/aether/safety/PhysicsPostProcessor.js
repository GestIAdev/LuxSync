...
export class PhysicsPostProcessor {
    ...
    registerDevice(definition) {
        if (_registered.has(definition.deviceId))
            return;
    ...
    process(arbitrated, graph, deltaMs) {
        ...
        for (const [nodeId, channels] of arbitrated) {
            ...
            for (const key in channels) {
                this._scratch[key] = channels[key];
            }
            // Convertir 0-1 -> DMX 0-255 para el driver
            const targetPanDMX = hasPan ? _clamp(channels['pan'] * 255, 0, 255) : 127;
            const targetTiltDMX = hasTilt ? _clamp(channels['tilt'] * 255, 0, 255) : 127;
            // Llamar al driver — interpola con inertia
            const result = this._driver.translateDMX(nodeData.deviceId, targetPanDMX, targetTiltDMX, deltaMs, false);
            ...
            if (cal) {
                if (cal.invertPan)
                    physPan = 255 - physPan;
                if (cal.invertTilt)
                    physTilt = 255 - physTilt;
                physPan = _clamp(physPan + (cal.panOffset ?? 0), 0, 255);
                physTilt = _clamp(physTilt + (cal.tiltOffset ?? 0), cal.tiltLimitMin ?? 0, cal.tiltLimitMax ?? 255);
            }
            // Convertir de vuelta a 0-1 (NodeResolver escala a DMX)
            if (hasPan)
                this._scratch['pan'] = physPan / 255;
            if (hasTilt)
                this._scratch['tilt'] = physTilt / 255;
