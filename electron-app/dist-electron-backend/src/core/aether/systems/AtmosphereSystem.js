...
// ATMOSPHERE SYSTEM
// ─────────────────────────────────────────────────────────────────────────────
export class AtmosphereSystem extends BaseSystem {
    constructor() {
        super();
        this.name = 'AtmosphereSystem';
     ...
     */
    _processFog(node, outputBase, fanBase, densityBase, energyMod, context, bus) {
     ...
     */
    _processHaze(node, densityBase, fanBase, energyMod, bus) {
     ...
     */
    _processSpark(node, energy, vibeIntensity, context, bus) {
     ...
     */
    _processFan(node, fanBase, energy, bus) {
        // El fan siempre tiene un mínimo para no sonar a cero.
        const fanSpeed = BaseSystem.clamp01(Math.max(FAN_MIN_OUTPUT, fanBase + energy * 0.25));
     ...
     */
    _processCustom(node, outputBase, fanBase, energyMod, bus) {
     ...
     */
    _pushIntent(node, output, fanSpeed, density, bus) {
        this._intentScratch.nodeId = node.nodeId;
