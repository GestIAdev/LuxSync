...
import { ControlLayer, DEFAULT_ARBITER_CONFIG, getChannelCategory, } from './types';
...
import { LayerStateManager, } from './state/LayerStateManager';
// ─────────────────────────────────────────────────────────────────────────────
...
export class ArbitrationDirector extends EventEmitter {
    ...
    constructor(config = {}, layerState) {
        ...
        this._lastOutputEnabled = false; // for gate change detection
        // ── WAVE 2910 WIRETAP ──────────────────────────────────────────────────────
    ...
    setFixtures(fixtures) {
        ...
        for (const fixture of fixtures) {
            ...
            if (this.isMovingFixture(fixture))
                moverCount++;
            const chCount = fixture.channels?.length ?? 0;
    ...
    isMovingFixture(fixture) {
        ...
        const hasPanTilt = fixture.channels?.some((c) => c.type === 'pan' || c.type === 'tilt');
    ...
    getFixtureIdsByZone(effectZone) {
        ...
        if (result.length === 0) {
            console.warn(`[ArbitrationDirector] ⚠️ WAVE 2543.4: Zone "${effectZone}" matched 0 fixtures — falling back to wildcard`);
    ...
    removeEffect(type) {
        this.layerState.removeEffect(type);
    ...
    arbitrate() {
        ...
        if (this.playbackActive) {
            ...
            for (const fixtureId of allFixtureIds) {
                ...
                if (chronosData) {
                    ...
                    const chronosHasColor = !!(chronosData.color.r || chronosData.color.g || chronosData.color.b);
        ...
        if (!this._outputEnabled && this.frameNumber % 150 === 0) {
            ...
            console.log(`[ArbitrationDirector] 🚦 ARMED STATE: Output DISABLED | DMX gate active in HAL | Press GO to enable DMX`, { outputEnabled: this._outputEnabled, lastGateChange: last ?? null });
        }
                    ...
                    const srcLabel = src === ControlLayer.MANUAL ? 'MANUAL'
                        : src === ControlLayer.EFFECTS ? 'EFFECTS'
                            : src === ControlLayer.TITAN_AI ? 'TITAN_AI'
                                : `UNKNOWN(${src})`;
                    console.warn(`[📡 GHOST_WHITE] 👻 ${target.fixtureId} ` +
        ...
        for (const e of this.layerState.getActiveEffects())
    ...
    arbitrateFixture(fixtureId, now) {
        ...
        if (prevHadPosition && !nowHasPosition) {
            console.trace(`🚨 [WIRETAP WAVE 2910] TRANSICIÓN: ${fixtureId} pasó de MANUAL a ` +
    ...
    calculatePatternOffset(pattern, now) {
        ...
        if (this.frameNumber % 60 === 0) {
            console.log(`[Pattern] 🔄 type=${pattern.type} speed=${pattern.speed.toFixed(3)}Hz ` +
    ...
    getAdjustedPosition(fixtureId, titanValues, manualOverride, now) {
        ...
        if (pattern) {
            ...
            if (this.frameNumber % 60 === 0) {
                console.log(`[Position] 📍 ${fixtureId.substring(0, 8)} liveCenter=P${liveCenterPan.toFixed(0)}/T${liveCenterTilt.toFixed(0)} ` +
    ...
    getTitanValuesForFixture(fixtureId) {
        ...
        if (intent.movement && fixture) {
            ...
            if (isMover) {
                ...
                if (!mechanic && intent.mechanics) {
                    const rootMech = intent.mechanics;
    ...
    buildGlobalEffectsState() {
        ...
        return {
            ...
            strobeSpeed: effects.find(e => e.type === 'strobe')?.params.speed ?? 0,
