 ...
 * @pure
 */
export function applyGrandMasterAndInhibit(rawDimmer, grandMaster, inhibitLimit) {
 ...
 * @pure
 */
export function safePosition(value, lastKnown) {
 ...
 */
export function resolveHybridBlend(input) {
    ...
    // ── COLOR ────────────────────────────────────────────────────────────────
    const chronosHasColor = chronosColor.r > 0 || chronosColor.g > 0 || chronosColor.b > 0;
    let finalColor;
...
export class MergeStrategyResolver {
    ...
    resolve(channel, candidates) {
        ...
        const strategy = DEFAULT_MERGE_STRATEGIES[channel] ?? 'LTP';
     ...
     */
    resolveWithMode(mode, candidates) {
        switch (mode) {
