...
export class LfxClipInstance {
     ...
     */
    static fromLegacyLfx(input) {
        const safe = (input ?? {});
        const clipBlock = typeof safe['clip'] === 'object' && safe['clip'] !== null
            ? safe['clip']
            : safe;
        const id = typeof clipBlock['id'] === 'string' && clipBlock['id'].length > 0
            ? clipBlock['id']
            : `legacy-${Date.now()}-${Math.floor(Math.random() * 1e6)}`;
        const title = typeof clipBlock['name'] === 'string'
            ? clipBlock['name']
            : typeof clipBlock['title'] === 'string'
                ? clipBlock['title']
                : 'Untitled Clip';
        const author = typeof clipBlock['author'] === 'string' ? clipBlock['author'] : 'unknown';
        // Cognitive DNA legacy
        const dna = typeof clipBlock['cognitiveDNA'] === 'object' && clipBlock['cognitiveDNA'] !== null
            ? clipBlock['cognitiveDNA']
            : null;
        const genome = dna && typeof dna['genome'] === 'object' && dna['genome'] !== null
            ? dna['genome']
            : null;
        const acoTriad = {
            aggression: typeof genome?.['aggression'] === 'number' ? genome['aggression'] : 0.5,
            chaos: typeof genome?.['chaos'] === 'number' ? genome['chaos'] : 0.5,
            organicity: typeof genome?.['organicity'] === 'number' ? genome['organicity'] : 0.5,
        };
        // EnergyZones derivadas del rango legacy
        let energyZones = [];
        const ezr = dna && typeof dna['energyZone'] === 'object' && dna['energyZone'] !== null
            ? dna['energyZone']
            : null;
        if (ezr && isEnergyZone(ezr['min']) && isEnergyZone(ezr['max'])) {
            ...
            energyZones = ENERGY_ZONES.slice(lo, hi + 1);
        }
        ...
        const spatialBehavior = isSpatialBehavior(sb) ? sb : 'static';
        // Safety declaration
        const safety = typeof clipBlock['safetyDeclaration'] === 'object' && clipBlock['safetyDeclaration'] !== null
            ? clipBlock['safetyDeclaration']
            : null;
        const maxStrobeFreqHz = typeof safety?.['maxStrobeFreqHz'] === 'number' ? safety['maxStrobeFreqHz'] : 0;
        // Inferir arquetipo: tags conocidos > simulationMeta flags > utility
        const userArchetype = LfxClipInstance._inferArchetypeFromLegacy(clipBlock);
     ...
     *  - default → 'utility'
     */
    static _inferArchetypeFromLegacy(clipBlock) {
        const tags = Array.isArray(clipBlock['tags']) ? clipBlock['tags'] : [];
        const tagSet = new Set(tags.filter((t) => typeof t === 'string'));
        const sim = typeof clipBlock['simulationMeta'] === 'object' && clipBlock['simulationMeta'] !== null
            ? clipBlock['simulationMeta']
            : null;
        if (tagSet.has('strobe') || sim?.['isStrobe'] === true)
            return 'strobe';
        if (sim?.['isDivineCandidate'] === true)
            return 'divine';
        if (sim?.['isHeavyCandidate'] === true || tagSet.has('heavy'))
            return 'heavy';
        if (tagSet.has('ambient') || tagSet.has('chill'))
            return 'ambient';
        return 'utility';
    }
    // ─── INTERNALS ───────────────────────────────────────────────────────────
