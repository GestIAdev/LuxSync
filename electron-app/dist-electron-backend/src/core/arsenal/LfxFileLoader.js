...
export class LfxFileLoader {
    ...
    async loadAll(directories) {
        ...
        console.log(`[LfxFileLoader 🏛️] Scan complete: scanned=${scanned} accepted=${accepted} ` +
     ...
     */
    _parseAndValidate(raw, filePath, source) {
        let parsed;
        try {
        ...
        const dna = validated.clip.cognitiveDNA;
        const safetyDecl = validated.clip.safetyDeclaration;
        ...
        if (source === 'user') {
            if (dna.genome.aggression > USER_SAFETY_POLICY.MAX_AGGRESSION) {
                console.warn(`[LfxFileLoader ⚠️] USER policy: aggression=${dna.genome.aggression} > ` +
            ...
            if (declaredHz > USER_SAFETY_POLICY.MAX_STROBE_HZ) {
                console.warn(`[LfxFileLoader ⚠️] USER policy: strobeFreq=${declaredHz}Hz > ` +
...
function _isStructurallyValid(clip) {
    if (!clip || typeof clip !== 'object')
        return false;
    if (clip.$schema !== 'hephaestus/v2.1')
        return false;
    if (typeof clip.version !== 'string')
        return false;
    if (typeof clip.checksum !== 'string')
        return false;
    ...
    if (typeof c.durationMs !== 'number' || !Number.isFinite(c.durationMs) || c.durationMs <= 0)
        return false;
    if (typeof c.effectType !== 'string')
        return false;
    if (!c.curves || typeof c.curves !== 'object')
        return false;
    if (!c.staticParams || typeof c.staticParams !== 'object')
        return false;
    if (!Array.isArray(c.tags))
        return false;
    if (!Array.isArray(c.vibeCompat))
        return false;
