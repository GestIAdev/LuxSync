...
const VALID_VISIBILITIES = new Set(['all', 'manual_only']);
/** Valores válidos de `executionHints.intensityScaling`. */
const VALID_INTENSITY_SCALINGS = new Set(['proportional', 'fixed', 'energyDriven']);
/** Valores válidos de `executionHints.overlayMode`. */
const VALID_OVERLAY_MODES = new Set(['absolute', 'relative', 'additive']);
/** Valores válidos de `executionHints.fixtureTargeting`. */
const VALID_FIXTURE_TARGETINGS = new Set([
 ...
 */
function _sanitizeExecutionHints(raw, filePath) {
    if (!raw || typeof raw !== 'object')
        return;
    if (raw.intensityScaling !== undefined && !VALID_INTENSITY_SCALINGS.has(raw.intensityScaling)) {
    ...
    if (raw.overlayMode !== undefined && !VALID_OVERLAY_MODES.has(raw.overlayMode)) {
    ...
    if (raw.fixtureTargeting !== undefined && !VALID_FIXTURE_TARGETINGS.has(raw.fixtureTargeting)) {
...
export class LfxFileLoader {
    ...
    _parseAndValidateV3(raw, filePath, source) {
        ...
        if (rawDna && rawDna.visibility !== undefined && !VALID_VISIBILITIES.has(rawDna.visibility)) {
        ...
        _sanitizeExecutionHints(clip.executionHints, filePath);
        ...
        const v3File = {
            ...
            clip: {
                ...
                executionHints: clip.executionHints || undefined,
                schemaVersion: '3.0',
