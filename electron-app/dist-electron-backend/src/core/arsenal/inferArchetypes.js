...
// ════════════════════════════════════════════════════════════════════════════
import { USER_ARCHETYPES, ARCHETYPE_BIAS_MAP, } from './LfxClipInstance';
// ─── CONSTANTES ─────────────────────────────────────────────────────────────
...
const MAX_WEIGHTED_DISTANCE = weightedDistance({ aggression: 0, chaos: 0, organicity: 0 }, { aggression: 1, chaos: 1, organicity: 1 });
/**
 ...
 */
function isContainedByBias(aco, arch) {
    const bias = ARCHETYPE_BIAS_MAP[arch];
    if (bias.aggressionMin != null && aco.aggression < bias.aggressionMin)
        return false;
    if (bias.aggressionMax != null && aco.aggression > bias.aggressionMax)
        return false;
    if (bias.chaosMin != null && aco.chaos < bias.chaosMin)
        return false;
    if (bias.chaosMax != null && aco.chaos > bias.chaosMax)
        return false;
    if (bias.organicityMin != null && aco.organicity < bias.organicityMin)
        return false;
    if (bias.organicityMax != null && aco.organicity > bias.organicityMax)
        return false;
    return true;
}
/**
 ...
 */
function zoneCompatibility(zones, arch) {
    if (!zones || zones.length === 0)
        return 0;
    const allowed = ARCHETYPE_BIAS_MAP[arch].allowedZones;
    if (!allowed || allowed.length === 0)
        return 0.05; // utility bonus pequeño
 ...
 */
export function inferArchetypeFromACO(aco, zones) {
    ...
    for (const arch of USER_ARCHETYPES) {
        ...
        if (isContainedByBias(safe, arch))
            confidence += 0.10;
    ...
    const ranking = Object.freeze(candidates.map(c => Object.freeze({
 ...
 */
export function inferArchetype(aco, zones) {
 ...
 */
export function inferArchetypesBatch(items) {
 ...
 */
export function semanticLabel(axis, value) {
