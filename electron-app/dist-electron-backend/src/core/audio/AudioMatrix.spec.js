...
import { createSharedRingBuffer } from './SharedRingBuffer';
...
class StubProvider {
    ...
    pushAudio(buffer, sampleRate = 44100) {
        this.onAudioData?.(buffer, sampleRate);
    }
    async initialize(_config) { }
    async start() { this.setState('streaming'); }
    async stop() { this.setState('ready'); }
    dispose() { this.setState('disposed'); }
...
// HELPERS
// ============================================
function makeSignal(length, amplitude = 1.0) {
    const buf = new Float32Array(length);
    buf.fill(amplitude);
    return buf;
}
function sumAbsolute(buf) {
    let s = 0;
...
let writer;
let ringtWrites = [];
function captureWrites(w) {
    const original = w.write.bind(w);
    vi.spyOn(w, 'write').mockImplementation((samples) => {
...
describe('AudioMatrix -- audio routing strict gating', () => {
    ...
    beforeEach(() => {
        ...
        writer = rb.writer;
        writtenSamples = 0;
        vi.spyOn(writer, 'write').mockImplementation((buf) => {
