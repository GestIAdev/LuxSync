...
// HELPERS
// ============================================
function parseBuffer(hexOrArray) {
...
function float32BEToBytes(v) {
    const b = Buffer.alloc(4);
    b.writeFloatBE(v, 0);
    return [...b];
}
function int32BEToBytes(v) {
    const b = Buffer.alloc(4);
    b.writeInt32BE(v, 0);
    return [...b];
}
...
describe('parseOSCMessage + buildOSCMessage -- valid roundtrip', () => {
    it('float32 argument roundtrip', () => {
        ...
        expect(result.args[0].value).toBeCloseTo(1.5, 4);
    });
    it('int32 argument roundtrip', () => {
        const args = [{ type: 'i', value: 42 }];
    ...
    it('multi-arg roundtrip: f + i + s', () => {
        ...
        expect(result.args[0].value).toBeCloseTo(0.75, 4);
        expect(result.args[1].value).toBe(12345);
        expect(result.args[2].value).toBe('vibe');
    });
    ...
    it('LuxSync publishState message structure roundtrip', () => {
        ...
        expect(result.args[1].value).toBeCloseTo(128.0, 1);
        expect(result.args[2].value).toBe(1);
        expect(result.args[3].value).toBe('verse');
    });
    ...
    it('string with padding alignment preserved (length not multiple of 4)', () => {
        ...
        for (const s of ['a', 'ab', 'abc', 'abcd', 'abcde']) {
            ...
            const result = parseOSCMessage(buf);
            expect(result.args[0].value).toBe(s);
        }
    });
    it('float32 precision: 0.0 and 1.0 roundtrip exactly', () => {
        for (const v of [0.0, 1.0, -1.0]) {
            ...
            const result = parseOSCMessage(buf);
            expect(result.args[0].value).toBeCloseTo(v, 6);
        }
    });
    it('large blob (4096 bytes Float32) roundtrip', () => {
        ...
        const result = parseOSCMessage(buf);
        const parsed = result.args[0].value;
