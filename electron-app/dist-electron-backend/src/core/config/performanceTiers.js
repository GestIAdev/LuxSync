...
export const PERFORMANCE_TIERS = ['hq', 'balanced', 'eco'];
// ═══════════════════════════════════════════════════════════════════════════════
// VALIDATION
// ═══════════════════════════════════════════════════════════════════════════════
/**
 ...
 */
export function isPerformanceTier(value) {
    return typeof value === 'string' && PERFORMANCE_TIERS.includes(value);
}
/**
 * Coerce an unknown value to a valid tier, falling back to `fallback`.
 * Used when reading a possibly-corrupt config file.
 */
export function coercePerformanceTier(value, fallback = 'hq') {
 ...
 */
export function createDefaultPerformanceProfile() {
    return {
        tier: 'hq',
        userConfirmed: false,
        skipLauncher: false,
 ...
 */
export function scoreHardware(hw) {
    ...
    if (hw.cpuCores <= 2)
        return 'eco';
    // ── Mid range ──
    if (hw.totalMemoryGB <= 8 || hw.cpuCores <= 4)
        return 'balanced';
    return 'hq';
}
// ═══════════════════════════════════════════════════════════════════════════════
 ...
 */
export function shouldShowLauncher(profile, argv = typeof process !== 'undefined' ? process.argv : []) {
    // Dev / support escape hatch — always wins.
    if (argv.includes(FORCE_LAUNCHER_FLAG))
        return true;
    ...
    if (profile.skipLauncher && profile.userConfirmed)
        return false;
    return true;
}
