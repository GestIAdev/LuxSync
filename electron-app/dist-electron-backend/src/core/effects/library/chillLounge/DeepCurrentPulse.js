...
export class DeepCurrentPulse extends BaseEffect {
    constructor(config) {
        ...
        this.effectType = 'deep_current_pulse';
        this.name = 'Deep Current Pulse';
    ...
    getOutput() {
        ...
        const getFlowIntensity = (zonePos) => {
            const distance = Math.abs(zonePos - (wavePosition % 1.5));
            if (distance > waveWidth)
                return 0.1; // Siempre hay algo de corriente
