 ...
 */
function channelTypeToIntentKey(type) {
...
// ═══════════════════════════════════════════════════════════════════════════
function makeInputDmxNode(channelType, channelIndex, yPosition) {
    const nodeId = `in-${channelType}-${channelIndex}`;
    const config = {
        nodeType: 'input_dmx',
...
function makeOutputDmxNode(channel, yPosition) {
    const nodeId = `out-${channel.type}-${channel.index}`;
    const config = {
        nodeType: 'output_dmx',
...
function makeEdge(sourceNodeId, targetNodeId, edgeIndex) {
    const edgeId = `edge-${edgeIndex.toString().padStart(3, '0')}`;
    return {
        id: edgeId,
...
function makeEmptyMeta(autoMigrated, dmxFootprint) {
    return {
...
export class NodeGraphBuilder {
     ...
     */
    static fromChannels(channels, meta) {
