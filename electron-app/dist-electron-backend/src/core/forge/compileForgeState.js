...
// ═══════════════════════════════════════════════════════════════════════════
function validateState(state, errors, warnings) {
...
function makeOutputDmxNode(ch, rowIndex, aetherNodeId, aetherZone, cellLabel) {
    const nodeId = `out-${ch.type}-${ch.index}`;
    const config = {
        nodeType: 'output_dmx',
        channelType: ch.type,
        dmxOffset: ch.index,
...
function makeEdge(inNodeId, outNodeId, idx) {
    return {
 ...
 */
function compileNodeGraph(state, resolvedChannels) {
    const nodes = [];
    const edges = [];
    // Mapa rápido channelIdx → célula propietaria
    const channelToCell = new Map();
    for (const cell of state.cells) {
        for (const idx of cell.channelIndices) {
    ...
    for (const ch of sortedChannels) {
        if (ch.type === 'unknown')
            continue; // canales unknown = ignorados
...
// ═══════════════════════════════════════════════════════════════════════════
function buildFixtureId(manufacturer, name) {
 ...
 */
export function compileForgeState(state) {
    const errors = [];
    const warnings = [];
    // ── Fase A: Validación ────────────────────────────────────────────────
