...
// ═══════════════════════════════════════════════════════════════════════════
// ── 0: noop ──────────────────────────────────────────────────────────────
function op_noop(_wire, _state, _instr, _ctx) { }
...
function op_input_dmx(_wire, _state, _instr, _ctx) { }
// ── 2: input_audio_band — passthrough ────────────────────────────────────
function op_input_audio_band(_wire, _state, _instr, _ctx) { }
// ── 3: input_beat — passthrough ──────────────────────────────────────────
function op_input_beat(_wire, _state, _instr, _ctx) { }
// ── 4: input_bpm — passthrough ───────────────────────────────────────────
function op_input_bpm(_wire, _state, _instr, _ctx) { }
// ── 5: input_energy — passthrough ────────────────────────────────────────
function op_input_energy(_wire, _state, _instr, _ctx) { }
// ── 6: input_constant ────────────────────────────────────────────────────
// params[0] = constant value
function op_input_constant(wire, _state, instr, _ctx) {
    wire[instr.outputOffset] = instr.params[0];
}
// ── 7: input_time — passthrough ──────────────────────────────────────────
function op_input_time(_wire, _state, _instr, _ctx) { }
// ── 8: proc_lfo ──────────────────────────────────────────────────────────
// params: [waveform, frequencyHz, syncToBpm(0/1), bpmDivisor, phase]
// state:  [phaseAccumulator]
function op_proc_lfo(wire, state, instr, ctx) {
...
// state:  [previousValue]
function op_proc_smooth(wire, state, instr, ctx) {
...
function op_proc_map_range(wire, _state, instr, _ctx) {
...
// params: [operation]
// inputs: [a, b]
function op_proc_math(wire, _state, instr, _ctx) {
    ...
    if (op === 0)
        ...
        result = b !== 0 ? a / b : 0; // divide
    else if (op === 4)
        result = b !== 0 ? a % b : 0; // modulo
    else if (op === 5)
        result = Math.pow(a, b); // power
    else
        result = a;
...
// ── 12: proc_clamp ───────────────────────────────────────────────────────
// params: [min, max]
function op_proc_clamp(wire, _state, instr, _ctx) {
...
function op_proc_delay(wire, state, instr, _ctx) {
...
function op_proc_merge(wire, _state, instr, _ctx) {
    ...
    for (let i = 1; i < count; i++) {
        ...
        if (strategy === 0)
            result = v > result ? v : result; // max
        else if (strategy === 1)
            result = v < result ? v : result; // min
        else if (strategy === 2)
            result += v; // accumulate for avg
        else if (strategy === 3)
            result += v; // sum
    }
    if (strategy === 2 && count > 1)
        result /= count; // average
    wire[instr.outputOffset] = result;
}
// ── 15: proc_invert ──────────────────────────────────────────────────────
function op_proc_invert(wire, _state, instr, _ctx) {
...
function op_proc_curve(wire, _state, instr, _ctx) {
...
// state:  [lastOutput]
function op_logic_threshold(wire, state, instr, _ctx) {
...
// ── 18: logic_gate ───────────────────────────────────────────────────────
// inputs: [signal, gate]
function op_logic_gate(wire, _state, instr, _ctx) {
...
function op_logic_switch(wire, _state, instr, _ctx) {
    const threshold = instr.params[0];
...
// ── 20: logic_and ────────────────────────────────────────────────────────
// inputs: [a, b]
function op_logic_and(wire, _state, instr, _ctx) {
    const a = wire[instr.inputOffset];
    const b = instr.inputCount > 1 ? wire[instr.inputOffset + 1] : 0.0;
...
// ── 21: logic_or ─────────────────────────────────────────────────────────
// inputs: [a, b]
function op_logic_or(wire, _state, instr, _ctx) {
    const a = wire[instr.inputOffset];
    const b = instr.inputCount > 1 ? wire[instr.inputOffset + 1] : 0.0;
    wire[instr.outputOffset] = (a > 0.5 || b > 0.5) ? 1.0 : 0.0;
}
...
// state:  [count]
function op_logic_counter(wire, state, instr, _ctx) {
...
function op_output_dmx(wire, _state, instr, _ctx) {
