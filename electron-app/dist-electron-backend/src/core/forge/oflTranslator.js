...
function extractWheels(oflJson, channels) {
    ...
    const colorSlots = slots.filter((s) => Array.isArray(s?.colors) && s.colors.length > 0);
    ...
    if (hasRgb && hasW)
        ...
        colorEngine = 'cmy';
    else if (hasColorWheelChannel && colorSlots.length > 0)
        colorEngine = 'wheel';
    else if (hasRgb || hasCmy || hasColorWheelChannel)
        colorEngine = 'hybrid';
    if (colorSlots.length === 0 && colorEngine === 'none')
        return null;
    const colors = colorSlots.map((s, i) => {
