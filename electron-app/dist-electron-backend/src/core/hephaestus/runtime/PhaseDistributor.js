...
export class PhaseDistributor {
     ...
     */
    static resolve(fixtureIds, config, durationMs) {
        ...
        for (let i = 0; i < N; i++) {
            ...
            const offset = PhaseDistributor.computeOffset(localIndex, localN, spreadMs, config.symmetry, config.direction);
            results[i] = {
     ...
     */
    static computeOffset(localIndex, localN, spreadMs, symmetry, direction) {
        if (localN <= 1)
            return 0;
        let offset;
        switch (symmetry) {
