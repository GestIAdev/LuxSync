...
export class SeleneTitanConscious extends EventEmitter {
    ...
    constructor(config = {}) {
        ...
        // ═══════════════════════════════════════════════════════════════════════
        this.FFT_XRAY_BUFFER_SIZE = 180; // 3s @ 60fps
        this.FFT_XRAY_LOG_INTERVAL_MS = 3000; // Log cada 3s
        this._fftXRayLow = new Float32Array(this.FFT_XRAY_BUFFER_SIZE);
        this._fftXRayMid = new Float32Array(this.FFT_XRAY_BUFFER_SIZE);
        this._fftXRayHigh = new Float32Array(this.FFT_XRAY_BUFFER_SIZE);
        this._fftXRayTotal = new Float32Array(this.FFT_XRAY_BUFFER_SIZE);
        this._fftXRayHead = 0;
        this._fftXRayCount = 0;
        this._fftXRayLastLogMs = 0;
        ...
        this._lastCooldownLog = 0;
        this._lastV3BassGateLog = 0;
    ...
    async process(titanState) {
        ...
        if (this.stats.framesProcessed % 600 === 0) {
            ...
            for (const [key, ts] of this._dnaLogThrottle) {
                if (ts < cutoff)
                    this._dnaLogThrottle.delete(key);
            }
            for (const key in this.lastGatekeeperLogs) {
                if (this.lastGatekeeperLogs[key] < cutoff)
                    delete this.lastGatekeeperLogs[key];
            }
            for (const key in this._constitutionLogThrottle) {
                if (this._constitutionLogThrottle[key] < cutoff)
                    delete this._constitutionLogThrottle[key];
            }
        }
        ...
        this.updateHistory(pattern);
        {
            ...
            if (this._v3Ignite) {
                ...
                if (now - this._lastSuppressedLog > 1000) {
                    ...
                    console.log(`[SeleneTitanConscious 🌊] V3 IGNITE: C=${lv.confidence.toFixed(3)} ` +
        ...
        // 🩸 WAVE 7574: ENDURECIDO — bass gate 0.35→0.45, Z-floor 1.0→1.5.
        //   0.35 es mantequilla: la resonancia del autotune grave vive ahí.
        //   0.45 deja la voz nasal fuera. Z=1.5 exige anomalía real (drop).
        // ─────────────────────────────────────────────────────────────────────
        ...
        if (this._lastLiquidVerdict) {
            ...
            if (this._lastLiquidVerdict.ignite && finalOutput.effectDecision) {
                ...
                if (now - this._lastV3IgnitionLog > 1000) {
                    console.log(`[SeleneTitanConscious 🌊✅] V3 IGNITION MATERIALIZED: ` +
                ...
                if (now - this._lastSuppressedLog > 1000) {
                    console.log(`[SeleneTitanConscious 🌊⏸️] V3 IGNITE SUPPRESSED: no valid effect candidate ` +
     ...
     */
    _fftXRayUpdate(low, mid, high, total, nowMs) {
        ...
        this._fftXRayHead++;
        if (this._fftXRayCount < this.FFT_XRAY_BUFFER_SIZE)
            this._fftXRayCount++;
        if (nowMs - this._fftXRayLastLogMs >= this.FFT_XRAY_LOG_INTERVAL_MS) {
            ...
            console.log(`[FFT X-RAY 🎧] 3s Avg -> LOW: ${avgLow.toFixed(3)} | MID: ${avgMid.toFixed(3)} | ` +
        ...
        if (output.effectDecision) {
            ...
            if (divineArsenal && divineArsenal.length > 0) {
                ...
                if (availableWeapon) {
                    ...
                    console.log(`[SeleneTitanConscious ${originEmoji}] ${originLabel}: Selected ${effectDisplayName(availableWeapon)} from [${divineArsenal.map(effectDisplayName).join(', ')}]`);
                }
                else {
            ...
            if (availability.available && output.effectDecision) {
                ...
                if (nowTime - (this.lastGatekeeperLogs[gatekeeperKey] ?? 0) > 3000) {
                    console.log(`[SeleneTitanConscious] 🚪 GATEKEEPER BLOCKED: ${effectDisplayName(intent)} | ${availability.reason}`);
