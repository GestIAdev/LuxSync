...
export class EffectDreamSimulator {
    constructor() {
        ...
        this.PRE_BUFFER_MIN_TIME_MS = 2000; // Solo buffer si > 2s hasta evento
        this.PRE_BUFFER_MAX_AGE_MS = 5000; // Expira después de 5s
    ...
    filterByZone(effects, zone) {
        ...
        if (filtered.length < 4) {
            ...
            if (zoneIdx > 0)
                adjacentZones.add(ZONE_ORDER[zoneIdx - 1]);
            if (zoneIdx < ZONE_ORDER.length - 1)
                adjacentZones.add(ZONE_ORDER[zoneIdx + 1]);
            ...
            console.log(`[DREAM_SIMULATOR] 🧘 Zone ${zone} pool too small (${filtered.length} < 4), ` +
    ...
    generateCandidates(state, prediction, context) {
        ...
        if (!this._lastFilterAuditTs || Date.now() - this._lastFilterAuditTs > 30000) {
            ...
            console.log(`[DREAM_AUDIT 🔍] vibe="${state.vibe}" zone=${projectedZone} pressure=${currentPressure.toFixed(3)} | ` +
        ...
        const activeZoneSet = new Set(_hwManifest
        ...
        if (prediction.confidence > 0.6 && prediction.predictionType !== 'none') {
            console.log(`[DREAM_SIMULATOR] 🔮 CASSANDRA: type=${prediction.predictionType} ` +
        ...
        if (candidates.length === 0 && pressureFilteredEffects.length > 0) {
            ...
            for (const effect of pressureFilteredEffects) {
                ...
                if (entry.simMeta.isStrobe && zScore <= 0 && !relaxGuardsForFuture)
                    continue;
                const intensity = this.calculateIntensity(prediction.predictedEnergy, effect);
                const isSuggestedByOracle = prediction.suggestedEffects?.some(suggested => effect.includes(suggested) || suggested.includes(effect)) ?? false;
     ...
     */
    _isTargetingAvailable(targeting, activeZones) {
        switch (targeting) {
    ...
    projectBeauty(effect, state, context) {
        ...
        if (!simMeta)
            return 0.5; // No en registry → neutro
    ...
    * @returns { relevance: 0-1, distance: 0-√3, targetDNA: TargetDNA }
    ...
    calculateVibeCoherence(effect, context) {
        ...
        if (!entry)
            return 0.4; // Unknown effect — neutral
        ...
        if (compatibleVibes.length === 0)
            return 0.6; // No vibe restriction — neutral
        if (compatibleVibes.includes(context.vibe))
            return 0.85; // Family
    ...
    rankScenarios(scenarios, prediction) {
        ...
        const predType = prediction.predictionType ?? 'none';
        console.log(`[DREAM_RANKING] 🏆 TOP 5 (${scored.length} total) | pred=${predType} conf=${prediction.confidence.toFixed(3)}:\n` +
            top5.map((s, i) => {
                const sc = s.scenario;
