 ...
 * ═══════════════════════════════════════════════════════════════════════════
 */
import { senseBeauty } from '../sense/BeautySensor';
// ═══════════════════════════════════════════════════════════════════════════
// CONSTANTS
// ═══════════════════════════════════════════════════════════════════════════
 ...
 */
export function dream(state, pattern, currentBeauty, config = {}) {
    ...
    for (const scenarioType of prioritizedTypes) {
        if (scenarios.length >= cfg.maxScenarios)
            break;
        const scenario = simulateScenario(scenarioType, state, pattern, currentBeauty);
        if (scenario) {
    ...
    if (scenarios.length < cfg.maxScenarios) {
        const hueExplorations = exploreHueShifts(state, pattern, currentBeauty, cfg.maxScenarios - scenarios.length);
    ...
    const { recommendation, reason } = determineRecommendation(bestScenario, currentBeauty, cfg);
    const result = {
 ...
 * Reset para tests
 */
export function resetDreamEngine() {
    lastDream = null;
    dreamCount = 0;
}
// ═══════════════════════════════════════════════════════════════════════════
// SCENARIO GENERATION
// ═══════════════════════════════════════════════════════════════════════════
function determineContext(state, pattern) {
    if (pattern.isBuilding)
        return 'building';
    if (pattern.energyPhase === 'valley')
        return 'recovering'; // Valley = post-drop recovery
    if (state.smoothedEnergy < 0.3)
        return 'low_energy';
    return 'stable';
}
function simulateScenario(type, state, pattern, currentBeauty) {
    const palette = state.currentPalette;
    if (!palette)
        return null;
    let projectedPalette;
    let decision;
    let description;
    let riskLevel;
    switch (type) {
        ...
        case 'energy_prepare': {
            ...
            projectedPalette = projectPaletteWithSaturation(projectPaletteWithHue(palette, (palette.primary.h + 10) % 360), 0.2);
...
function exploreHueShifts(state, pattern, currentBeauty, maxCount) {
    const palette = state.currentPalette;
    if (!palette)
        return [];
    const explorations = [];
    ...
    for (const delta of HUE_EXPLORATION_DELTAS) {
        if (explorations.length >= maxCount)
            break;
        if (delta === 0)
            continue; // Ya cubierto por hold_steady
        const newHue = (palette.primary.h + delta + 360) % 360;
...
// ═══════════════════════════════════════════════════════════════════════════
function projectPaletteWithHue(palette, newHue) {
...
function projectPaletteWithSaturation(palette, boost) {
    const clampS = (s) => Math.max(0, Math.min(1, s + boost));
    return {
...
function projectPaletteWithContrast(palette, boost) {
...
function selectOptimalHueDelta(palette, pattern) {
...
// ═══════════════════════════════════════════════════════════════════════════
function calculateScenarioScore(scenario, cfg) {
    return (scenario.beautyDelta * cfg.beautyWeight +
        scenario.riskLevel * cfg.riskWeight +
        scenario.projectedConsonance * cfg.consonanceWeight) * scenario.simulationConfidence;
}
function determineRecommendation(bestScenario, currentBeauty, cfg) {
    if (!bestScenario) {
