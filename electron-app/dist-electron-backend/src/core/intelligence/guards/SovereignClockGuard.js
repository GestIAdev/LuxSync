...
function selectRerouteCandidate(arsenal, effectHistory, now) {
    ...
    const notInCooldown = lighter.filter(e => !effectHistory.some(h => h.type === e.id && (now - h.timestamp) < 8000));
 ...
 */
export function selectUpgradeCandidate(arsenal, effectHistory, now) {
