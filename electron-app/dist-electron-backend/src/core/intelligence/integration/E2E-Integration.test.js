 ...
 * @date 2026-01-20
 */
import { DreamEngineIntegrator } from './DreamEngineIntegrator';
import { effectBiasTracker } from '../dream/EffectBiasTracker';
// ═══════════════════════════════════════════════════════════════════════════
// TEST SUITE
// ═══════════════════════════════════════════════════════════════════════════
export async function runE2EIntegrationTests() {
    console.log('\n');
    ...
    try {
        ...
        if (decision.ethicalVerdict?.violations.some((v) => v.value === 'audience_safety')) {
    ...
    try {
        ...
        const results = await Promise.all(contexts.map(ctx => integrator.executeFullPipeline(ctx)));
