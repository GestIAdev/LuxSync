...
export class CrestDetector {
    constructor() {
        this._base = 0; // B(t) — baseline en dominio log
        this._muL = 0; // E[L] — media del residuo
        this._madL = 0; // E[|L−μ|] — dispersión robusta
        this._rate = 0; // R(t) — eventos por segundo
        this._armed = 0; // estado del Schmitt trigger (0/1)
        this._tLast = -1e9; // timestamp del último evento aceptado (s)
        this._event = 0; // 1 en el frame en que dispara un evento
        this._theta = LN2; // umbral efectivo actual (telemetría)
    }
    /**
    ...
    get event() { return this._event === 1; }
...
export class MultiBandCrestDetector {
    ...
    get event() { return this._event === 1; }
    get rate() { return this._rFused; }
    /** Tasa por banda (eventos/s) — telemetría */
    bandRate(k) { return this._r[k]; }
    reset() {
        this._b.fill(0);
        this._mu.fill(0);
        this._mad.fill(0);
