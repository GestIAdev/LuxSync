...
export class StateCouplingEnforcer {
    enforce(zone, phase) {
        ...
        if (zone.label === 'silence' && phase.phase !== 'silence') {
            return {
                zone,
                phase: this.overridePhase(phase, 'silence', `COUPLING ENFORCED: zone=silence but phase=${phase.phase}. Overridden to silence.`),
                corrected: true,
            };
        }
        ...
        if ((zone.label === 'valley' || zone.label === 'ambient') &&
            ...
            return {
                zone,
                phase: this.overridePhase(phase, 'valley', `COUPLING ENFORCED: zone=${zone.label} but phase=climax. Overridden to valley. The music does not support a climax.`),
                corrected: true,
            };
        }
        ...
        if ((zone.label === 'peak' || zone.label === 'intense') &&
            ...
            return {
                zone,
                phase: this.overridePhase(phase, 'building', `COUPLING ENFORCED: zone=${zone.label} but phase=${phase.phase}. Overridden to building. High energy detected but narrative phase lagged.`),
                corrected: true,
            };
        }
        return { zone, phase, corrected: false };
    }
    overridePhase(original, newPhase, reason) {
        return {
            ...original,
            phase: newPhase,
            verdict: 'OVERRIDDEN',
