    ...
    validate(proposedSection, evidence, zone) {
        ...
        if (proposed === 'drop' || proposed === 'chorus') {
            ...
            if (isClassicalClimax || isSustainedClimax) {
                return this.accept(proposed, 'climax', ev, `Climax validated: ${isSustainedClimax ? `sustained E=${ev.eTotal.toFixed(2)}` : `Z_total=${ev.zTotal.toFixed(1)} Z_low=${ev.zLow.toFixed(1)}`}`);
            }
            if (isTexturalClimax) {
                return {
                    phase: 'textural',
        ...
        if (proposed === 'buildup') {
            if (ev.energyDelta > 0.02 && ev.zTotal > -0.5) {
                ...
                if (ev.eTotal > 0.65 && ev.zTotal > +0.3) {
                    return this.accept(proposed, 'climax', ev, `Buildup overridden to climax: sustained E=${ev.eTotal.toFixed(2)} Z=${ev.zTotal.toFixed(1)} despite rising ΔE`);
                }
            ...
            if (ev.eTotal > 0.65 && ev.zTotal > +0.3) {
                return this.accept(proposed, 'climax', ev, `Buildup overridden to climax: sustained E=${ev.eTotal.toFixed(2)} Z=${ev.zTotal.toFixed(1)} with stable ΔE`);
            }
            return {
