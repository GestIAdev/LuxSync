...
// ═══════════════════════════════════════════════════════════════════════════
// WAVE 2200.3 definía una lista hardcodeada de 8 efectos "nucleares".
// WAVE 4843 la destruye: la pesadez de un efecto viene de su .lfx, concretamente
// de `simulationMeta.isHeavyCandidate`. El Registry es la única fuente de verdad.
//
// Regla de negocio preservada: un efecto "heavy" tiene `isHeavyCandidate: true`
// en su JSON y por tanto NO se dispara en buildups — espera al clímax.
//
// Para los efectos strobe (isStrobe: true), la restricción de sección aplica igualmente:
// un strobe pesado declarado `validSections: ['drop', 'peak']` se bloquea en buildup
// usando `isEffectAllowedInSection()`. isHeavyEffect() cubre ambos semánticamente.
// ═══════════════════════════════════════════════════════════════════════════
/**
 ...
 */
function isHeavyEffect(effectId) {
 ...
 */
function updateDropLock(currentSection) {
...
function determineDecisionType(inputs) {
    ...
    if (activeDictator) {
        ...
        if (zone === 'silence' || zone === 'valley') {
            ...
            if (!energyGateOpen) {
                ...
                const kickThreshold = (maxHistoric ?? 0) * 0.75;
                const hasHeavyKick = lowBand >= kickThreshold;
                console.log(`[DecisionMaker 🌴] DIVINE BLOCKED (SpectralGate): ${!hasHeavyKick ? 'Low-Band Insufficient' : 'Vocals Eclipse Beat'} | ` +
            ...
            console.log(`[DecisionMaker 🌩️] DIVINE SUPPRESSED: Z=${currentZ.toFixed(2)}σ but rawEnergy=${effectiveEnergy.toFixed(2)} < ${DIVINE_ENERGY_GATE} ` +
    ...
    if (huntDecision.worthiness >= WORTHINESS_THRESHOLD && huntDecision.confidence > 0.50) {
        if (fuzzyBlockedByBuildup) {
            console.log(`[DecisionMaker 🛡️] HUNT BUILDUP WALL: worthiness=${huntDecision.worthiness.toFixed(2)} ` +
...
function calculateCombinedConfidence(inputs, cfg) {
    ...
    // Ponderación
    let combined = huntConf * cfg.huntWeight +
...
function rankArsenalByDiversity(arsenal) {
    ...
    console.log(`[DecisionMaker 🎲] DIVERSITY SELECT: winner=${winner.id} score=${winner.score.toFixed(3)} ` +
        `from [${arsenal.join(', ')}]`);
    ...
    if (prediction.probability > 0.7 && isDropImminent) {
        ...
        if (!dropAbsGateOpen) {
            if (!dropEnergyGateOpen) {
                ...
                throttledLog('drop_blocked_abs_gate', `[DecisionMaker 🐘] DROP BLOCKED (AbsGate): E=${dropRawEnergy.toFixed(2)} < ` +
                    `${dropGateThreshold.toFixed(2)} (60% of max=${(dropMaxHistoric ?? 0).toFixed(2)}) → no heavy fire in a valley`, 1000);
            }
            else {
                ...
                throttledLog('drop_blocked_spectral_gate', `[DecisionMaker 🌴] DROP BLOCKED (SpectralGate): ${!hasHeavyKick ? 'Low-Band Insufficient' : 'Vocals Eclipse Beat'} | ` +
                    `LOW=${lowBand.toFixed(3)} MID=${midBand.toFixed(3)}`, 1000);
            }
        ...
        if (dropAbsGateOpen) {
            ...
            const dropArsenal = _registryDivineForDrop.map(e => e.id);
            if (dropArsenal.length === 0) {
                ...
                const isTechnoVibeForDrop = vibeId === 'techno-club' || vibeId === 'hard-techno' || vibeId?.includes('techno') || false;
                ...
                if (isHeavyEffect(suggestedEffect) && currentZ < antiFakeThreshold) {
                    ...
                    throttledLog('anti_fake_drop', `[DecisionMaker 🛡️] ANTI-FAKE-DROP (${isLatinoVibe ? 'LATINO' : 'STANDARD'}): "${suggestedEffect}" ABORTED — ` +
                        `Z=${currentZ.toFixed(2)}σ < ${antiFakeThreshold} (energy insufficient for heavy arsenal)`, 1000);
