 ...
 */
export function validateColorDecision(decision, constitution) {
    ...
    if (decision.suggestedHue !== undefined) {
        ...
        for (const [min, max] of forbiddenRanges) {
            if (isHueInRange(decision.suggestedHue, min, max)) {
                ...
                const nearestAllowed = findNearestAllowedHue(decision.suggestedHue, constitution.allowedHueRanges ?? [[0, 360]], forbiddenRanges);
                violations.push({
                    type: 'forbidden_hue',
    ...
    if (decision.suggestedStrategy !== undefined) {
        ...
        if (constitution.forceStrategy &&
            ...
            corrected.suggestedStrategy = constitution.forceStrategy;
        }
    }
    return {
 ...
 */
export function enforceConstitution(decision, constitution) {
    if (!decision)
        return null;
 ...
 */
function findNearestAllowedHue(forbiddenHue, allowedRanges, forbiddenRanges) {
    // Normalizar
 ...
 */
function isHueInAnyForbiddenRange(hue, forbiddenRanges) {
    for (const [min, max] of forbiddenRanges) {
