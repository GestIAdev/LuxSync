...
export class FixtureProfileResolver {
    ...
    resolveFixtureDefinitionForAether(fixture) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    normalizeFixtureDefinitionForAether(definition, fixture, profileId) {
        ...
        if (hasDuplicateIndices) {
            console.warn(`[FixtureProfileResolver] ⚠️ WAVE 4674: fixture "${fixture.id ?? profileId}" tiene índices de canal duplicados ` +
    ...
    // ── FixtureV2 builder ────────────────────────────────────────────────────
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    buildFixtureV2ForAether(fixture, definition) {
    ...
    // ── Profile ID ───────────────────────────────────────────────────────────
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolveFixtureProfileId(fixture) {
    ...
    normalizeFixtureChannelIndex(rawIndex, fallback) {
    ...
    normalizeFixtureChannelType(type, name) {
    ...
    inferFixtureChannelTypeFromName(name) {
    ...
    resolveAetherChannelDefaultValue(type, value) {
