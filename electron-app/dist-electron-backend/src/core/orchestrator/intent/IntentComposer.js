...
export class IntentComposer {
     ...
     */
    compose(effectOutput, fixtures, chronosIds = new Set(), outMap = new Map()) {
     ...
     */
    _composeFromZoneOverrides(effectOutput, fixtures, chronosIds, outMap, mixBus, globalComposition) {
        ...
        for (const zoneId in zoneOverrides) {
            if (!Object.prototype.hasOwnProperty.call(zoneOverrides, zoneId))
                continue;
            const zoneData = zoneOverrides[zoneId];
            for (const fixture of fixtures) {
                if (!fixture.id)
                    continue;
                if (chronosIds.has(fixture.id))
                    continue;
     ...
     */
    _composeGlobalFallback(effectOutput, fixtures, chronosIds, outMap, mixBus, globalComposition) {
        if (effectOutput.dimmerOverride === undefined)
            return;
        ...
        if (effectOutput.colorOverride) {
            color = hslToRgb(effectOutput.colorOverride.h, effectOutput.colorOverride.s, effectOutput.colorOverride.l);
        }
        for (const fixture of fixtures) {
            if (!fixture.id)
                continue;
