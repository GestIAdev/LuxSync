...
const DMX_OUTPUT_ZEROS = new Array(512).fill(0);
export class BroadcastManager {
     ...
     */
    accumulatePeaks(fixtureStates, chronosPlaying) {
        if (chronosPlaying)
            return;
        ...
        for (let i = 0; i < fixtureStates.length; i++) {
            ...
            if (f.dimmer > prev)
                peakHoldMap.set(id, f.dimmer);
        }
    }
    /**
     * WAVE 2510/4559: Emit hot-frame with fixture dynamic data at 44Hz.
     */
    emitHotFrame(fixtureStates, now, engineAudioMetrics, bass, mid, high, energy, chronosPlaying) {
     ...
     */
    emitFullTruth(fixtureStates, now, engineAudioMetrics, bass, mid, high, energy, chronosPlaying, context, intent) {
        ...
        if (!logManager.getBroadcastCallback() || !shouldBroadcastFullTruth)
            return;
     ...
     */
    publishOSCState(energy, context, beatState) {
        const { oscProvider, engine, frameCount } = this.ctx;
        if (!oscProvider || frameCount % 3 !== 0)
            return;
...
BroadcastManager.HOT_FRAME_DIVIDER = 2; // 44Hz → 22Hz
BroadcastManager.TRUTH_BROADCAST_DIVIDER = 6; // ~7Hz
