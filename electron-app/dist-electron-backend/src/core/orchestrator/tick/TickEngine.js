...
export class TickEngine {
    ...
    constructor(ctx) {
        ...
        this._impactNodeByDeviceScratch = new Map();
        ...
        if (workerBpm > 0 && workerConfidence > 0.5) {
            ...
            if (octaveRef > 0 && workerConfidence > 0.1) {
                ...
                if (ratio > 1.85 && ratio < 2.15) {
                    ...
                    console.log(`[TickEngine] 🔬 WAVE 7525: FREEWHEEL OCTAVE-DOWN correction: ` +
                    ...
                    console.log(`[TickEngine] 🔬 WAVE 7525: FREEWHEEL OCTAVE-UP correction: ` +
                        `stale=${freewheelBpm.toFixed(2)} → oracle=${octaveRef.toFixed(2)} ` +
                        `(ratio=${ratio.toFixed(3)}, conf=${workerConfidence.toFixed(3)})`);
                    freewheelBpm = octaveRef;
                    this.audioPipeline.lastStableWorkerBpm = octaveRef;
        ...
        if (this._aetherHasDevices && this.hal) {
            ...
            if (!aetherArbiter ||
                ...
                if (intent.movement?.mechanicsL && intent.movement?.mechanicsR) {
                    ...
                    this._aetherGraph.getView(NodeFamily.IMPACT).forEach((node) => {
                        if (node.channels?.some((c) => c.type === 'dimmer')) {
                ...
                aetherResolver.setResolveContext(engineAudioMetrics.bpm, engineAudioMetrics.beatConfidence, _isChillVibe);
