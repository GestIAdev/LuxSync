...
export class TempoOracle {
     ...
     */
    process(needle, deterministicTsMs) {
        ...
        if (this.framesSinceEval < EVAL_INTERVAL)
            return;
        this.framesSinceEval = 0;
        this.evaluate();
    }
    ...
    applyOdfRate(rateHz) {
        ...
        if (tMin < 2)
            tMin = 2; // need τ−1 ≥ 1 for parabola
        if (tMax > MAX_LAG >> 2)
            tMax = MAX_LAG >> 2; // 4τ must stay indexable
        if (tMax <= tMin + 1)
            tMax = tMin + 2;
        this.tauMin = tMin;
        this.tauMax = tMax;
     ...
     */
    pickShortestPeak(tMin, tMax, threshold, shade, pPrev, sigInv) {
        const score = this.score;
        let fallbackT = tMin;
