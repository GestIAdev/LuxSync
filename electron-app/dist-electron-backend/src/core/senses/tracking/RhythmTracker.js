 ...
 */
export function foldToPocket(raw, targetMin, targetMax, lastMusicalBpm) {
    if (raw <= 0)
        return 0;
...
export class RhythmTracker {
    constructor() {
        ...
        this.tempoOracle = new TempoOracle();
        /** FASE — onset gate + acumulador de beat phase (cero asignaciones). */
        this.phaseGate = new KickPhaseGate();
    ...
    kalmanUpdate(measurement, confidence) {
        ...
        if (this.kalmanBpm < 40)
            this.kalmanBpm = 40;
        else if (this.kalmanBpm > 300)
            this.kalmanBpm = 300;
        return this.kalmanBpm;
    }
    /**
