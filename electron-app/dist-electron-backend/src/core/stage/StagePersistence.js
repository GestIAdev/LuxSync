...
import { validateShowFile, getSchemaVersion, createEmptyShowFile } from './ShowFileV2';
...
export class StagePersistence {
     ...
     */
    async saveShowAs(showFile, name) {
        // Sanitize filename
        ...
        try {
            ...
            if (version === '2.0.0') {
                ...
                if (validateShowFile(data)) {
                    ...
                    return { success: true, showFile: data, migrated: false };
                }
                else {
     ...
     */
    async migrateAndLoad(legacyData, originalPath) {
        const result = autoMigrate(legacyData);
    ...
    async listShows() {
        try {
            ...
            for (const file of files) {
                if (!file.endsWith(SHOW_EXTENSION))
                    continue;
    ...
    removeFromRecentShows(showPath) {
        this.recentShows = this.recentShows.filter(p => p !== showPath);
        this.saveRecentShowsList();
    }
    // ═══════════════════════════════════════════════════════════════════════
