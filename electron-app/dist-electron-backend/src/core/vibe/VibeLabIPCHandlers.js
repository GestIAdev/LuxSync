...
import { graft as graftToBackend } from '../../engine/vibe/custom/VibeGraftRegistry';
const CHANNEL = {
    LIST: 'vibeLab:list',
    READ: 'vibeLab:read',
    SAVE: 'vibeLab:save',
 ...
 */
export function registerVibeLabIPCHandlers() {
    if (_registered)
        return;
    _registered = true;
    // ── LIST ────────────────────────────────────────────────────────────
    ...
    // ── READ ────────────────────────────────────────────────────────────
    ipcMain.handle(CHANNEL.READ, async (_evt, key) => {
        return vibeLabPersistence.read(key);
    });
    // ── SAVE ────────────────────────────────────────────────────────────
    ipcMain.handle(CHANNEL.SAVE, async (_evt, data) => {
        return vibeLabPersistence.save(data);
    });
    // ── DELETE ──────────────────────────────────────────────────────────
    ipcMain.handle(CHANNEL.DELETE, async (_evt, key) => {
        return vibeLabPersistence.delete(key);
    });
    // ── EXPORT (abre diálogo "save as") ─────────────────────────────────
    ipcMain.handle(CHANNEL.EXPORT, async (evt, data) => {
    ...
    ipcMain.handle(CHANNEL.GRAFT_VIBE, async (_evt, bundle) => {
        try {
