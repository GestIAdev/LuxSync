 ...
 */
const DYNAMIC_COMPOSITE_RESOLVERS = {
    'all-movers': f => f.type === 'moving-head' ||
        f.type === 'scanner' ||
        f.type === 'spot' ||
        f.capabilities?.hasMovementChannels === true,
    'movers': f => f.type === 'moving-head' ||
        f.type === 'scanner' ||
        f.type === 'spot' ||
        f.capabilities?.hasMovementChannels === true,
    'all-pars': f => f.type === 'par' ||
        f.type === 'wash' ||
        f.type === 'bar',
    'pars': f => f.type === 'par' ||
        f.type === 'wash' ||
...
export function resolveZone(zone, fixtures) {
    ...
    return fixtures.filter(f => {
        if (f.enabled === false)
            return false;
        if (normalizeZone(f.zone) === zCanonical)
            return true;
        // 🧩 COMPOUND FIXTURE: parent zone doesn't match — check internal channel zones
        if (f.channelZones && f.channelZones.length > 0) {
            for (let i = 0; i < f.channelZones.length; i++) {
                if (normalizeZone(f.channelZones[i]) === zCanonical)
                    return true;
            }
        }
        return false;
 ...
 */
export function fixtureMatchesZone(fixtureZone, targetZone, positionX, fixture) {
    ...
    if (compositeTargets) {
        if (compositeTargets.includes(fz))
            return true;
        // 🌊 WAVE 4951: If string match fails, try capability-based match
        if (fixture && DYNAMIC_COMPOSITE_RESOLVERS[tz]?.(fixture) === true)
            return true;
        return false;
    }
    ...
    if (fixture?.channelZones && fixture.channelZones.length > 0) {
        for (let i = 0; i < fixture.channelZones.length; i++) {
            if (normalizeZone(fixture.channelZones[i]) === tzCanonical)
                return true;
        }
    }
    return false;
 ...
 */
export function resolvePrimaryTrackZone(input) {
    // ── Backwards compat: accept a raw string array (old callers) ──
    // If a string array is passed, treat it as spatialZones with no DNA.
    const cognitiveDNA = Array.isArray(input) ? null : input?.cognitiveDNA;
    const spatialZones = Array.isArray(input) ? input : input?.spatialZones ?? null;
    ...
    if (!spatialZones || spatialZones.length === 0)
        return 'global';
    if (spatialZones.some(z => z.toLowerCase() === 'all' || z === '*'))
        return 'global';
    ...
    if (CANONICAL_ZONES.includes(normalized)) {
        return normalized;
    }
