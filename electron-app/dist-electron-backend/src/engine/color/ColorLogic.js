...
export class ColorLogic {
     ...
     */
    detectSubGenre(bpm, vibeId) {
        // Priorizar por vibe
        if (vibeId.includes('techno'))
            return 'techno';
        if (vibeId.includes('chill'))
            return 'chill';
     ...
     */
    detectMachineGun(audio, subGenre) {
        // Cumbia no tiene machine gun
        if (subGenre === 'cumbia')
            return false;
        const energyDelta = audio.previousEnergy - audio.energy;
        const isNegativeDrop = (energyDelta >= NEGATIVE_DROP_THRESHOLD &&
     ...
     */
    detectSolarFlare(audio, subGenre) {
     ...
     */
    calculateBasePalette(context, vibeProfile) {
        ...
        const primary = withHex({
            h: baseHue,
            s: energySat,
            l: 0.5,
        });
        const secondary = withHex({
            h: (baseHue + 0.33) % 1, // +120° (triádico)
            s: energySat * 0.9,
            l: 0.5,
        });
        const accent = withHex({
            h: (baseHue + 0.66) % 1, // +240° (triádico)
            s: energySat,
            l: 0.6,
        });
        const ambient = withHex({
            h: baseHue,
            s: energySat * 0.3,
            l: 0.2,
        });
     ...
     */
    applyNeonInjection(palette, bassLevel) {
        this.beatCounter++;
     ...
     */
    createBlackoutPalette() {
        const black = withHex({ h: 0, s: 0, l: 0 });
        return {
            primary: black,
            secondary: black,
            accent: black,
     ...
     */
    interpolatePalettes(from, to, factor) {
        const lerp = (a, b) => a + (b - a) * factor;
        const lerpColor = (a, b) => withHex({
            h: lerp(a.h, b.h),
            s: lerp(a.s, b.s),
            l: lerp(a.l, b.l),
        });
