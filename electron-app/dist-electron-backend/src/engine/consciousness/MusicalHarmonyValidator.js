...
export class MusicalHarmonyValidator {
    ...
    static validateMusicalHarmony(key, scale) {
        ...
        if (!scaleData)
            return 0.5; // Default si escala no existe
        const keyEmotion = this.KEY_EMOTIONS[key];
        if (!keyEmotion)
            return 0.5; // Default si clave no existe
     ...
     */
    static calculateIntervalHarmony(intervals) {
     ...
     */
    static getIntervalWeight(semitones) {
     ...
     */
    static calculateMoodMatch(mood, emotion) {
        switch (mood) {
            case 'bright':
     ...
     */
    static validateComplete(key, scale) {
        const harmony = this.validateMusicalHarmony(key, scale);
        const dissonance = this.calculateDissonance(scale);
    ...
    static validateHarmonyProgression(keys) {
        if (keys.length < 2)
            return true;
        if (keys.length > 5)
            return false; // Máximo 5 cambios para estabilidad
        let totalHarmony = 0;
     ...
     */
    static suggestScaleByEnergy(targetEnergy) {
        let closest = 'major';
     ...
     */
    static suggestKeyByBrightness(targetBrightness) {
        let closest = 'C';
        let minDiff = Infinity;
 ...
 */
MusicalHarmonyValidator.MUSICAL_SCALES = {
    major: {
        name: 'Mayor',
 ...
 */
MusicalHarmonyValidator.HARMONY_WEIGHTS = {
 ...
 */
MusicalHarmonyValidator.KEY_EMOTIONS = {
