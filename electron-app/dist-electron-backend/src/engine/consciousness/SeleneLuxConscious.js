...
import { AudioToMusicalMapper } from './AudioToMusicalMapper';
import { UltrasonicHearingEngine } from './UltrasonicHearingEngine';
import { ConsciousnessToLightMapper } from './ConsciousnessToLightMapper';
import { ColorEngine } from '../color/ColorEngine';
import { MovementEngine } from '../color/MovementEngine';
import { BeatDetector } from '../audio/BeatDetector';
// 🧬 Wave 6: Evolution Integration
import { SeleneEvolutionEngine } from './SeleneEvolutionEngine';
// 🌙 Wave 7: Meta-Consciousness Integration
import { DreamForgeEngine } from './DreamForgeEngine';
...
export class SeleneLuxConscious extends EventEmitter {
    constructor(config) {
        ...
        this.lastLightCommand = null;
        // === 🔇 WAVE 39.5: Throttle para logs ===
        this._lastStrikeLogTime = 0;
        this._lastDreamLogTime = 0;
        this.stalkCycles = 0;
        // === CACHE DE ESTADO ===
     ...
     */
    setupMetaConsciousnessEvents() {
        // DreamForge: escuchar sueños completados
        this.dreamForge.on('dream-completed', (result) => {
        ...
        this.selfAnalysis.on('bias-detected', (bias) => {
        ...
        this.selfAnalysis.on('correction-applied', (correction) => {
     ...
     */
    syncConsciousnessState(evolutionState) {
     ...
     */
    processAudioFrame(audio, deltaTime) {
        this.frameCount++;
        // === FASE 1: PERCEPCIÓN ===
        ...
        let interval = null;
        if (this.lastPattern) {
            interval = this.ultrasonicHearing.analyzeInterval(pattern.note, this.lastPattern.note, pattern.element, this.lastPattern.element);
            this.lastInterval = interval;
            // Guardar consonancia en historial
            this.consonanceHistory.push(interval.totalConsonance);
            if (this.consonanceHistory.length > 100)
                this.consonanceHistory.shift();
        }
        // Guardar belleza en historial
        this.beautyHistory.push(pattern.avgBeauty);
        if (this.beautyHistory.length > 100)
    ...
    evaluateHunt(pattern, interval) {
        ...
        const confidence = ((pattern.avgBeauty * 0.25) +
     ...
     */
    executeStrike(command, audio, beat) {
        ...
        this.lastColors = this.colorEngine.generate(audio, beat, this.lastPattern);
        // Cambio de movimiento
        this.movementEngine.setPattern(command.movement);
        // Calcular movimiento
     ...
     * 🔍 REGISTRAR COMPORTAMIENTO PARA AUTO-ANÁLISIS
     */
    recordBehaviorForAnalysis(command, beauty) {
     ...
     */
    evolveGradually(_command, audio, beat, deltaTime) {
     ...
     */
    evolveConsciousness(pattern, decision) {
     ...
     */
    buildState(pattern, interval, decision) {
        const defaultColors = {
    ...
    getAverageConsonance() {
        if (this.consonanceHistory.length === 0)
            return 0.5;
     ...
     */
    recordFeedback(isPositive, details) {
    ...
    getMetaConsciousnessSummary() {
        ...
        let mentalState = 'idle';
        if (dreamState.status === 'dreaming')
            mentalState = 'dreaming';
        else if (analysisState.status === 'analyzing')
            mentalState = 'analyzing';
        else if (dreamState.status === 'analyzing')
            mentalState = 'executing';
        ...
        if (highBiases.length > 0)
            biasSeverity = 'high';
        else if (mediumBiases.length > 0)
            biasSeverity = 'medium';
        else if (this.activebiases.length > 0)
            biasSeverity = 'low';
        return {
            mentalState,
            dreamStats: {
...
export function createSeleneLuxConscious(config) {
