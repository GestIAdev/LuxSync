...
export class SelfAnalysisEngine extends EventEmitter {
    constructor(config = {}) {
        ...
        this.intensityBuckets = [0, 0, 0, 0, 0]; // 0-0.2, 0.2-0.4, ...
    ...
    initializeHistograms() {
        ...
        for (const p of EXPECTED_PALETTES)
            this.paletteHistogram.set(p, 0);
        for (const m of EXPECTED_MOVEMENTS)
            this.movementHistogram.set(m, 0);
        for (const e of EXPECTED_EFFECTS)
            this.effectHistogram.set(e, 0);
        for (const mood of EXPECTED_MOODS)
            this.moodHistogram.set(mood, 0);
    }
    // ============================================================================
    // PRIVATE: ACTUALIZACIÓN DE HISTOGRAMAS
    // ============================================================================
    updateHistograms(sample) {
        // Paleta
    ...
    analyzePaletteBias() {
        ...
        if (total < this.config.minSamplesForAnalysis)
            return null;
    ...
    analyzeMovementBias() {
        ...
        const unusedMovements = EXPECTED_MOVEMENTS.filter(m => (this.movementHistogram.get(m) || 0) === 0);
    ...
    analyzeVarietyBias() {
        ...
        const varietyScore = ((palettesUsed / EXPECTED_PALETTES.length) +
            (movementsUsed / EXPECTED_MOVEMENTS.length) +
            (effectsUsed / EXPECTED_EFFECTS.length)) / 3;
        if (varietyScore < 0.3) {
            return {
        ...
        for (const bias of biases) {
            ...
            switch (bias.type) {
                ...
                case 'movement_neglect':
                    correction.parameters = {
                        suggestedMovements: EXPECTED_MOVEMENTS.filter(m => (this.movementHistogram.get(m) || 0) === 0).slice(0, 2)
                    };
                    break;
                case 'mood_stagnation':
                    correction.parameters = {
                        suggestedMoods: EXPECTED_MOODS.filter(m => m !== this.getMostUsedMood())
                    };
                    break;
                case 'variety_deficit':
                    correction.parameters = {
                        unusedPalettes: this.getLeastUsedPalettes(3),
                        unusedMovements: EXPECTED_MOVEMENTS.filter(m => (this.movementHistogram.get(m) || 0) === 0)
                    };
                    break;
            }
     ...
     */
    estimateBiasDuration(category, value) {
        ...
        for (let i = this.behaviorHistory.length - 1; i >= 0; i--) {
            ...
            switch (category) {
                case 'palette':
                    sampleValue = sample.palette;
                    break;
                case 'mood':
                    sampleValue = sample.mood;
                    break;
                case 'movement':
                    sampleValue = sample.movement;
                    break;
            }
     ...
     */
    getSuggestedAlternatives(category, current) {
        let alternatives = [];
        switch (category) {
            case 'palette':
...
// Factory
export function createSelfAnalysisEngine(config) {
