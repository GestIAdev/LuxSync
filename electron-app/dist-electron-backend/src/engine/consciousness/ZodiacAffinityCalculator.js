...
export class ZodiacAffinityCalculator {
     ...
     */
    static calculateAspectAffinity(pos1, pos2) {
     ...
     */
    static generateAffinityDescription(sign1, sign2, affinity) {
     ...
     */
    static getCompatibleSigns(position, threshold = 0.7) {
        const compatible = [];
        for (let i = 0; i < 12; i++) {
     ...
     */
    static getSignsByElement(element) {
     ...
     */
    static getSignByName(name) {
        return this.ZODIAC_SIGNS.find(s => s.name.toLowerCase() === name.toLowerCase());
    }
}
/**
 * Los 12 signos zodiacales con sus propiedades
 */
ZodiacAffinityCalculator.ZODIAC_SIGNS = [
    {
 ...
 * etc.
 */
ZodiacAffinityCalculator.ELEMENTAL_COMPATIBILITY = {
 ...
 */
ZodiacAffinityCalculator.QUALITY_COMPATIBILITY = {
