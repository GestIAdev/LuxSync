...
export class FixturePhysicsDriver {
    constructor() {
        ...
        this.currentRevLimitPanPerSec = 50;
        this.currentRevLimitTiltPerSec = 35;
     ...
     *  respuesta del radar.
     */
    translateDMX(fixtureId, targetPanDMX, targetTiltDMX, deltaTime = 16, isManualPosition = false) {
        const config = this.configs.get(fixtureId);
        if (!config) {
        ...
        // ═══════════════════════════════════════════════════════════════════════
        let smoothedDMX = targetDMX; // Inicializado con target safe
        if (deltaTime > 200) {
            // ═══════════════════════════════════════════════════════════════════
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    getEffectivePhysicsLimits(config) {
    ...
    applyPhysicsEasing(fixtureId, targetDMX, deltaTime) {
        ...
        for (const axis of axes) {
            ...
            if (absDistance <= brakingDistance + 5) {
                ...
                acceleration = Math.max(-this.SAFETY_CAP.maxAcceleration, Math.min(this.SAFETY_CAP.maxAcceleration, acceleration));
            }
            else {
        ...
        if (Math.abs(newVel.pan) < jitterThreshold)
            newVel.pan = 0;
        if (Math.abs(newVel.tilt) < jitterThreshold)
            newVel.tilt = 0;
        this.currentPositions.set(fixtureId, newPos);
        this.velocities.set(fixtureId, newVel);
        return newPos;
