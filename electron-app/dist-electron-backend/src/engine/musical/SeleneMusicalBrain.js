...
// Context & Analysis
import { MusicalContextEngine, } from './context/MusicalContextEngine';
// Memory
import { getMemoryManager, resetMemoryManager, } from './learning/SeleneMemoryManager';
// Palette Generation
import { ProceduralPaletteGenerator, } from './mapping/ProceduralPaletteGenerator';
// Light Mapping
import { MusicToLightMapper, } from './mapping/MusicToLightMapper';
// ═══════════════════════════════════════════════════════════════════════════
...
export class SeleneMusicalBrain extends EventEmitter {
    constructor(config = {}) {
        ...
        this.hasMemoryDB = false; // 🧠 WAVE 10: Track if SQLite is available
        this.frameCount = 0;
        this.lastOutput = null;
        // 🔒 WAVE 14: Histéresis para evitar parpadeo entre fuentes
        this.sourceHysteresis = {
            lastSource: 'procedural',
            lastSwitchTime: 0,
    ...
    process(audio) {
        ...
        if (contextResult.mode === 'reactive') {
            ...
            output = this.processReactiveMode(contextResult, audio, timestamp, perfMetrics);
        }
        else {
            // Modo inteligente - Usar memoria y aprendizaje
            output = this.processIntelligentMode(contextResult, timestamp, perfMetrics, zodiacElement);
        }
        // ─────────────────────────────────────────────────────────────────────────
        // PASO 3: Finalizar métricas
        // ─────────────────────────────────────────────────────────────────────────
     ...
     */
    processIntelligentMode(result, timestamp, perf, zodiacElement // 🔮 WAVE 13.5
    ) {
        ...
        const preferredSource = (pattern && pattern.timesUsed >= this.config.minPatternUsage) ? 'memory' : 'procedural';
    ...
    learnFromSuccess(context, palette, beautyScore) {
        ...
        // Guardar en memoria
        this.memory.learnPattern(genre, key, mode, section, beautyScore, {
    ...
    calculateBeautyScore(palette, context) {
        ...
        const intensityMatch = 1 - Math.abs(context.energy - (palette.primary.l / 100));
        score += intensityMatch * 0.15;
        // Factor 3: Saturación apropiada para el género
        const genreExpectsSaturated = ['reggaeton', 'techno', 'house', 'edm'].includes(context.genre?.primary ?? '');
    ...
    updateSessionStats(output) {
        ...
        this.sessionStats.maxBeautyScore = Math.max(this.sessionStats.maxBeautyScore, output.estimatedBeauty);
        this.sessionStats.minBeautyScore = Math.min(this.sessionStats.minBeautyScore, output.estimatedBeauty);
    }
    /**
    ...
    recordFeedback(feedback) {
        ...
        if (feedback.patternHash) {
            this.memory.recordPatternFeedback(feedback.patternHash, feedback.rating > 0);
        }
     ...
     */
    forceColorMutation(reason = 'Manual trigger') {
        if (this.config.debug) {
 ...
 */
export function getMusicalBrain(config) {
    if (!brainInstance) {
 ...
 */
export async function resetMusicalBrain() {
    if (brainInstance) {
