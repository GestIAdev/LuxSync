...
export class RhythmAnalyzer {
    ...
    analyze(audio, beat, clarity) {
        ...
        // ═══════════════════════════════════════════════════════════════════════
        if (drums.kickDetected)
            this.recordPhase(this.kickPhases, beat.phase);
        if (drums.snareDetected)
            this.recordPhase(this.snarePhases, beat.phase);
        if (drums.hihatDetected)
            this.recordPhase(this.hihatPhases, beat.phase);
        // ═══════════════════════════════════════════════════════════════════════
    ...
    detectDrumsBySlope(audio) {
        ...
        const kickDetected = this.confirmHit(this.kickSlope, kickSlope, adaptedKickThreshold, audio.bass > 0.35 // Nivel mínimo para considerar
        );
        const snareDetected = this.confirmHit(this.snareSlope, snareSlope, adaptedSnareThreshold, audio.mid > 0.30);
        const hihatDetected = this.confirmHit(this.hihatSlope, hihatSlope, adaptedHihatThreshold, audio.treble > 0.25);
     ...
     */
    updateSlope(state, currentValue) {
     ...
     */
    confirmHit(state, slope, threshold, levelOk) {
     ...
     */
    detectPatternType(audio, drums, groove, bpm) {
        // REGGAETON (Dembow)
    ...
    detectFill(audio, drums, now) {
        if (now - this.lastFillTime < this.config.minFillInterval) {
            if (this.consecutiveHighEnergy > 3)
                return true;
        }
