...
export class RhythmAnalyzer {
    constructor(config = {}) {
        ...
        this.kickHistory = []; // Fases donde se detectó kick
        this.snareHistory = []; // Fases donde se detectó snare
        this.hihatHistory = []; // Fases donde se detectó hihat
        this.historySize = 32;
        // Estado de fill
        this.lastFillTime = 0;
    ...
    analyze(audio, beat) {
        ...
        if (drums.kickDetected)
            this.recordHit(this.kickHistory, beat.phase);
        if (drums.snareDetected)
            this.recordHit(this.snareHistory, beat.phase);
        if (drums.hihatDetected)
            this.recordHit(this.hihatHistory, beat.phase);
