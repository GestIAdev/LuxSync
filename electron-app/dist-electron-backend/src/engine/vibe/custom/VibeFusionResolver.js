 ...
 */
function mapOverridePath(fullPath) {
    // ── PHYSICS ──────────────────────────────────────────────────────────
 ...
 *                     no está en este set.
 */
function enforceInvariants(bundle, diagnostics, mutatedPaths) {
    ...
    if (morphFloor >= morphCeiling) {
        ...
        bundle.liquidProfile.morphFloor = corrected;
    }
    ...
    if (zoomRange.min > zoomRange.max) {
        ...
        bundle.movementPreset.optics.zoomRange = { min: zoomRange.max, max: zoomRange.min };
    }
    ...
    if (focusRange.min > focusRange.max) {
        ...
        bundle.movementPreset.optics.focusRange = { min: focusRange.max, max: focusRange.min };
    }
    ...
    if (!mutatedPaths.has('physics.strobe.strobeDuration'))
        return;
    ...
    if (strobeDuration < MIN_STROBE_MS && strobeDuration > 0) {
        ...
        bundle.liquidProfile.strobeDuration = MIN_STROBE_MS;
    }
}
// ═══════════════════════════════════════════════════════════════════════════
// MAIN RESOLVER
 ...
 */
export function resolveCustomVibe(doc) {
    const diagnostics = [];
    // ── 1. Validar esquema ───────────────────────────────────────────────
    ...
    const vibeConfig = deepClone(canonicalVibeConfig);
    const stereoConfig = deepClone(canonicalStereoConfig);
    const movementPreset = deepClone(canonicalMovementPreset);
    ...
    const applyLeaf = (fullPath, value) => {
        // 3a. Bloquear sealed
        ...
        switch (mapping.target) {
            ...
            case 'patternConfigs': {
                ...
                if (segs.length >= 2) {
                    ...
                    ;
                    patternConfigs[patternId][geneName] = finalValue;
                }
                break;
            }
            case 'spatial':
                spatial = setByPath((spatial ?? {}), mapping.canonicalPath, finalValue);
                break;
            case 'grandMaster':
                grandMaster = setByPath((grandMaster ?? {}), mapping.canonicalPath, finalValue);
                break;
            default: {
                // liquidProfile, colorConstitution, vibeConfig, stereoConfig, movementPreset
                const targetObj = targets[mapping.target];
                if (targetObj) {
                    const updated = setByPath(targetObj, mapping.canonicalPath, finalValue);
    ...
    const bundle = {
        ...
        baseDNA,
        vibeProfile,
        liquidProfile: targets.liquidProfile,
        colorConstitution: targets.colorConstitution,
        vibeConfig: targets.vibeConfig,
        stereoConfig: targets.stereoConfig,
        movementPreset: targets.movementPreset,
        tiltOffset,
 ...
 */
export function resolveCustomVibeSafe(input) {
 ...
 */
export function getFusedValue(bundle, fullPath) {
    const mapping = mapOverridePath(fullPath);
    if (!mapping)
        return undefined;
    switch (mapping.target) {
