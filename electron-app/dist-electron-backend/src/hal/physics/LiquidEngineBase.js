...
export class LiquidEngineBase {
    ...
    constructor(profile = TECHNO_PROFILE, layout = '7.1') {
        ...
        this._lastFluxOnset = 0; // WAVE 7749.4: Separate cooldown for flux trigger
    ...
    applyBands(input) {
        ...
        if (input.snare_energy !== undefined) {
            ...
            //
            // 1. RAW DELTA trigger (NEW — primary): raw_snare_delta > 0.05
            //    The raw delta catches the actual transient edge. NO EMA FLOOR — the
            //    raw delta is self-validating (a spike from 0→0.3 is a real transient
            //    regardless of what the EMA says). This fixes the breakdown recovery lag:
            //    the EMA takes seconds to climb back, but the raw delta fires immediately.
            //    Threshold 0.05 (raised from 0.02) — 0.02 was too sensitive, firing on
            //    sub-bass leakage and hi-hat bleed. 0.05 requires a real transient.
            //    80ms cooldown for 16th-note fills.
            //
            // 2. EMA DELTA trigger (backup): snareDelta > 0.008 + EMA > 0.05
            //    Catches sudden EMA jumps (breakdowns, drops, first beat after silence).
            //    Kept as backup for cases where raw_snare_delta is unavailable.
            //
            // 3. FLUX trigger (secondary): spectralFlux > 0.04 + EMA > 0.08
            //    Catches spectral change events. Separate cooldown.
            const rawSnareDelta = input.raw_snare_delta ?? 0;
            const photonFlux = input.photon?.spectralFlux ?? 0;
            const rawOnset = rawSnareDelta > 0.05 && (now - this._lastSnareOnset > 80);
            const emaOnset = snareDelta > 0.008 && rawSnareEnergy > 0.05 && (now - this._lastSnareOnset > 80);
            const fluxOnset = photonFlux > 0.04 && rawSnareEnergy > 0.08 && (now - this._lastFluxOnset > 80);
            const snareOnset = rawOnset || emaOnset || fluxOnset;
            snareOnsetThisFrame = snareOnset;
            if (snareOnset) {
                this._lastSnareOnset = now;
                if (fluxOnset)
                    this._lastFluxOnset = now;
            ...
            // Default 0.04 (fast). Techno 0.30 (bridges micro-gaps without sawtooth flicker).
            const snareImpulseThisFrame = this._snareImpulse;
            this._snareImpulse *= (p.snareImpulseDecay ?? 0.04);
        ...
        if (photon !== undefined && hybridSnare > 0) {
            ...
            // WAVE 7749.7: Fixed the 134-line blackout — the 200ms throttle was hiding
            // 8 of every 9 frames, making diagnosis impossible. Now we log:
            //   - EVERY onset frame (snareOnsetThisFrame) immediately, no throttle
            //   - Non-onset frames: throttled to 200ms (to see veto state between hits)
            // This gives the Architect the true rhythmic pattern without blind spots.
            const shouldLogOnset = snareOnsetThisFrame;
            const shouldLogThrottled = input.snare_energy !== undefined && input.snare_energy > 0.05 &&
                (now - this._lastSnareTelemetryLog > 200);
            if (shouldLogOnset || shouldLogThrottled) {
                this._lastSnareTelemetryLog = now;
                console.log(`[SNARE_TELEMETRY] ` +
