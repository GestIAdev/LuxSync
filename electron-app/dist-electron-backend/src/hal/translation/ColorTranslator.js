...
export class ColorTranslator {
    ...
    findNearestColorLab(target, wheel, profileId) {
        ...
        if (targetHsl.h >= 35 && targetHsl.h <= 55) {
            const amberSlot = wheel.colors.find((s) => s.name === 'Amber' || s.name === 'Gold' || s.name === 'Orange');
            if (amberSlot) {
     ...
     */
    _resolveDuplicateDmx(profileId, finalColor, wheel, interpolatedDmx) {
     ...
     */
    _mechanicalDistance(fromDmx, toDmx, allowsContinuousSpin) {
     ...
     */
    _updateWheelPosition(profileId, dmx, wheel) {
