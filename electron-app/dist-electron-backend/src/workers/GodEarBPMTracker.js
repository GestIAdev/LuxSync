...
export class GodEarBPMTracker {
    constructor(sampleRate = 44100, bufferSize = 2048, overrideFrameDurationMs) {
     ...
     */
    process(rawBassEnergy, _externalKickDetected, timestamp = Date.now()) {
        this.frameCount++;
        ...
        if (timeSinceLastFrame > TIME_WARP_THRESHOLD_MS && this.sampleCount > 0) {
            console.warn(`[🥁 TIME WARP] ${timeSinceLastFrame.toFixed(0)}ms gap detected. ` +
        ...
        if (this.frameCount % 120 === 0) {
            console.log(`[🥁 GODEAR BPM] ${this.stableBpm}bpm (raw=${this.rawBpm}) ` +
    ...
    runAutocorrelationScan() {
        ...
        if (VERBOSE_AUTOCORR_LOGS && this.frameCount % 120 === 0) {
            ...
            // Log detected peaks
            const peakDump = peaks.map(p => `L${p.lag}→${p.interpolatedLag.toFixed(2)}(${Math.round(p.bpm)}bpm)r=${p.correlation.toFixed(3)}`).join(' | ');
        ...
        for (const p of peaks) {
            if (p.correlation > globalMaxCorr)
                globalMaxCorr = p.correlation;
        }
        let bestPeak = null;
        ...
        for (const highPeak of peaks) {
            if (highPeak.bpm < MIN_BPM || highPeak.bpm > MAX_BPM)
                continue;
            if (highPeak.correlation < globalMaxCorr * HARMONIC_SIEVE_RATIO)
                continue;
        ...
        if (!bestPeak) {
            ...
            if (topPeak) {
                ...
                for (const alt of peaks) {
                    if (alt === topPeak)
                        continue;
                    if (alt.bpm < MIN_BPM || alt.bpm > MAX_BPM)
                        continue;
                    ...
                    if (syncRatio >= POLYRHYTHM_RATIO_MIN && syncRatio <= POLYRHYTHM_RATIO_MAX) {
                        ...
                        if (alt.correlation >= topPeak.correlation * POLYRHYTHM_PREFERENCE) {
                            ...
                            console.log(`[🥁 POLYRHYTHM] ${Math.round(topPeak.bpm)}→${Math.round(alt.bpm)} BPM ` +
        ...
        const topPeaks = [...peaks]
            ...
            .join(' ');
        console.log(`[🥁 SIEVE] F${this.frameCount} → ${Math.round(bestPeak.bpm)}bpm ` +
            ...
            if (this.stableBpm === 0) {
                ...
                if (isOctaveJump) {
                    ...
                    if (this.octaveLockCounter >= OCTAVE_LOCK_SCANS) {
                        ...
                        if (this.frameCount % 120 === 0) {
                            console.log(`[🥁 OCTAVE ACCEPT] ${this.stableBpm}→${Math.round(finalBpm)} BPM ` +
                        ...
                        if (this.frameCount % 120 === 0) {
                            console.log(`[🥁 OCTAVE BLOCK] ${this.stableBpm}→${Math.round(finalBpm)} BPM ` +
                    ...
                    if (diff > 30) {
                        ...
                        // EMA smoothing
                        this.stableBpm = Math.round(this.stableBpm + BPM_SMOOTH_FACTOR * (finalBpm - this.stableBpm));
                    }
                }
            }
        }
     ...
     */
    upsample(input, inputLength, factor) {
