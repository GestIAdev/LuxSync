...
function splitRadixDIT_recursive(re, im, outRe, outIm, N, offset, stride) {
    if (N === 1) {
        outRe[0] = re[offset];
        outIm[0] = im[offset];
        return;
...
// TEST
// ═══════════════════════════════════════════════════════════════
function testRecursiveSR(signal, label) {
    ...
    for (let k = 0; k < N; k++) {
        ...
        if (err > maxErr)
            maxErr = err;
        if (N <= 16) {
            console.log(`  X[${k}]: SR=(${sr.re[k].toFixed(4)}, ${sr.im[k].toFixed(4)}) ` +
...
const cos64 = [];
for (let n = 0; n < N64; n++)
    cos64.push(Math.cos(2 * Math.PI * 5 * n / N64));
testRecursiveSR(cos64, 'Cosine f=5 N=64');
// Large N
const N1024 = 1024;
const chirp = [];
for (let n = 0; n < N1024; n++)
    chirp.push(Math.sin(Math.PI * n * n / N1024));
testRecursiveSR(chirp, 'Chirp N=1024');
const N4096 = 4096;
const signal4k = [];
for (let n = 0; n < N4096; n++) {
    signal4k.push(Math.cos(2 * Math.PI * 3 * n / N4096) +
...
for (let n = 0; n < 4096; n++)
    perfSignal[n] = Math.sin(Math.PI * n * n / 4096);
const warmup = 5;
