...
export class PacemakerV2 {
     ...
     */
    process(kickOnset, energy, timestamp) {
        this.frameCount++;
        ...
        if (this.frameCount % DIAGNOSTIC_INTERVAL_FRAMES === 0 && this.stableBpm > 0) {
            console.log(`[💓 PACEMAKER v2] ${this.stableBpm}bpm (raw=${this.rawBpm}) ` +
     ...
     */
    updateBpmFromClusters(timestamp) {
    ...
    applyHysteresis(newBpm) {
        ...
        if (this.isOctaveJump(newBpm, this.stableBpm)) {
            ...
            if (this.octaveLockCounter >= OCTAVE_LOCK_FRAMES) {
                ...
                console.log(`[💓 PACEMAKER v2 OCTAVE ACCEPT] ${this.stableBpm}→${Math.round(newBpm)} BPM ` +
        ...
        if (this.candidateFrames >= requiredFrames) {
            ...
            if (this.warmupComplete) {
                const maxChange = 2;
                const clamped = Math.max(this.stableBpm - maxChange, Math.min(this.stableBpm + maxChange, roundedCandidate));
     ...
     */
    detectAdaptiveOnset(punch, timestamp, currentBpm) {
