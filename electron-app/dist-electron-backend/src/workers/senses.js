...
function pollSharedRingBuffer() {
    ...
    if (samplesRead > 0) {
        ...
        // WAVE 7749.7: Diagnostic — verify raw_snare_delta survives the pipeline
        if (state.frameCount % 44 === 0) {
            const r = analysis.rhythmic;
...
function generateHealthReport() {
    ...
    return {
        ...
        avgProcessingTime: state.messagesProcessed > 0 ? state.totalProcessingTime / state.messagesProcessed : 0,
        status,
...
function handleMessage(message) {
    try {
        switch (message.type) {
            ...
            case MessageType.AUDIO_BUFFER: {
                ...
                if (sabPollInterval !== null) {
                    ...
                    sendMessage(MessageType.FORENSIC_LOG, 'alpha', { tag: 'ZOMBIE_RADAR', text: zombieText }, MessagePriority.HIGH);
            ...
            case MessageType.STATE_RESTORE: {
                ...
                const s = snap?.state;
                if (s && typeof s.frameCount === 'number')
        ...
        sendMessage(MessageType.WORKER_ERROR, 'alpha', { nodeId: NODE_ID, error: errorMsg, messageType: message.type }, MessagePriority.CRITICAL);
    }
}
// ============================================
// SEND MESSAGE
...
if (parentPort) {
    ...
    process.on('uncaughtException', (error) => {
        ...
        sendMessage(MessageType.WORKER_ERROR, 'alpha', { nodeId: NODE_ID, error: error.message, fatal: true }, MessagePriority.CRITICAL);
    });
    process.on('unhandledRejection', (reason) => {
        console.error('[BETA] Unhandled rejection:', reason);
        sendMessage(MessageType.WORKER_ERROR, 'alpha', { nodeId: NODE_ID, error: String(reason), fatal: false }, MessagePriority.CRITICAL);
    });
}
else {
