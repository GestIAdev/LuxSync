...
function processAudioBuffer(incomingBuffer) {
    ...
    const bpmResult = bpmTracker.process(needle, false, deterministicTimestampMs);
    ...
    if (state.frameCount % 20 === 0) {
        ...
        console.log(`[INTERVAL] F${state.frameCount}` +
            ` bpm=${musicalBpm}` +
            ` raw=${bpmResult.bpm}` +
    ...
    if (bpmResult.confidence > 0.05) {
        ...
        if (isLatinaVibe && musicalBpm > LATIN_BPM_CEILING) {
            ...
            console.log(`[DEMBOW CEILING 🩸] Octave corrected: ${musicalBpm}→${corrected} BPM` +
