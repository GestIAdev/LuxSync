...
export class HysteresisTrigger {
     ...
     */
    constructor(name, thresholdOn, thresholdOff) {
        this.state = false;
        ...
        if (thresholdOff >= thresholdOn) {
            throw new Error(`HysteresisTrigger [${name}]: OFF threshold (${thresholdOff}) must be < ON threshold (${thresholdOn})`);
        }
