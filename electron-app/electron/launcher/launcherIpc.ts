...
export interface LauncherGetProfileResult {
  ...
  /**
   * The persisted "Don't ask again" flag. `usePerformanceStore.hydrate()`
   * ignores it; the Tactical Hub reads it to show whether the Launcher is
   * currently suppressed at boot.
   */
  skipLauncher: boolean
...
/** Response of `launcher:reset` — consumed by the Tactical Hub restore button. */
...
export function registerLauncherIpc(deps: LauncherIpcDeps): void {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  ipcMain.handle('launcher:reset', async (): Promise<LauncherResetResult> => {
    try {
