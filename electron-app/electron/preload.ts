...
const api = {
  ...
  vibeLab: {
    ...
    list: () =>
      ipcRenderer.invoke('vibeLab:list'),
    /** Lee un .luxvibe por key. */
    read: (key: string) =>
      ipcRenderer.invoke('vibeLab:read', key),
    /** Guarda (mint) un CustomVibeOverride a disco. */
    save: (data: any) =>
      ipcRenderer.invoke('vibeLab:save', data),
    /** Elimina un .luxvibe por key. */
    delete: (key: string) =>
      ipcRenderer.invoke('vibeLab:delete', key),
    /** Exporta un .luxvibe a una ruta elegida (diálogo save-as). */
    export: (data: any) =>
      ipcRenderer.invoke('vibeLab:export', data),
    /** Importa un .luxvibe externo al vault (diálogo open). */
    import: () =>
      ipcRenderer.invoke('vibeLab:import'),
  },
}
...
type TheiaGlassPortKind = 'video-port' | 'telemetry-port'

/** Ports entregados por main antes de que la página hiciera pull. */
const _theiaPending = new Map<TheiaGlassPortKind, { port: MessagePort; role?: string }>()
/** Kinds que la página ya pidió — re-links se auto-entregan. */
const _theiaWanted = new Set<TheiaGlassPortKind>()

function _theiaDeliver(kind: TheiaGlassPortKind, port: MessagePort | undefined, role?: string): void {
  if (!port) return
...
ipcRenderer.on('theia:video-port', (event, meta: { role?: string }) => {
...
window.addEventListener('message', (ev: MessageEvent) => {
  ...
  const kind = (ev.data as Record<string, unknown> | null)?.[THEIA_GLASS_REQ_KEY]
  if (kind !== 'video-port' && kind !== 'telemetry-port') return
...
const luxApi = {
  ...
  dumpLiquidTelemetry: () => ipcRenderer.invoke('liquid-telemetry:dump') as Promise<{ success: boolean; filePath?: string; error?: string }>,
   ...
   */
  resetLauncherPrompt: () =>
    ipcRenderer.invoke('launcher:reset'),
  ...
  aether: {
     ...
     */
    purgeBaseSpatial: (fixtureIds: string[]) => {
     ...
     */
    kineticHandoff: (args: { fixtureIds: string[] }) =>
      ipcRenderer.invoke('lux:aether:kineticHandoff', args),
  ...
  arsenal: {
    getCatalog: (): Promise<{ id: string; name: string; energyZone: string; compatibleVibes: string[] }[]> =>
      ipcRenderer.invoke('lux:arsenal:getCatalog'),
  },
}
...
const luxDebug = {
   ...
   */
  ikDebug: (enabled: boolean) => ipcRenderer.invoke('lux:ik:setDebug', { enabled }),
