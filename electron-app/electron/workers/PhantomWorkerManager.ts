...
export interface AnalysisProgress {
  requestId: string
  phase: 'loading' | 'decoding' | 'waveform' | 'energy' | 'beats' | 'sections' | 'transients' | 'complete'
  progress: number
  message: string
}
...
export interface AnalysisResult {
  requestId: string
  success: boolean
  data?: any  // AnalysisData from GodEarOffline
  error?: string
  durationMs?: number
}
...
export class PhantomWorkerManager {
  private phantomWindow: BrowserWindow | null = null
  ...
  private setupIpcHandlers(): void {
    ...
    ipcMain.on('phantom:analysis-complete', (_, result: AnalysisResult) => {
    ...
    ipcMain.on('phantom:analysis-error', (_, result: AnalysisResult) => {
    ...
    ipcMain.on('phantom:analysis-progress', (_, progress: AnalysisProgress) => {
   ...
   */
  async analyzeAudioFile(request: AnalysisRequest): Promise<AnalysisResult> {
   ...
   */
  private async handlePhantomCrash(): Promise<void> {
...
let phantomInstance: PhantomWorkerManager | null = null

export function getPhantomWorker(): PhantomWorkerManager {
  if (!phantomInstance) {
...
export function destroyPhantomWorker(): void {
  if (phantomInstance) {
