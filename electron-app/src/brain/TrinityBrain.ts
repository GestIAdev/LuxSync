...
export interface TrinityBrainEvents {
  ...
  'audio-levels': (levels: { bass: number; mid: number; treble: number; energy: number; kickCount?: number; [key: string]: unknown }) => void
  'ready': () => void
...
export class TrinityBrain extends EventEmitter {
  ...
  private handleAudioAnalysis(analysis: AudioAnalysis): void {
    ...
    this.emit('audio-levels', {
      ...
      oracleRawBpm: (analysis as ExtendedAudioAnalysis).oracleRawBpm,
      oraclePeakHeight: (analysis as ExtendedAudioAnalysis).oraclePeakHeight,
