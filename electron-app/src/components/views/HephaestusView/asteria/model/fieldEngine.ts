 ...
 * Pendientes (siguiente parte): glyph (raster), slice (bucketing),
 * noise (Perlin) — resuelven su máscara de cobertura (mask[i]=1).
 *
  ...
  function evaluate(stack: readonly Gesture[]): FieldSnapshot {
    ...
    for (let g = 0; g < stack.length; g++) {
      ...
      switch (gesture.kind) {
        ...
        // glyph / slice / noise — siguientes misiones: por ahora solo
        // se resuelve la HUELLA (mask) — el canvas ya visualiza qué
        // nodos cubre cada gesto antes de que exista el campo de valores.
        default: {
