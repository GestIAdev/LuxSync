...
const _patternScratch: PatternXY = { x: 0, y: 0 }
...
// K0-BATCH-2: Pattern functions mutate `out` in place — zero alloc @ 44Hz.
type PatternFn = (phase: number, out: PatternXY) => void
...
export class AetherKineticEngine {
   ...
   */
  private readonly _smoothOffsetX = new Map<string, number>()
  private readonly _smoothOffsetY = new Map<string, number>()
  private static readonly EMA_ALPHA = 0.05
  ...
  setManualKinetics(
    ...
    for (let i = 0; i < total; i++) {
      ...
      if (!this._smoothOffsetX.has(nodeId)) this._smoothOffsetX.set(nodeId, 0)
      if (!this._smoothOffsetY.has(nodeId)) this._smoothOffsetY.set(nodeId, 0)
