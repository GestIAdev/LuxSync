...
// ═══════════════════════════════════════════════════════════════════════════
// INTENT SLOT — Objeto de shape estable para reutilización
// ═══════════════════════════════════════════════════════════════════════════

/**
 * IntentSlot es el tipo concreto que vive en el buffer pre-allocated.
 *
 * RAZÓN DE SER:
 * Si pusheáramos objetos INodeIntent literales nuevos en cada frame,
 * estaríamos creando N objetos × 44 Hz = miles de allocaciones/segundo.
 * V8 tendría que GC constantemente, causando pauses impredecibles.
 *
 * En su lugar, pre-allocamos MAX_CAPACITY slots al inicio y los
 * reutilizamos. En push(), solo sobrescribimos las propiedades.
 * El slot conserva su "forma" de objeto (hidden class) → V8 puede
 * generar código optimizado JIT con inline property access.
 *
 * EXCEPCIÓN — `values`:
 * El campo `values` (Record<string, number>) es el único reto.
 * No podemos reusar el mismo objeto Record porque dos intents
 * en el mismo frame podrían compartir la referencia.
 *
 * SOLUCIÓN: Mantenemos un pool de values objects en _valuePool.
 * Se pre-alloca el mismo número que _slots, y se asigna uno por push.
 * En clear(), resetamos el pool pointer a 0 sin desalocar.
 * Esta es la única fuente de allocaciones — y son 0 porque reutilizamos.
 */
 ...
 */
export class IntentBus implements IIntentBus {
  // ── Buffer pre-allocated de intent slots ────────────────────────────

  /**
   * Array de MAX_CAPACITY slots pre-allocated.
   *
   * La pre-asignación se hace en el constructor.
   * En push(), solo sobrescribimos nodeId, priority, confidence, source.
   * Para `values`, usamos el pool de _valuePool.
   */
  private readonly _slots: IntentSlot[]

  /**
   * Pool pre-allocated de objetos Record<string, number> para `values`.
   *
   * Mismo tamaño que _slots. Cada push() en el frame obtiene
   * _valuePool[_writeHead] y sobrescribe sus propiedades.
   *
   * DECISIÓN DE DISEÑO: No podemos reusar un único Record compartido
   * porque Arbiter leerá múltiples intents del mismo nodo y necesita
   * sus `values` individualmente accesibles. Necesitamos tantos
   * objetos como capacidad del bus.
   *
   * Pre-alloc en constructor: 4096 objetos {} → ~786 KB.
   * Aceptable para pre-asignación en init.
   */
  private readonly _valuePool: Record<string, number>[]
   ...
   */
  private readonly _nodeRangeIndex = new Map<NodeId, number>()

  /**
   * Buffer plano Int32Array de (MAX_UNIQUE_NODES × 2) entradas.
   * Cada par [i, i+1] almacena [startSlot, count] para un NodeId.
   *
   * DECISIÓN: Int32Array en lugar de Array<{start, count}> para que
   * el read en getIntentsForNode() acceda a memoria contigua sin
   * indirección de objeto.
   *
   * Posición i*2 = startSlot (primer slot en _slots para ese NodeId)
   * Posición i*2+1 = count   (número de intents consecutivos o esparcidos)
   *
   * NOTA: Los intents de un mismo nodeId no están necesariamente
   * contiguos en _slots (5 Systems escriben en orden, no por nodo).
   * Por ello, _buildNodeIndex() construye listas de índices auxiliares
   * usando _nodeSlotLists.
   */
  private readonly _nodeRangeBuffer: Int32Array

  /**
   * Write cursor para el _nodeRangeBuffer (cuántos NodeIds únicos hemos visto).
   * Resetado en cada buildIndex().
   */
  private _nodeRangeHead = 0

  /**
   * Buffer auxiliar de listas de slots por NodeId.
   *
   * NO-ALLOC SOLUTION PARA INTENTS DISPERSOS:
   * Los intents del mismo nodo pueden estar en posiciones no contiguas
   * del buffer (ej: ColorSystem escribe slot 0 para node_A,
   * ImpactSystem escribe slot 1 para node_B, slot 2 para node_A otra vez).
   *
   * Si intentamos retornar un slice contiguo de _slots, fallaría para
   * slots dispersos. Solución: mantenemos para cada NodeId una lista
   * de sus índices en _slots, pre-allocated como Int32Array plano.
   *
   * _nodeSlotBuffer: espacio para MAX_UNIQUE_NODES × MAX_INTENTS_PER_NODE índices
   * _nodeSlotHead: cursor en ese buffer
   * _nodeSlotListStart[rangeIdx]: donde empieza la lista de un NodeId en _nodeSlotBuffer
   * _nodeSlotListCount[rangeIdx]: cuántos slots tiene esa lista
   */
  private readonly _nodeSlotBuffer: Int32Array

  /** Donde empieza la lista de slots de cada NodeId indexado */
  private readonly _nodeSlotListStart: Int32Array

  /** Cuántos slots tiene la lista de cada NodeId indexado */
  private readonly _nodeSlotListCount: Int32Array

  /** Cursor en _nodeSlotBuffer durante buildIndex() */
  private _nodeSlotHead = 0

  // ── Capacidad pública ────────────────────────────────────────────────

  public readonly capacity: number

  // ── Arrays de retorno pre-allocated para getIntentsForNode() ────────

  /**
   * Buffer temporal para el retorno de getIntentsForNode().
   *
   * EL DILEMA DEL RETORNO ZERO-ALLOC:
   * `getIntentsForNode()` debe retornar `readonly INodeIntent[]`.
   * Si creamos un nuevo array cada vez → alloc.
   * Si retornamos slice del buffer → solo funciona para slots contiguos.
   *
   * SOLUCIÓN ADOPTADA: Pre-allocar un array de MAX_INTENTS_PER_RESULT
   * que reutilizamos como "ventana de retorno". Copiamos las referencias
   * (no los objetos) de _slots al _resultBuffer, ajustamos su longitud,
   * y lo retornamos.
   *
   * TRADE-OFF: Sí, hay una copia de referencias (punteros de 8 bytes),
   * pero NO hay alloc de nuevos objetos INodeIntent.
   * La llamada típica tendrá 1-5 intents por nodo → copia de 5 punteros.
   *
   * LIMITACIÓN: No es thread-safe. El Arbiter debe leer _resultBuffer
   * antes de la siguiente llamada a getIntentsForNode().
   * En nuestro modelo single-threaded (frame loop síncrono), está OK.
   */
  private readonly _resultBuffer: IntentSlot[]

  // Longitud actual del _resultBuffer (property trick para no reasignar length)
  private _resultCount = 0

  // ── Vista pública del buffer de resultado ────────────────────────────

  /**
   * Proxy de array de solo lectura que no alloca.
   * Envuelve _resultBuffer con una interfaz readonly INodeIntent[].
   *
   * Implementado como objeto con una interfaz iterable simulada
   * — no es un Array real, evita herencia de prototype de Array.
   */
  private readonly _resultView: IntentSlotReadonlyView

  constructor(capacity = DEFAULT_CAPACITY) {
    this.capacity = capacity

    // Pre-alloca los slots de intent con objetos vacíos de shape estable
    this._slots = new Array<IntentSlot>(capacity)
    ...
    this._resultBuffer = new Array<IntentSlot>(MAX_INTENTS_PER_RESULT)
  ...
  getIntentsForNode(nodeId: NodeId): readonly INodeIntent[] {
    ...
    if (rangeIdx === undefined) {
      ...
      this._resultCount = 0
      return this._resultView as unknown as readonly INodeIntent[]
    }
...
class IntentSlotReadonlyView {
  constructor(
    private readonly _buffer: IntentSlot[],
    private readonly _getCount: () => number,
  ) {}
  ...
  get(index: number): INodeIntent {
    return this._buffer[index]
  }

  /** Iterador for...of compatible  — zero-alloc para casos con count ≤ 5 */
  [Symbol.iterator](): Iterator<INodeIntent> {
    const buffer = this._buffer
    const count = this._getCount()
    let i = 0
    return {
      next(): IteratorResult<INodeIntent> {
        if (i < count) {
          return { value: buffer[i++], done: false }
        }
        return { value: undefined as unknown as INodeIntent, done: true }
      },
    }
  }
}
