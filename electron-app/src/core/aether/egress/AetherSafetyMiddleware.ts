...
export class AetherSafetyMiddleware {
  ...
  private readonly _pendingColorChangeNodes = new Set<NodeId>()
  ...
  private readonly _pendingSinceMs = new Map<NodeId, number>()
   ...
   */
  notifyPendingColorChange(nodeId: NodeId): void {
  ...
  checkDarkSpin(
    ...
    minTransitMs: number,
    allowsContinuousSpin: boolean = false,
  ): boolean {
    ...
    if (currentWheelDmx !== s.lastStableWheelDmx) {
      ...
      const dynamicTransitMs = Math.min(
        MAX_TRANSIT_MS,
        Math.max(minTransitMs, minTransitMs + dmxDistance * TIME_PER_DMX_STEP + SETTLEMENT_MS),
      )
