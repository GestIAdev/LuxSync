...
import type { INodeIntent } from '../intent-bus'
...
const CHANNEL_HASH_MAP = new Map<number, string>()
...
export function registerCalibrationChannel(channelName: string): void {
...
export function resolveChannelHash(hash: number): string | undefined {
  return CHANNEL_HASH_MAP.get(hash >>> 0)
}

// ── Factory ───────────────────────────────────────────────────────────────────

export function createCalibrationSAB(): SharedArrayBuffer {
...
// ── Writer (Renderer — useLiveCalibration hook) ───────────────────────────────
...
export class CalibrationSABWriter {
  private readonly _meta: Int32Array
  private readonly _body: Uint32Array

  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < CALIB_SAB_BYTE_LENGTH) {
      throw new RangeError(
        `CalibrationSABWriter: SAB too small (${sab.byteLength} < ${CALIB_SAB_BYTE_LENGTH})`,
      )
    }
   ...
   */
  publish(entries: readonly CalibrationEntry[]): void {
...
export class CalibrationSABReader {
  ...
  /** Pre-allocated intent pool — zero-alloc in hot-path. */
  private readonly _intentPool: INodeIntent[]
  private _poolCursor = 0

  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < CALIB_SAB_BYTE_LENGTH) {
      throw new RangeError(
        `CalibrationSABReader: SAB too small (${sab.byteLength} < ${CALIB_SAB_BYTE_LENGTH})`,
      )
    }
    this._meta = new Int32Array(sab, 0, CALIB_HEADER_INT32S)
    this._body = new Uint32Array(sab, CALIB_HEADER_BYTES, CALIB_BODY_BYTES / Uint32Array.BYTES_PER_ELEMENT)
   ...
   */
  readIfNew(): readonly INodeIntent[] | null {
    const seq = Atomics.load(this._meta, HDR_SEQ)
    if (seq === this._lastSeq) return null
    this._lastSeq = seq

    if (Atomics.load(this._meta, HDR_ACTIVE) !== 1) return null
    ...
    for (let i = 0; i < count && i < CALIB_MAX_INTENTS; i++) {
      ...
      for (let u = 0; u < INTENT_NODEID_UINT32S; u++) {
        ...
        if (lo !== 0) nodeId += String.fromCharCode(lo)
        if (hi !== 0) nodeId += String.fromCharCode(hi)
      }

      // Read channel count
      const chCount = body[baseUint32 + INTENT_CHANNEL_COUNT_OFFSET]
      if (chCount === 0 || nodeId === '') continue

      // Acquire intent from pool
      const intent = pool[this._poolCursor++]
      const values: Record<string, number> = intent.values as Record<string, number>
      ...
      const mutable = intent as { nodeId: string; values: Record<string, number>; priority: number }
