...
function waitEven(hdr: Int32Array, slot: number): number {
  let s: number
  ...
  do { s = Atomics.load(hdr, slot) } while (s & 1)
  return s
}
...
export class DmxUniverseWriter {
  ...
  commitFrame(
    frameId: number,
    universes: ReadonlyArray<Uint8Array | null | undefined>,
    timestamp: number,
  ): void {
    const hdr  = this._hdr
    ...
    Atomics.add(hdr, DmxHdr.SEQLOCK, 1)

    // ── Cabecera ──────────────────────────────────────────────────────────
    hdr[DmxHdr.FRAME_ID] = frameId | 0
    ...
    hdr[DmxHdr.UNIVERSE_MASK]    = maskLo
    hdr[DmxHdr.UNIVERSE_MASK_HI] = maskHi
    hdr[DmxHdr.ACTIVE_UNIS]      = activeCount

    // Timestamp en dos Int32 (simulando BigInt split)
    const tsMs = timestamp | 0
    hdr[DmxHdr.TIMESTAMP_LO] = tsMs
    hdr[DmxHdr.TIMESTAMP_HI] = 0
  ...
  peekSeqlock(): number {
    return Atomics.load(this._hdr, DmxHdr.SEQLOCK)
  }
}
...
export class DmxUniverseReader {
   ...
   */
  readCoherent(destUniverse: Uint8Array, lastFrameId: number): number | null {
    const hdr  = this._hdr
    const data = this._data
    ...
    do {
      ...
      s1 = waitEven(hdr, DmxHdr.SEQLOCK)
      ...
      s2 = Atomics.load(hdr, DmxHdr.SEQLOCK)
    } while (s1 !== s2)
   ...
   */
  tryReadIfStable(destUniverse: Uint8Array): number | null {
    const hdr  = this._hdr
    const data = this._data

    const s1 = Atomics.load(hdr, DmxHdr.SEQLOCK)
    if (s1 & 1) return null  // seqlock impar: escritura en curso

    const frameId = hdr[DmxHdr.FRAME_ID]
    destUniverse.set(data)

    const s2 = Atomics.load(hdr, DmxHdr.SEQLOCK)
    if (s1 !== s2) return null  // tearing detectado
  ...
  peekHeader(): { frameId: number; activeUnis: number; maskLo: number; maskHi: number } {
    const hdr = this._hdr
    return {
      frameId:   hdr[DmxHdr.FRAME_ID],
...
export class FixtureStateWriter {
   ...
   */
  writeFixtureField(fixtureIndex: number, field: number, value: number): void {
...
export class FixtureStateReader {
  ...
  readCoherent(
    ...
    do {
      s1 = waitEven(hdr, FixHdr.SEQLOCK)
   ...
   */
  readFixtureField(fixtureIndex: number, field: number): number {
