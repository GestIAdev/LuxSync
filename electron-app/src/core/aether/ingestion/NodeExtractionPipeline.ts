...
export class NodeExtractionPipeline {
   ...
   * @param universe         — Universo DMX (0-based, ArtNet convention).
  ...
  public extract(
    ...
    if (typeof dmxAddressOrFixtureV2 === 'object') {
      ...
      const legacyFv2  = fv2 as FixtureV2 & {
        installationType?: string
        physics?: { orientation?: string }
      }
  ...
  private _buildNodesFromForgeGraph(
    ...
    const outputNodes = graph.nodes.filter(
      (n): n is OutputNode => n.type === 'output_dmx' && n.config.nodeType === 'output_dmx',
    )
   ...
   */
  private _inferAetherSuffix(channelType: string): string {
    const norm = this._normalizeChannelType(channelType)
    if (COLOR_CHANNEL_TYPES.has(norm))     return 'color'
    if (IMPACT_CHANNEL_TYPES.has(norm))    return 'impact'
    if (KINETIC_CHANNEL_TYPES.has(norm))   return 'kinetic'
    if (BEAM_CHANNEL_TYPES.has(norm))      return 'beam'
    return 'atmosphere'
  }
  ...
  private _detectMixingTypeFromSet(typeSet: Set<string>): ColorMixingType {
    if (typeSet.has('cyan') && typeSet.has('magenta') && typeSet.has('yellow')) return 'cmy'
    if (typeSet.has('color_wheel')) return 'wheel'
    const rgb = typeSet.has('red') && typeSet.has('green') && typeSet.has('blue')
    if (rgb && (typeSet.has('white') || typeSet.has('amber'))) return 'rgbw'
    if (rgb) return 'rgb'
    return 'rgb'
  }
   ...
   */
  private _pairDimmerFine(channels: INodeChannelDef[]): INodeChannelDef[] {
    const fineOffsets = new Set<number>()
    const coarseByOffset = new Map<number, number>() // dmxOffset → index in array
    ...
    if (fineOffsets.size === 0) return channels
    ...
    const result: INodeChannelDef[] = []
    for (let i = 0; i < channels.length; i++) {
      const ch = channels[i]
      if (ch.type === 'dimmer_fine') {
        // Skip if paired (coarse dimmer exists at offset-1)
        if (coarseByOffset.has(ch.dmxOffset - 1)) continue
   ...
   */
  private _pairPanTiltFine(channels: INodeChannelDef[]): INodeChannelDef[] {
    let panFineOffset: number | undefined
    let tiltFineOffset: number | undefined
    ...
    if (!hasPanFine && !hasTiltFine) return channels
