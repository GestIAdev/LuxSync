...
export class AetherVirtualStrobeEngine {
  ...
  private readonly _phase = new Map<string, number>()
   ...
   */
  computeMask(fixtureId: string, strobeRate: number, shutter: number, dtMs: number): number {
    ...
    if (step > MAX_PHASE_STEP) step = MAX_PHASE_STEP
