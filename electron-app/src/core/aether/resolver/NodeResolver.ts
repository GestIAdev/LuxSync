...
import type { INodeChannelDef, IColorNodeData, IKineticNodeData, AnyNodeData } from '../capability-node'
...
// El freeze de rueda mecánica en chill ya NO depende de bpmConfidence.
// El vibe chill es estático (ChillAmbientEngine = función pura de t).
// El flag _isChillVibe se inyecta vía setResolveContext desde TickEngine,
// que lo deriva del nombre del vibe activo.
...
// 🌑 WAVE 4685.1: DarkSpin buffer sweep — pre-computed wheel device entry.
// Built at patch time, iterated zero-alloc in hot path.
interface WheelDeviceEntry {
  readonly deviceId: DeviceId
  readonly colorNodeId: NodeId
  readonly universe: number
  /** Absolute index in the universe buffer (0-based) for the color_wheel channel */
  readonly wheelBufIdx: number
...
export class NodeResolver implements INodeResolver {
  ...
  private readonly _ditherError = new Map<string, number>()
  ...
  private readonly _prev16bitNorm = new Map<string, number>()
  private readonly _prev16bitRaw = new Map<string, number>()
  ...
  private readonly _prev8bitNorm = new Map<string, number>()
  private readonly _prev8bitDmx = new Map<string, number>()
  ...
  private readonly _lastWheelBytes = new Map<DeviceId, number>()
   ...
   */
  getSafetyMiddleware(): AetherSafetyMiddleware | null {
  ...
  private _precomputeIgnitionMap(deviceId: DeviceId): void {
    ...
    const allChannels: { type: string; dmxOffset: number; nodeId: NodeId; zoneId: string }[] = []
    ...
    for (const nodeId of nodeIds) {
      ...
      for (const ch of node.channels) {
        ...
        for (const dep of ch.ignitionDeps) {
          ...
          let target: (typeof allChannels)[number] | undefined
          ...
          if (!target) {
            ...
            if (target) {
              console.warn(
                `[NodeResolver] ⚠️ Ignition dep "${dep.targetChannelType}" for ` +
   ...
   * PATCH TIME — cero alloc en hot path.
   */
  private _precomputeWheelDeviceEntry(deviceId: DeviceId): void {
    const device = this._graph.getDevice(deviceId)
    if (!device) return
    ...
    for (let ni = 0; ni < nodeIds.length; ni++) {
      ...
      if (!this._isDarkSpinEligibleColorNode(colorNode)) continue
      ...
      for (let ci = 0; ci < node.channels.length; ci++) {
        ...
        if (wheelBufIdx < 0 || wheelBufIdx >= DMX_UNIVERSE_SIZE) continue
  ...
  private _accumulateForgeNodeValues(
    ...
    const virtualDim =
      brightnessRaw !== undefined && Number.isFinite(brightnessRaw)
      ...
      if (pers) {
        if (chDef.type === DIMMER_CHANNEL && pers.minDimmer !== undefined) {
          ...
          if (rawNormalized <= 0) {
            ...
            dmxValue = Math.floor(
              pers.strobeRangeMin + rawNormalized * (pers.strobeRangeMax - pers.strobeRangeMin)
            )
          }
        }
      }
   ...
   */
  private _applyDarkSpinBufferSweep(): void {
    const sm = this._safetyMiddleware
    if (!sm) return
   ...
   */
  private _applyDarkSpinFinalBlackout(): void {
    const sm = this._safetyMiddleware
    if (!sm) return
      ...
      case 'hybrid': {
        ...
        if (this._safetyMiddleware && darkSpinEligible && aetherWheel.minTransitionMs > 0) {
          ...
          const inBlackout = this._safetyMiddleware.checkDarkSpin(
            ...
            wheelDmxForDarkSpin,
            aetherWheel.minTransitionMs,
            aetherWheel.allowsContinuousSpin ?? false,
          )
          if (inBlackout) {
