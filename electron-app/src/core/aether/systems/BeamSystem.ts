...
import type { DarkSpinState } from '../types'
...
const ZOOM_BY_SECTION: Readonly<Record<string, number>> = {
  intro:   0.80,
  verse:   0.75,
  build:   0.50,
  drop:    0.20,
...
const FOCUS_BY_SECTION: Readonly<Record<string, number>> = {
  intro:   0.50,
  verse:   0.55,
  build:   0.65,
  drop:    0.95,
...
const IRIS_BY_SECTION: Readonly<Record<string, number>> = {
  intro:   0.70,
  verse:   0.65,
  build:   0.55,
  drop:    0.30,
...
const GOBO_BY_SECTION: Readonly<Record<string, number>> = {
  intro:   0.0,
  verse:   0.0,
  build:   0.3,
  drop:    0.7,
...
const FROST_BY_SECTION: Readonly<Record<string, number>> = {
  intro:   0.20,
  verse:   0.25,
  build:   0.10,
  drop:    0.00,
...
const PRISM_BY_SECTION: Readonly<Record<string, boolean>> = {
  intro:   false,
  verse:   false,
  build:   true,
  drop:    true,
...
export class BeamSystem
  extends BaseSystem<IBeamNodeData>
  implements IAetherSystem<IBeamNodeData>
{
  ...
  process(
    ...
    const focusTarget = BaseSystem.clamp01(
      focusBase + (audio.energy - 0.5) * 0.1 * vibe.beamExpressiveness,
    )

    // Velocidad de rotacion de prisma reactiva a BPM + energia
    const prismRotSpeed = BaseSystem.clamp01(
      (audio.bpm / 300) * 0.4 + audio.energy * 0.6,
    ...
    view.forEach((node: IBeamNodeData) => {
      ...
      if (node.hasGoboRotation && goboBase > 0) {
        const goboRot = BaseSystem.clamp01(
          audio.energy * 0.5 + audio.mid * 0.3,
        ) * vibe.beamExpressiveness
   ...
   */
  private _canChangeMechanical(node: IBeamNodeData, nowMs: number, holdMs: number): boolean {
    const ds = node.darkSpinState
    if (!ds) return true
    if (ds.isLocked) return false
   ...
   */
  private _canChangePrism(node: IBeamNodeData, nowMs: number, targetValue: number): boolean {
    const ds = node.darkSpinState
    if (!ds) return true
    if (ds.isLocked) return false
    const currentNormalized = node.state[1]
    if (Math.abs(currentNormalized - targetValue) < 0.01) return false
   ...
   */
  private _stampMechanicalChange(node: IBeamNodeData, nowMs: number): void {
    if (!node.darkSpinState) {
      // Primera inicializacion -- unica allocation en patch time
      const ds: DarkSpinState = { lastChangeMs: nowMs, isLocked: false }
      ;(node as { darkSpinState: DarkSpinState }).darkSpinState = ds
      return
    }
    const ds = node.darkSpinState as { lastChangeMs: number; isLocked: boolean }
