...
export class ColorSystem
  extends BaseSystem<IColorNodeData>
  implements IAetherSystem<IColorNodeData>
{
  readonly name = 'ColorSystem'
  readonly family = NodeFamily.COLOR

  // ── Pre-allocated scratch buffers (zero-alloc) ─────────────────────────

  /**
   * Buffer RGB para conversiones intermedias.
   * Reutilizado en cada iteración para HSL→RGB y CMY.
   */
  private readonly _rgbScratch: { r: number; g: number; b: number }

  /**
   * Buffer RGB para el color target (resultado de palette lookup).
   * Separado de _rgbScratch para no colisionar en cálculos de wheel.
   */
  ...
  process(
    ...
    view.forEach((node: IColorNodeData) => {
      ...
      const lerpSpeed = BaseSystem.lerp(
        COLOR_LERP_SPEED_BASE,
        COLOR_LERP_SPEED_MAX,
        audio.energy,
      )
   ...
   */
  private _selectPaletteColor(
    node: IColorNodeData,
    palette: readonly ColorEntry[],
    audio: FrameContext['audio'],
  ): void {
    if (palette.length === 0) {
      // Paleta vacía = blanco neutro
      this._targetRgb.r = 1; this._targetRgb.g = 1; this._targetRgb.b = 1
      return
    }
   ...
   */
  private _computeSaturation(c: { r: number; g: number; b: number }): number {
   ...
   */
  private _derivePixelFraction(nodeId: string): number {
    let num  = 0
    let found = false
    let mul  = 1
