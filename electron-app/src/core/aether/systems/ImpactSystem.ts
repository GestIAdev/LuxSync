...
export class ImpactSystem
  extends BaseSystem<IImpactNodeData>
  implements IAetherSystem<IImpactNodeData>
{
   ...
   */
  process(
    view: INodeView<IImpactNodeData>,
   ...
   */
  private _resolveBandMix(role: string, vibe: FrameContext['vibe']): BandMixWeights {
    if (vibe.bandMatrixOverride) {
      ...
      if (override) {
        ...
        return override as BandMixWeights
      }
    }
  ...
  private _getDefaultBandMix(role: string): BandMixWeights {
    switch (role) {
   ...
   */
  private _applyNodeCurve(value: number, node: IImpactNodeData): number {
    const curve = node.transferCurve
    switch (curve.type as TransferCurveType) {
      case 'exponential':
        return BaseSystem.applyExponential(
          value,
          curve.exponent ?? 2.5,
          curve.noiseGate ?? 0,
        )
  ...
  private _stepEnvelope(
    ...
    if (shapedValue >= current) {
      // Ataque instantáneo
      ;(env as { current: number }).current = shapedValue
      ;(env as { velocity: number }).velocity = 0
      return shapedValue
    }
    ...
    ;(env as { current: number }).current = next
    ;(env as { velocity: number }).velocity = (next - current) / (deltaMs || 1)
    return next
  }

  /** Retorna la tasa de decay (por ms) para un rol. */
  private _getDecayRate(role: string): number {
    switch (role) {
   ...
   */
  private _applyFloor(value: number, role: string): number {
    switch (role) {
      case 'breath':
