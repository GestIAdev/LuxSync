...
export class KineticSystem
  extends BaseSystem<IKineticNodeData>
  implements IAetherSystem<IKineticNodeData>
{
  readonly name = 'KineticSystem'
   ...
   */
  process(
    view: INodeView<IKineticNodeData>,
    ...
    if ('pan_fine'  in this._valuesDict) delete this._valuesDict['pan_fine']
    if ('tilt_fine' in this._valuesDict) delete this._valuesDict['tilt_fine']
    if ('rotation'  in this._valuesDict) delete this._valuesDict['rotation']
    if ('speed'     in this._valuesDict) delete this._valuesDict['speed']

    const kineticTotal = view.count
    view.forEach((node: IKineticNodeData, _index: number) => {
      ...
      switch (pattern) {
        ...
        case 5: { // build — tilt sube progresivamente
          ...
          pan  = HOME_PAN + SWEEP_AMPLITUDE_PAN * 0.5 * Math.sin(
            (context.nowMs / 1000) * 0.8 * TWO_PI
          )
          break
        }
   ...
   */
  private static _selectPattern(musical: MusicalContext): number {
    if (musical.dropImminent) return 5  // build antes del drop
...
function BASE_lerp(a: number, b: number, t: number): number {
  return a + (b - a) * t
}
