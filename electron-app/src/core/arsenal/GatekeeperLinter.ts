...
import type { LfxClipInstance, EnergyZoneId, UserArchetype, AcoTriad } from './LfxClipInstance'
...
export type LinterRuleId =
  | 'ARCHETYPE_BIAS_VIOLATION'
  | 'AMBIENT_AGGRESSION_OVERFLOW'
  | 'STROBE_FREQ_DANGEROUS'
  | 'STROBE_FREQ_UNDECLARED'
  | 'ZONE_INCOHERENT_FOR_ARCHETYPE'
  | 'HEAVY_IN_LOW_ZONE'
  | 'DIVINE_NOT_PEAK_ONLY'
  | 'EMPTY_ENERGY_ZONES'
  | 'EMPTY_VIBE_LIST'
  | 'STROBE_LOW_FREQ_FOR_ARCHETYPE'

/**
 * Una observación emitida por una regla del Gatekeeper. Estructura plana,
 * serializable, lista para que la UI la pinte como card.
 */
export interface LinterWarning {
  readonly id: LinterRuleId
  readonly severity: WarningSeverity
  readonly title: string
  readonly message: string
  /** Campos del átomo a los que apunta este warning (UI focus). */
  readonly affectedFields: readonly string[]
  /**
   * Correlación con el motor real de Selene que dispara la regla.
   * Útil para el tooltip "por qué lo veo".
   */
...
export interface LinterResult {
  ...
  /** `false` si hay al menos un `error` o `critical`. */
  readonly canSave: boolean
  /** Conteo por severidad para UI badge. */
...
const HARD_ZONES: ReadonlySet<EnergyZoneId> = new Set<EnergyZoneId>([
  'active',
  'intense',
  'peak',
])
...
const LOW_ZONES: ReadonlySet<EnergyZoneId> = new Set<EnergyZoneId>([
  'silence',
  'valley',
  'ambient',
  'gentle',
...
function summarize(warnings: readonly LinterWarning[]): LinterResult['summary'] {
  let info = 0,
    warning = 0,
    error = 0,
    critical = 0
  for (const w of warnings) {
    if (w.severity === 'info') info++
    else if (w.severity === 'warning') warning++
    else if (w.severity === 'error') error++
    else if (w.severity === 'critical') critical++
  }
  return Object.freeze({ info, warning, error, critical })
}

function hasAnyHardZone(zones: readonly EnergyZoneId[]): boolean {
  for (const z of zones) if (HARD_ZONES.has(z)) return true
  return false
}

function hasAnyLowZone(zones: readonly EnergyZoneId[]): boolean {
  for (const z of zones) if (LOW_ZONES.has(z)) return true
  return false
}

// ─── LAS REGLAS ─────────────────────────────────────────────────────────────

type Rule = (clip: LfxClipInstance) => LinterWarning | null
...
function makeBiasRule(rawAco: AcoTriad): Rule {
  return (clip) => {
    ...
    return Object.freeze({
      id: 'ARCHETYPE_BIAS_VIOLATION' as const,
      severity: 'error' as WarningSeverity,
      title: 'Archetype Bias Violation',
      message:
        `Archetype "${clip.userArchetype}" requires an ACO envelope that the current triad ` +
        `violates: ${violations.join(' · ')}. ` +
        `Adjust the sliders to satisfy the bias constraints, or choose a different archetype.`,
      affectedFields: Object.freeze(['userArchetype', 'acoTriad'] as const),
      seleneCorrelation: Object.freeze({
        engine: 'EnergyConsciousness' as const,
...
const ruleAmbientAggressionOverflow: Rule = (clip) => {
  if (clip.userArchetype !== 'ambient') return null
  if (clip.acoTriad.aggression <= AMBIENT_AGGRESSION_HARD_CEILING) return null
...
const ruleStrobeFreqDangerous: Rule = (clip) => {
  if (clip.userArchetype !== 'strobe') return null
  if (clip.maxStrobeFreqHz <= STROBE_FREQ_MAX_SAFE_HZ) return null
  ...
  return Object.freeze({
    ...
    message:
      `maxStrobeFreqHz=${clip.maxStrobeFreqHz}Hz exceeds the safety ceiling ` +
      `(${STROBE_FREQ_MAX_SAFE_HZ}Hz). Selene's SafetyMiddleware will reject this clip ` +
      `at ingest (Gate G6 — epilepsy risk).`,
    affectedFields: Object.freeze(['maxStrobeFreqHz'] as const),
    seleneCorrelation: Object.freeze({
      engine: 'SafetyMiddleware' as const,
...
const ruleStrobeFreqUndeclared: Rule = (clip) => {
  if (clip.userArchetype !== 'strobe') return null
  if (clip.maxStrobeFreqHz > 0) return null
  ...
  return Object.freeze({
    ...
    message:
      `Archetype "strobe" requires maxStrobeFreqHz > 0. Selene cannot route this clip ` +
...
const ruleStrobeLowFreq: Rule = (clip) => {
  if (clip.userArchetype !== 'strobe') return null
  if (clip.maxStrobeFreqHz === 0) return null // ya cubierto por R3
  if (clip.maxStrobeFreqHz >= STROBE_FREQ_MIN_RECOMMENDED_HZ) return null
  ...
  return Object.freeze({
    ...
    message:
      `maxStrobeFreqHz=${clip.maxStrobeFreqHz}Hz is below the perceptual strobe threshold ` +
      `(${STROBE_FREQ_MIN_RECOMMENDED_HZ}Hz). Consider archetype "heavy" or "utility" instead.`,
    affectedFields: Object.freeze(['maxStrobeFreqHz', 'userArchetype'] as const),
    seleneCorrelation: Object.freeze({
      engine: 'EffectDreamSimulator' as const,
...
const ruleZoneIncoherent: Rule = (clip) => {
  if (clip.energyZones.length === 0) return null
  ...
  if (arch === 'heavy' && !hasAnyHardZone(clip.energyZones)) {
    return Object.freeze({
      ...
      message:
        `Archetype "heavy" was mapped to zones [${clip.energyZones.join(', ')}] which contain ` +
        `no hard zone (active/intense/peak). Selene's DNAAnalyzer will never select this clip ` +
        `during high-energy sections.`,
      affectedFields: Object.freeze(['energyZones', 'userArchetype'] as const),
      seleneCorrelation: Object.freeze({
        engine: 'DNAAnalyzer' as const,
  ...
  if (arch === 'divine') {
    ...
    const hasNonPeakIntense = clip.energyZones.some(
      (z) => z !== 'peak' && z !== 'intense',
    )
    if (hasNonPeakIntense) {
      return Object.freeze({
        ...
        message:
          `Archetype "divine" must live in peak/intense zones only. Current zones ` +
  ...
  if (arch === 'ambient' && !hasAnyLowZone(clip.energyZones)) {
    return Object.freeze({
      ...
      message:
        `Archetype "ambient" was mapped to [${clip.energyZones.join(', ')}] without any low zone ` +
...
const ruleEmptyZones: Rule = (clip) => {
  if (clip.energyZones.length > 0) return null
  return Object.freeze({
    ...
    message:
      `The clip has no energyZones. Selene's DNAAnalyzer cannot match it against any ` +
      `musical context and the clip will be permanently dormant.`,
    affectedFields: Object.freeze(['energyZones'] as const),
    seleneCorrelation: Object.freeze({
      engine: 'DNAAnalyzer' as const,
...
const ruleEmptyVibes: Rule = (clip) => {
  if (clip.compatibleVibes.length > 0) return null
  return Object.freeze({
    id: 'EMPTY_VIBE_LIST',
    severity: 'warning' as WarningSeverity,
    title: 'No compatible vibes declared',
    message:
      `compatibleVibes is empty. Selene will only consider this clip in the universal ` +
      `fallback bucket — heavily deprioritized vs. vibe-tagged effects.`,
    affectedFields: Object.freeze(['compatibleVibes'] as const),
    seleneCorrelation: Object.freeze({
      engine: 'DNAAnalyzer' as const,
...
const STATIC_RULES: readonly Rule[] = Object.freeze([
 ...
 */
export function validateClip(clip: LfxClipInstance, rawAco?: AcoTriad): LinterResult {
  ...
  if (biasWarning !== null) warnings.push(biasWarning)
 ...
 */
export function listFiredRules(clip: LfxClipInstance): readonly LinterRuleId[] {
...
export { ENERGY_ZONES }
export type { UserArchetype, EnergyZoneId }
