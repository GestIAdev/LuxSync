...
import { CalibrationReport, SignalReport, StatSummary } from './CalibrationRunner'
 ...
 */
function formatSignalReport(signal: SignalReport): string {
  const lines: string[] = []
 ...
 */
function formatStatRow(name: string, stats: StatSummary): string {
 ...
 */
export function formatReportAsSummary(report: CalibrationReport): string {
  const lines: string[] = []
 ...
 */
export function generateEnergyGraph(signal: SignalReport, width: number = 60, height: number = 10): string {
