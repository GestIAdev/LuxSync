...
export interface SignalConfig {
  sampleRate: number      // Usually 44100 or 48000
  duration: number        // Seconds
  bufferSize: number      // FFT window size (2048, 4096, etc)
}

export interface GeneratedSignal {
  name: string
  description: string
  buffers: Float32Array[]  // Array of buffers to feed sequentially
  totalSamples: number
  config: SignalConfig
}
...
export class SignalGenerator {
   ...
   */
  generateSilence(): GeneratedSignal {
   ...
   */
  generateWhiteNoise(amplitude: number = 0.8): GeneratedSignal {
    const totalSamples = this.config.sampleRate * this.config.duration
    const numBuffers = Math.ceil(totalSamples / this.config.bufferSize)
    const buffers: Float32Array[] = []
   ...
   */
  generatePinkNoise(amplitude: number = 0.7): GeneratedSignal {
    const totalSamples = this.config.sampleRate * this.config.duration
    const numBuffers = Math.ceil(totalSamples / this.config.bufferSize)
    const buffers: Float32Array[] = []
    ...
    const generatePinkSample = (): number => {
   ...
   */
  generateSineWave(frequency: number, amplitude: number = 0.8): GeneratedSignal {
    const totalSamples = this.config.sampleRate * this.config.duration
    const numBuffers = Math.ceil(totalSamples / this.config.bufferSize)
    const buffers: Float32Array[] = []
   ...
   */
  generateTechnoKick(bpm: number = 128, amplitude: number = 0.9): GeneratedSignal {
    const totalSamples = this.config.sampleRate * this.config.duration
    const numBuffers = Math.ceil(totalSamples / this.config.bufferSize)
    const buffers: Float32Array[] = []
   ...
   */
  generatePodcast(amplitude: number = 0.4): GeneratedSignal {
    const totalSamples = this.config.sampleRate * this.config.duration
    const numBuffers = Math.ceil(totalSamples / this.config.bufferSize)
    const buffers: Float32Array[] = []
   ...
   */
  generateTheDrop(amplitude: number = 1.0): GeneratedSignal {
    const totalSamples = this.config.sampleRate * this.config.duration
    const numBuffers = Math.ceil(totalSamples / this.config.bufferSize)
    const buffers: Float32Array[] = []
   ...
   */
  generateBuildup(duration: number = 16, amplitude: number = 0.9): GeneratedSignal {
   ...
   */
  generateAllStandardSignals(): GeneratedSignal[] {
    return [
