...
interface OroSolidoConfig {
  ...
  /** Exponente de decay en Fase 2 (mayor = más rápido al inicio) */
  decayCurve: number

  /** Intensidad mínima al final del barrido (no llega a 0) */
  decayFloor: number

  /** Ángulo de Pan para movers izquierda (0-255 DMX) */
  panLeft: number

  /** Ángulo de Pan para movers derecha (0-255 DMX) */
  panRight: number

  /** Tilt frontal en Fase 1 — apunta al público (0-255 DMX, 127 = centro) */
  tiltFront: number

  /** Tilt en Fase 2 — se abre hacia arriba durante el barrido */
  tiltSweep: number

  /** Color PEAK: Oro Puro / Ámbar Saturado */
  colorPeak: { r: number; g: number; b: number; w: number; a: number }

  /** Color DECAY: Ámbar cálido residual */
  colorDecay: { r: number; g: number; b: number; w: number; a: number }
}
...
export class OroSolido extends BaseEffect {
  ...
  readonly name = 'Oro Sólido'
  readonly category: EffectCategory = 'physical'
  readonly priority = 98         // Entre latina_meltdown (95) y solar_flare (100)
  ...
  getOutput(): EffectFrameOutput | null {
    ...
    let color: { r: number; g: number; b: number; w: number; a: number }
    if (inLatch) {
      color = colorPeak
    } else {
    ...
    const tiltNow = Math.round(
      this.config.tiltFront + (this.config.tiltSweep - this.config.tiltFront) * sweepProgress
    )
...
export function createOroSolido(config?: Partial<OroSolidoConfig>): OroSolido {
