...
const FORGE_COLOR_CHANNEL_TYPES = new Set<string>([
  'red', 'green', 'blue', 'white', 'amber', 'uv',
  'cyan', 'magenta', 'yellow', 'color_wheel',
])
const FORGE_IMPACT_CHANNEL_TYPES = new Set<string>([
  'dimmer', 'strobe', 'shutter',
])
const FORGE_KINETIC_CHANNEL_TYPES = new Set<string>([
  'pan', 'pan_fine', 'tilt', 'tilt_fine', 'speed', 'rotation',
])
const FORGE_BEAM_CHANNEL_TYPES = new Set<string>([
 ...
 */
const FORGE_CHANNEL_KEYS = new Set<string>([
...
export class ForgeGraphCompiler {
  ...
  private static _collectInputOwnerCells(
    flatGraph: IForgeNodeGraph,
    nodeMap: Map<ForgeNodeId, IForgeNode>,
  ): Map<ForgeNodeId, string[]> {
    ...
    for (const e of flatGraph.edges) {
      ...
      if (list) list.push(e.targetNode)
      else adjacency.set(e.sourceNode, [e.targetNode])
    }

    const owners = new Map<ForgeNodeId, string[]>()
    for (const node of flatGraph.nodes) {
      if (node.type !== 'input_dmx') continue

      // BFS over reachable outputs — covers direct edges AND math/proc chains
      const found: string[] = []
      const seen = new Set<ForgeNodeId>([node.id])
      const queue: ForgeNodeId[] = [node.id]
      while (queue.length > 0) {
        ...
        for (const t of targets) {
          ...
          if (tn.type === 'output_dmx') {
            const oc = tn.config as IOutputDmxConfig
            const cell = oc.aetherNodeId ?? ForgeGraphCompiler._inferCellSuffix(oc.channelType)
            if (cell && !found.includes(cell)) found.push(cell)
          }
          queue.push(t)
        }
      }
      if (found.length > 0) owners.set(node.id, found)
    }
    return owners
  }
   ...
   */
  private static _inferCellSuffix(channelType: string): string {
    const t = channelType.toLowerCase().trim()
    if (FORGE_COLOR_CHANNEL_TYPES.has(t))   return 'color'
    if (FORGE_IMPACT_CHANNEL_TYPES.has(t))  return 'impact'
    if (FORGE_KINETIC_CHANNEL_TYPES.has(t)) return 'kinetic'
    if (FORGE_BEAM_CHANNEL_TYPES.has(t))    return 'beam'
    return 'atmosphere'
  }
