...
export interface OrganismSignature {
  organismId: string
...
export interface SpeciationResult {
  ...
  speciesCount: number
  assignments: { organismId: string; speciesId: string }[]
  speciesSizes: Map<string, number>
}
...
interface KMeansResult {
  ...
  assignments: number[]  // cluster index per organism
}
...
function kMeans(
  ...
  for (let iter = 0; iter < MAX_ITERATIONS && !converged; iter++) {
    ...
    let changes = 0
    for (let i = 0; i < n; i++) {
      if (newAssignments[i] !== assignments[i]) changes++
    }
...
export class SpeciationEngine {
  ...
  runSpeciation(): SpeciationResult {
    ...
    const rows = db.prepare(
      `SELECT organism_id, bezier_signature, species_id
       FROM lfx_organisms
       WHERE status IN ('alive', 'champion')
       ORDER BY organism_id`,
    ).all() as { organism_id: string; bezier_signature: Buffer; species_id: string | null }[]
    ...
    const organisms: OrganismSignature[] = rows.map((row) => ({
      organismId: row.organism_id,
      signature: new Float32Array(
        row.bezier_signature.buffer,
        row.bezier_signature.byteOffset,
        Math.min(row.bezier_signature.byteLength / 4, SIGNATURE_LENGTH),
      ),
    ...
    const updateStmt = db.prepare(
      'UPDATE lfx_organisms SET species_id = @species WHERE organism_id = @id',
    )

    const speciesSizes = new Map<string, number>()
    const assignmentRecords: { organismId: string; speciesId: string }[] = []
    ...
    console.log(
      `[Speciation 🧬] Clustered ${organisms.length} organisms into ${speciesCount} species ` +
...
let _instance: SpeciationEngine | null = null

export function getSpeciationEngine(): SpeciationEngine {
  if (_instance == null) _instance = new SpeciationEngine()
  return _instance
}

export function __resetSpeciationEngineForTests(): void {
  _instance = null
}
