...
import type { ContextVector6D, RarityTier } from '../types'
...
const ALPHA_6D: readonly number[] = [0.30, 0.22, 0.15, 0.13, 0.10, 0.10]
...
export interface CustomsEvaluation {
  chosenByDecisionMaker: boolean
  passedGates: boolean
  rejectedByGate: boolean
}

export interface HeatmapContext {
  z_score_avg_3s: number
  low_band_avg_3s: number
  energy_phase: string
  vibe_id: string
  section_id: string
  texture: string
 ...
 */
export function evaluateCustoms(c: CustomsEvaluation): number {
  return (
    (c.chosenByDecisionMaker ? W_DM : 0) +
 ...
 */
function encodeContext6D(ctx: HeatmapContext): number[] {
  return [
    ctx.z_score_avg_3s,
    ctx.low_band_avg_3s,
 ...
 */
export function contextDistance6D(
  current: ContextVector6D,
  historical: HeatmapContext,
): number {
...
export function evaluateContext(
  current: ContextVector6D,
  heatmaps: readonly { context: HeatmapContext; survivalRate: number }[],
): number {
 ...
 */
export function computeDeltaF(
  customs: CustomsEvaluation,
  current: ContextVector6D,
  heatmaps: readonly { context: HeatmapContext; survivalRate: number }[],
): number {
 ...
 * F_birth(child) = F(parent) · β_inherit · rarity_bonus(child.tier)
 */
export function computeBirthFitness(
  parentFitness: number,
  childTier: RarityTier,
): number {
 ...
 */
export function updateFitnessInDB(
 ...
 */
export function evaluateFireEvent(
  organismId: string,
  customs: CustomsEvaluation,
  currentCtx: ContextVector6D,
  heatmaps: readonly { context: HeatmapContext; survivalRate: number }[],
  vault?: GenesisVaultService,
): FitnessUpdate {
