...
import type { LfxOrganism, RarityTier } from '../types'
...
const SUFFIXES_COMMON: string[] = ['', '', '', '', 'Mk.II', 'Mk.III', 'Beta', 'Raw']
const SUFFIXES_RARE: string[] = ['', '', 'Mk.IV', 'Prime', 'X', 'Plus', 'Alpha', 'EX']
const SUFFIXES_EPIC: string[] = ['', 'Prime', 'X', 'Ultra', 'Apex', 'Omega', 'Z', 'REX']
const SUFFIXES_LEGENDARY: string[] = ['Prime', 'Omega', 'Apex', 'Ultra', 'MAX', 'Z', 'REX', 'Nova']
...
const SUFFIXES_MYTHIC: string[] = ['Omega', 'Apex', 'MAX', 'REX', 'Nova', 'Zero', 'Alpha', 'Prime']

// ─── NAMING CONTEXT ─────────────────────────────────────────────────────────

/**
 * Optional blueprint DNA context for richer name selection.
 * When provided, adjectives are selected by aggression × texture.
 * When absent, falls back to organism metrics (rarityScore, l2Distance).
 */
export interface NamingBlueprintContext {
  dnaAggression: number
  dnaChaos: number
  dnaOrganicity: number
 ...
 */
export function generateOrganismName(
  organism: LfxOrganism,
  blueprint?: NamingBlueprintContext,
): string {
  ...
  if (blueprint) {
    ...
    if (aggressionHigh && !textureDirty) pool = ADJECTIVES_HIGH_CLEAN
    else if (aggressionHigh && textureDirty) pool = ADJECTIVES_HIGH_DIRTY
    else if (!aggressionHigh && !textureDirty) pool = ADJECTIVES_LOW_CLEAN
    else pool = ADJECTIVES_LOW_DIRTY
...
function getSuffixPool(tier: RarityTier): string[] {
  switch (tier) {
