...
/**
 * A reference anchor point in the A/C/O cube with semantic description.
 * Used to validate that author-declared genome values map to meaningful
 * behavioral characteristics.
 */
export interface GenomeAnchor {
  readonly id: string
 ...
 */
export const GENOME_ANCHORS: readonly GenomeAnchor[] = Object.freeze([
  Object.freeze({
    id: 'ambient-wash',
    label: 'Ambient Wash',
 ...
 */
export function validateGenome(genome: FrozenGenome): string | null {
 ...
 */
export function nearestAnchor(genome: FrozenGenome): GenomeAnchor {
 ...
 */
export function describeGenome(genome: FrozenGenome): string {
  const tick = (ticks: readonly { value: number; label: string }[], v: number): string => {
    let best = ticks[0]
    let bestDist = Infinity
