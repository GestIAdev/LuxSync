...
import { resolvePro, type PhaseConfigPro, type FixturePhase } from './PhaseConfigPro'

/**
 * Override manual de fase para un fixture individual.
 */
export interface PhaseOverride {
  ...
  /**
   * En modo 'delta': ms a sumar al offset algorítmico.
   * En modo 'absolute': ms absolutos desde t=0.
   * Rango canónico: [0, durationMs].
   */
  offsetMs: number

  /**
   * Si true, este fixture está "pinned" — su offset no se recalcula
   * cuando cambian spread/shuffle/wings. En modo delta, el offset
   * algorítmico se congela en el valor que tenía al pin.
   */
  pinned?: boolean
}

/**
 * Mapa de overrides: fixtureId → PhaseOverride.
 * Vacío o ausente → comportamiento algorítmico puro.
 */
 ...
 */
export function unbakeOverrides(): PhaseOverrideMap {
  return {}
}
 ...
 */
export function countOverrides(overrides: PhaseOverrideMap | undefined): number {
