...
export class EnergyConsciousnessEngine {
  ...
  private _rmsAverage3s: number = 0
   ...
   */
  process(rawEnergy: number, debugData?: EnergyDebugData, evidence?: SectionEvidence, vibe?: string): EnergyContext {
    const now = Date.now()
  ...
  private detectFlashbang(
    ...
    if (now - this._lastFlashbangTimestamp < this.FLASHBANG_COOLDOWN_MS) return false
    ...
    if (this._rmsAverage10s < 0.25) return false
...
export class MultiSpectralEnergyLadder {
   ...
   * 3. Final zone = baseZone + tensionElevation (clamped to 'peak')
   */
  classify(evidence: SectionEvidence, smoothedEnergy: number, vibe?: string): MultiSpectralZone {
