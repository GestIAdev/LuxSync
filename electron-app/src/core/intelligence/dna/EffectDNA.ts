...
export interface EffectDNA {
  ...
  /** ☢️ WAVE 3469: sesgo paramétrico para romper empates euclidianos */
 ...
 * breakdown = momento íntimo = orgánico · drop = explosión mecánica = sintético
 */
...
export class DNAAnalyzer {
  ...
  calculateRelevance(effectId: string, targetDNA: TargetDNA): number {
    ...
    // selectionBias no existe en FrozenGenome — usar 1.0 (neutro)
    const selectionBiased = Math.min(1, confidenceWeighted)
  ...
  calculateDistance(effectId: string, targetDNA: TargetDNA): number {
    const entry = getDynamicEffectRegistry().getEntry(effectId)
    if (!entry) return this.MAX_DISTANCE
  ...
  rankEffects(
    ...
    const allEntries = getDynamicEffectRegistry().getAllEntries()
    let effectIds = allEntries.map(e => e.id)
  ...
  getEffectDNA(effectId: string): EffectDNA | undefined {
    ...
    // Adaptar FrozenGenome a la interfaz EffectDNA para compatibilidad
    return {
      aggression: entry.dna.aggression,
      chaos: entry.dna.chaos,
      organicity: entry.dna.organicity,
      textureAffinity: entry.textureAffinity,
    }
  }
