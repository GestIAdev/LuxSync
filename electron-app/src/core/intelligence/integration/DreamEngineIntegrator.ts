...
export class DreamEngineIntegrator {
   ...
   */
  private _logChampionReject(reason: string, effect: { effect: string; effectName?: string }): void {
  ...
  public async executeFullPipeline(context: PipelineContext): Promise<IntegrationDecision> {
    ...
    let ethicalVerdict = await visualConscienceEngine.evaluate(
      candidates,
      safetyContext
    )
    ...
    if (isFastPath && ethicalVerdict.verdict === 'REJECTED') {
      console.log(
        `[INTEGRATOR] 🔮🔄 FAST PATH FALLBACK: pre-buffered "${effectDisplayName(dreamResult.bestScenario?.effect.effectName ?? dreamResult.bestScenario?.effect.effect ?? '?')}" was REJECTED by ethics ` +
      ...
      if (fallbackCandidates.length > 0) {
        const fallbackVerdict = await visualConscienceEngine.evaluate(
          fallbackCandidates,
          safetyContext
        )
        ...
        if (fallbackVerdict.verdict === 'APPROVED') {
          console.log(
            `[INTEGRATOR] 🔮✅ FAST PATH FALLBACK SUCCESS: "${effectDisplayName(fallbackVerdict.approvedEffect?.effectName ?? fallbackVerdict.approvedEffect?.effect ?? '?')}" approved from fallback pool`
          )
        }
    ...
    if (ethicalVerdict.verdict === 'REJECTED' && dreamResult.bestScenario) {
      this._logChampionReject(
        `ethics-veto (score=${ethicalVerdict.ethicalScore.toFixed(3)}, ` +
        `violations=[${ethicalVerdict.violations.map(v => v.value).join(', ')}])`,
        dreamResult.bestScenario.effect,
      )
    }
    ...
    if (moodAdjustedEffect) {
      ...
      if (rawIntensity !== adjustedIntensity) {
        console.log(
          `[INTEGRATOR] 🎭 Intensity adjusted: ${rawIntensity.toFixed(2)} → ${adjustedIntensity.toFixed(2)} (${_currentProfile.emoji})`
        )
    ...
    if (decision.approved && decision.effect) {
      if (decision.effect.intensity < 0.30) {
        ...
        this._logChampionReject(
          `intensity-gate (${decision.effect.intensity.toFixed(2)} < 0.30)`,
          decision.effect,
        )
        return {
          ...decision,
  ...
  private async dreamEffects(context: PipelineContext): Promise<EffectDreamResult> {
    ...
    try {
      ...
      let activeCooldowns: Map<string, number>
      try {
      ...
      if (narrativePhase.toLowerCase() === 'building') {
        const scaledCooldowns = new Map<string, number>()
