...
// ═══════════════════════════════════════════════════════════════════════════
// RESULTADO DEL ANÁLISIS DE BELLEZA
// ═══════════════════════════════════════════════════════════════════════════

export interface BeautyAnalysis {
  /** Score total de belleza (0-1) */
  totalBeauty: number
  
  /** Adherencia a proporciones PHI (0-1) */
  phiAlignment: number
  
  /** Distribución fibonacci de elementos (0-1) */
  fibonacciDistribution: number
  
  /** Armonía cromática de la paleta (0-1) */
  chromaticHarmony: number
  
  /** Equilibrio de contrastes (0-1) */
  contrastBalance: number
  
  /** Tendencia vs análisis anterior */
...
interface BeautyHistoryEntry {
  timestamp: number
  totalBeauty: number
}

const MAX_HISTORY = 30
const beautyHistory: BeautyHistoryEntry[] = []
let lastAnalysis: BeautyAnalysis | null = null
 ...
 */
export function getAverageBeauty(): number {
  if (beautyHistory.length === 0) return 0.5
 ...
 */
export function getBeautyTrend(): 'rising' | 'stable' | 'falling' {
  if (beautyHistory.length < 5) return 'stable'
 ...
 */
export function evaluateGoldenDistribution(proposedHue: number, existingHues: number[]): number {
  if (existingHues.length === 0) return 1.0 // Cualquier hue es válido si está vacío
 ...
 */
export function resetBeautyHistory(): void {
...
function calculateFibonacciDistribution(palette: SelenePalette): number {
  ...
  // Extraer hues
  const hues = colors.map(c => c!.h).filter(h => h !== undefined) as number[]
  if (hues.length < 2) return 0.5
...
function calculateChromaticHarmony(palette: SelenePalette): number {
...
function calculateContrastBalance(
  palette: SelenePalette,
  pattern: SeleneMusicalPattern
): number {
  ...
  if (!primary || !contrast) return 0.5
  ...
  if (hueContrast > 180) hueContrast = 360 - hueContrast
...
function calculateTrend(currentBeauty: number): 'rising' | 'stable' | 'falling' {
  if (beautyHistory.length < 3) return 'stable'
...
function updateHistory(entry: BeautyHistoryEntry): void {
