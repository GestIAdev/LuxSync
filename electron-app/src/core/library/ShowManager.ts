...
export interface ShowLoadResult {
  success: boolean
  data?: ShowData
  error?: string
}

export interface ShowDeleteResult {
  success: boolean
  error?: string
}
...
class ShowManager {
   ...
   */
  private ensureShowsFolder(): void {
    try {
   ...
   */
  private createDefaultShowIfEmpty(): void {
    try {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          const data = JSON.parse(content) as Partial<ShowData>
   ...
   */
  loadShow(filename: string): ShowLoadResult {
    try {
      const filePath = path.join(this.showsPath, filename)
      ...
      const content = fs.readFileSync(filePath, 'utf-8')
      const data = JSON.parse(content) as ShowData
   ...
   */
  deleteShow(filename: string): ShowDeleteResult {
    try {
      const filePath = path.join(this.showsPath, filename)
   ...
   */
  createNewShow(name: string, description: string = ''): ShowSaveResult {
    const defaultShow = createDefaultShow()
