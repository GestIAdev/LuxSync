...
export function setupArbiterHandlers(): void {
  ...
  ipcMain.handle('lux:arbiter:setManual', (_event, args: {
    ...
    const overrideChannels: ChannelType[] = channels || 
      (Object.keys(controls) as ChannelType[])
   ...
   */
  ipcMain.handle('lux:arbiter:blackout', (_event, active: boolean) => {
   ...
   */
  ipcMain.handle('lux:arbiter:removeEffect', (_event, type: string) => {
   ...
   */
  ipcMain.handle('lux:arbiter:setGrandMaster', (_event, value: number) => {
