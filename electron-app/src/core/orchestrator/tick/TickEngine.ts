...
import { FIX_DATA_FLOATS, CHANNELS_PER_UNI, MAX_UNIVERSES, FLOATS_PER_FIX, GLASS_HEADER_FLOATS, MAX_GLASS_FIXTURES, CELL_COLOR_BASE } from '../../aether/glass/layout'
...
export class TickEngine {
  ...
  private static readonly BPM_CANDIDATE_CONFIRM_FRAMES_OCT_DOWN = 180
  private static readonly BPM_CANDIDATE_CONFIRM_FRAMES_OCT_UP = 20
  private static readonly BPM_CANDIDATE_MIN_CONFIDENCE_OCT_DOWN = 0.9
  private static readonly BPM_CANDIDATE_MIN_CONFIDENCE_OCT_UP = 0.6
  ...
  async tick(): Promise<void> {
      ...
      if (workerBpm > 0 && workerConfidence > 0.5) {
        ...
        if (this._stableBpm > 0 && workerOracleRawBpm > 0) {
          ...
          if (oracleKalmanAgree && stableIsOctaveUp) {
            console.log(
              `[TickEngine] 🔬 WAVE 7544: DUAL-DETECTOR OCTAVE-DOWN correction: ` +
    ...
    for (let _fi = 0; _fi < fixtureCount; _fi++) {
      ...
      state.rAmbient = 0; state.gAmbient = 0; state.bAmbient = 0
      state.rAir = 0; state.gAir = 0; state.bAir = 0
      state.rStrobe = 0; state.gStrobe = 0; state.bStrobe = 0
      state.pan  = 128
      state.tilt = 128
      state.zoom = 128
      state.focus = 128
