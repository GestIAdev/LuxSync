...
import { chillAmbientEngine, type ChillAmbientFrame } from '../../hal/physics/ChillAmbientEngine'
...
export interface SeleneLuxAudioMetrics {
  ...
  // 🌊 WAVE 8003: Photon block — wallIntensity lower-bound for anti-collapse DMX
  photon?: GodEarPhoton;
...
export interface SeleneLuxOutput {
  ...
  // 🌊 WAVE 8004: Photon strobe override — rate + duty from StrobeEngine
  strobeOverride?: { rate: number; duty: number } | null;
  /** 🌊 WAVE 7129.6: Morph factor from ChillAmbientEngine [0.20, 0.80]. Drives hue drift. */
...
export class SeleneLux {
  ...
  private normalizeAudioMetrics(metrics: SeleneLuxAudioMetrics | null | undefined): SeleneLuxAudioMetrics {
    const src = (metrics ?? {}) as Partial<SeleneLuxAudioMetrics>
