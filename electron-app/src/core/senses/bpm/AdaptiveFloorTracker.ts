...
export class AdaptiveFloorTracker {
  private readonly floorBuffer: number[] = [];
  private currentFloor: number = ADAPTIVE_FLOOR_BOOTSTRAP;
   ...
   */
  update(rawBassFlux: number): number {
    ...
    this.currentFloor = Math.max(
      ADAPTIVE_FLOOR_MIN,
      Math.min(ADAPTIVE_FLOOR_MAX, median * ADAPTIVE_FLOOR_RATIO)
    );
