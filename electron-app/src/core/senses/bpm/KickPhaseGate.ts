...
export class KickPhaseGate {
  private peakEstimate: number;
  private lastKickTs: number;
  private phaseAnchorTs: number;
  private totalKicks: number;

  private kickThisFrame: boolean;
  ...
  get lastKickTimestampMs(): number {
   ...
   */
  process(needle: number, currentFloor: number, bpm: number, timestampMs: number): void {
    this.kickThisFrame = false;
    ...
    if (needle > 0 && needle > currentFloor * FLOOR_RATIO) {
      ...
      if (bpm > 0) {
        ...
        if (expected > debounceMs) debounceMs = expected;
      }
      ...
      if (this.lastKickTs === 0 || timestampMs - this.lastKickTs >= debounceMs) {
        ...
        if (passedPeak) {
          ...
          if (needle > this.peakEstimate) this.peakEstimate = needle;
        }
      }
    }
