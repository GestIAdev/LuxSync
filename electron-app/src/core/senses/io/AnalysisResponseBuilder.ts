...
/**
 * Extensión del protocolo base AudioAnalysis con los datos Wave8.
 * Misma forma que el ExtendedAudioAnalysis inline en senses.ts.
 */
export interface ExtendedAudioAnalysis extends AudioAnalysis {
  ...
  wave8?: {
    ...
    genre: GenreOutput;
    /** WAVE 1228: Mood pruning — solo primary se consume. Resto son phantom fields. */
    mood?: {
      primary: string;
...
/**
 * Todo el contexto necesario para construir un ExtendedAudioAnalysis.
 * Agrupa los outputs de todos los stages del pipeline.
 */
export interface AnalysisBuildInput {
  // -- Frame metadata --
  frameId: number;
  ...
  // -- BPM / Rhythm (Stage 2) --
  bpmResult: GodEarBPMResult;
  /** BPM musical post Dance-Pocket-Fold + Dembow Ceiling (WAVE 2175/2191) */
  musicalBpm: number;
  ...
  // -- Mood (Stage 3) --
  moodOutput: MoodState;

  // -- WAVE 3418: Raw peak telemetry --
  inputPeakAbs: number;
  inputRMS: number;
}
 ...
 */
export function buildPayload(input: AnalysisBuildInput): ExtendedAudioAnalysis {
  const {
    frameId,
    spectrum,
    agcResult,
  ...
  const mood: 'dark' | 'bright' | 'neutral' = 'neutral';
