...
import { buildPayload, calculateZeroCrossingRate } from '../io/AnalysisResponseBuilder';
import type { ExtendedAudioAnalysis } from '../io/AnalysisResponseBuilder';
...
import type { AudioMetrics } from '../../../workers/TrinityBridge';
import type { TrinityConfig } from '../../../workers/WorkerProtocol';

// ============================================
// TYPES
// ============================================

/** Forma pública del SensesPipeline. */
export interface ISensesPipeline {
 ...
 */
export class SensesPipeline implements ISensesPipeline {
  ...
  private readonly sectionTracker: SectionTracker;
  ...
  processFrame(incoming: Float32Array): ExtendedAudioAnalysis {
    ...
    const wave8Output = this.sectionTracker.analyze(
      audioMetrics,
      normalizedEnergy,
      beatState,
      this.frameCount,
    ...
    return buildPayload({
      ...
      moodOutput: {
        ...
        secondary: wave8Output.mood.secondary as string | null,
   ...
   */
  private _measureInputTelemetry(buffer: Float32Array): { peakAbs: number; rms: number } {
    let peakAbs = 0;
    let rmsSum = 0;
   ...
   */
  private _buildEarlyExit(inputPeakAbs: number, inputRMS: number): ExtendedAudioAnalysis {
    return {
      timestamp: Date.now(),
      frameId: this.frameCount,
      agcGainFactor: 1.0,
