...
import { RhythmTracker, type RhythmTrackResult } from '../tracking/RhythmTracker';
...
/** Output canónico del BPMService — alineado con la interfaz IBPMService del blueprint. */
export interface BPMOutput {
  /** BPM musical post Dance-Pocket-Fold + Dembow Ceiling */
  bpm: number;
  /** Confianza 0-1 del IntervalBPMTracker */
  confidence: number;
  /** Fase del beat 0-1 (posición en el ciclo actual) */
  beatPhase: number;
  /** ¿Kick detectado en este frame? */
...
export class BPMService {
  private readonly rhythmTracker = new RhythmTracker();
  private readonly shadowLogger = new ShadowLogger();

  private frameCount = 0;
  private lastResult: BPMOutput = {
    bpm: 0,
    confidence: 0,
    beatPhase: 0,
    kickDetected: false,
   ...
   */
  processFrame(spectrum: SpectrumResult, deterministicTimestampMs: number): BPMOutput {
    this.frameCount++;

    const track: RhythmTrackResult = this.rhythmTracker.process(spectrum, deterministicTimestampMs);
