...
import { MoodSynthesizer, type MoodState } from '../../../engine/musical/classification/MoodSynthesizer';
...
/**
 * Salida compuesta de los 4 analizadores Wave8.
 * Equivalente al objeto wave8 que senses.ts adjunta a ExtendedAudioAnalysis.
 */
export interface Wave8Output {
  rhythm: RhythmOutput;
  harmony: HarmonyOutput;
  section: SectionOutput;
  /** GenreOutput neutro — sin clasificación real (WAVE 61) */
  genre: GenreOutput;
  /** Mood simplificado — solo primary para EffectDNA.organicity (WAVE 1228) */
  mood: {
    primary: string;
...
function buildNeutralGenre(bpm: number, syncopation: number, hasFourOnFloor: boolean, energy: number): GenreOutput {
  return {
    ...
    mood: 'neutral' as GenreOutput['mood'],
  };
}
...
export class SectionTracker {
  ...
  analyze(
    ...
    const moodOutput: MoodState = this.moodSynthesizer.process(metricsForMood as any, beatState as any);
    ...
    const genreOutput = buildNeutralGenre(
      metrics.bpm,
      rhythmOutput.syncopation ?? 0,
      rhythmOutput.pattern === 'four_on_floor',
      energy,
    );
