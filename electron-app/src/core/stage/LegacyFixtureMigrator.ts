...
import { resolveRuntimeFixtureDefinition, type RuntimeFixtureDefinition } from '../library/RuntimeFixtureLibrary'
...
interface MigrationDetail {
  fixtureId: string
  fixtureName: string
  status: 'skipped' | 'migrated' | 'ambiguous' | 'unmatched'
  oldProfileId?: string
  oldDefinitionPath?: string
  newProfileId?: string
  candidates?: string[]
}
...
function isLegacyPath(value: string | undefined): boolean {
  if (!value || typeof value !== 'string') return false
  const lower = value.toLowerCase()
  return LEGACY_MARKERS.some(marker => lower.includes(marker))
}

function hasLegacyShape(fixture: FixtureV2): boolean {
  return isLegacyPath(fixture.definitionPath) ||
         isLegacyPath(fixture.profileId) ||
         isLegacyPath((fixture as any).definitionId) ||
         isLegacyPath((fixture as any).fixtureDefId)
}
 ...
 */
function matchLegacyFixture(fixture: FixtureV2): { definition: RuntimeFixtureDefinition; candidates: string[] } | null {
  ...
  if (definitionPath) {
    ...
    if (stem) candidateKeys.push(stem)
  }
  ...
  const matches = new Set<RuntimeFixtureDefinition>()
  for (const key of candidateKeys) {
    const def = resolveRuntimeFixtureDefinition([key])
    if (def) matches.add(def)
  }
 ...
 */
export function migrateLegacyFixtures(fixtures: FixtureV2[]): MigrationReport {
  ...
  for (const fixture of fixtures) {
    ...
    if (!match) {
      console.warn(
        `[LegacyFixtureMigrator] ✗ UNMATCHED: fixture=${fixture.id} name="${fixture.name}" ` +
    ...
    if (match.candidates.length > 1) {
      ...
      console.warn(
        `[LegacyFixtureMigrator] ⚠ AMBIGUOUS: fixture=${fixture.id} name="${fixture.name}" ` +
    ...
    fixture.channels = def.channels as any
    fixture.channelCount = def.channels.length

    console.log(
      `[LegacyFixtureMigrator] ✓ MIGRATED: fixture=${fixture.id} name="${fixture.name}" ` +
