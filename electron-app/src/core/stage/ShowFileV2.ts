 ...
 * - 'galvo': Laser galvanometer — ultra-low inertia, settle ~1ms (WAVE 7737/7749.29)
 */
export type MotorType = 'servo-pro' | 'stepper-quality' | 'stepper-cheap' | 'unknown' | 'galvo'
...
export interface PhysicsProfile {
   ...
   * @deprecated WAVE 2093.2 (CW-AUDIT-4): Use FixtureV2.calibration.panInvert instead.
   * This field is kept for backwards compatibility but is IGNORED by the runtime.
   * The HAL and PhysicsDriver read exclusively from calibration.panInvert.
   */
  invertPan: boolean
  
  /** 
   * Invert tilt direction (for fixtures mounted upside-down) 
   * @deprecated WAVE 2093.2 (CW-AUDIT-4): Use FixtureV2.calibration.tiltInvert instead.
   * This field is kept for backwards compatibility but is IGNORED by the runtime.
   * The HAL and PhysicsDriver read exclusively from calibration.tiltInvert.
   */
  invertTilt: boolean
...
// ═══════════════════════════════════════════════════════════════════════════
// VALIDATION — WAVE 2093.1: DEEP VALIDATION
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Validation result with detailed diagnostics
 */
export interface ShowFileValidationResult {
  valid: boolean
  errors: string[]
  warnings: string[]
}
 ...
 */
export function validateShowFileDeep(data: unknown): ShowFileValidationResult {
  const errors: string[] = []
  ...
  for (let i = 0; i < fixtures.length; i++) {
    ...
    if (!f.physics || typeof f.physics !== 'object') {
      ...
      const physics = f.physics as Record<string, unknown>
      ...
      if (physics.tiltLimits && typeof physics.tiltLimits === 'object') {
        const tl = physics.tiltLimits as Record<string, unknown>
  ...
  if (Array.isArray(show.groups)) {
    const groups = show.groups as Array<Record<string, unknown>>
    for (let g = 0; g < groups.length; g++) {
      ...
      const fIds = group.fixtureIds as string[]
...
export function getSchemaVersion(data: unknown): string | null {
  ...
  if (typeof obj.schemaVersion === 'string' && obj.schemaVersion.startsWith('2.')) return '2.0.0'
