...
export class TheiaFileLoader {
  ...
  public load(
    ...
    if (!atom) return { ok: false, atom: null, error: 'A1 schema fail' }

    // ── A2 — Trim sanity ─────────────────────────────────────────────────
    if (atom.trim.endMs - atom.trim.startMs < MIN_TRIM_DURATION_MS) {
      return _fail(
        `A2: trim duration < ${MIN_TRIM_DURATION_MS}ms ` +
        `(start=${atom.trim.startMs}, end=${atom.trim.endMs})`,
        opts.filePath,
      )
    }
  ...
  private _normalize(
    ...
    // Wrapper canónico
    let atomRaw: Record<string, unknown> | null = null
    if (root.$schema === 'luxsync.theia/2.0' && root.atom && typeof root.atom === 'object') {
      atomRaw = root.atom as Record<string, unknown>
    ...
    if (typeof atomRaw.id !== 'string' || atomRaw.id.length === 0) {
      _warn('A1: atom.id missing/empty', filePath); return null
    }
    if (typeof atomRaw.filePath !== 'string' || atomRaw.filePath.length === 0) {
      _warn('A1: atom.filePath missing/empty', filePath); return null
    }

    const trim = atomRaw.trim as Record<string, unknown> | undefined
    if (!trim || typeof trim.startMs !== 'number' || typeof trim.endMs !== 'number') {
      _warn('A1: atom.trim missing', filePath); return null
    }

    const energyZone = atomRaw.energyZone as Record<string, unknown> | undefined
    if (!energyZone || !_isEnergyZone(energyZone.min) || !_isEnergyZone(energyZone.max)) {
      _warn('A1: atom.energyZone missing/invalid', filePath); return null
    }

    const compatibleVibes = Array.isArray(atomRaw.compatibleVibes)
      ? (atomRaw.compatibleVibes as string[])
      : []
    const validSections = Array.isArray(atomRaw.validSections)
      ? (atomRaw.validSections as string[])
      : []
    ...
    return {
      ...
      energyZone: {
        min: energyZone.min as EnergyZone,
        max: energyZone.max as EnergyZone,
      },
      validSections,
      trim: {
