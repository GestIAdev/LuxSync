...
import type { PhysicsMode } from '../core/vibe/VibeCanon'
...
import {
  generateStereoMovement,
  buildMechanicsBypassIntent,
...
export interface EngineAudioMetrics {
  ...
  // ⚒️ WAVE 7749.80: Treble-ghost delta — raw 5-15kHz transient for EDM snare rescue
  raw_hh_delta?: number
...
export class TitanEngine extends EventEmitter {
  ...
  public async update(context: MusicalContext, audio: EngineAudioMetrics): Promise<LightingIntent> {
    ...
    // ─────────────────────────────────────────────────────────────────────
    const masterIntensity = calculateMasterIntensity(audio.energy, vibeProfile.dimmer as DimmerConfig)
    ...
    // ─────────────────────────────────────────────────────────────────────
    let zones = calculateZoneIntents(audio as MovementAudioInput & { energy: number })
    ...
    if (nervousOutput.mechanics) {
      ...
      const movAudio: MovementAudioInput = {
        bass: audio.bass,
        mid: audio.mid,
        high: audio.high,
        beatPhase: audio.beatPhase,
      ...
      const movMusical: MovementMusicalInput = {
