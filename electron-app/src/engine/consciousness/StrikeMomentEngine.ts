...
import type { PreyCandidate } from './StalkingEngine'
...
/**
 * Condiciones evaluadas para el strike
 */
export interface StrikeConditions {
  ...
  trend: {
    ...
    required: 'rising' | 'rising_or_stable'
    met: boolean
  }
  ...
  conditionsMet: number // Cuántas condiciones se cumplen
  totalConditions: number
}

/**
 * Resultado de un strike ejecutado
 */
...
export interface MusicalInterval {
  ...
  semitones: number
  intervalName: string
  consonance: number      // 0-1 (1 = perfecta consonancia)
  zodiacHarmony: number   // 0-1 (compatibilidad elemental)
}

/**
 * Configuración del motor de strike
 */
export interface StrikeConfig {
  beautyThreshold: number       // Mínimo beauty para strike
  consonanceThreshold: number   // Mínima consonancia musical
  clusterHealthThreshold: number // Mínima salud del cluster
  strictMode: boolean           // Si true, TODAS las condiciones deben cumplirse
}
...
const CONSONANCE_SCORES: Record<number, number> = {
...
export class StrikeMomentEngine {
  ...
  private lastNote: { note: string; element: ElementType } | null = null
  private consecutiveStrikes = 0
  private cooldownUntil = 0
  private readonly strikeCooldownMs = 2000  // 2 segundos entre strikes
   ...
   */
  updateStrikeResult(strikeIndex: number, postStrikeBeauty: number): void {
  ...
  getStrikeHistory(): StrikeResult[] {
    return [...this.strikeHistory]
  }
  
  /** Obtener último strike */
  getLastStrike(): StrikeResult | null {
  ...
  updateConfig(config: Partial<StrikeConfig>): void {
    this.config = { ...this.config, ...config }
