...
import type { MovementIntent as ProtocolMovementIntent } from '../../core/protocol/LightingIntent'
import {
  vibeMovementManager,
  type AudioContext as VMMContext,
  type MovementIntent as VMMMovementIntent,
...
/**
 * Subconjunto mínimo de EngineAudioMetrics que necesita la generación de movimiento.
 * Usando structural typing — no importamos el tipo completo para evitar acoplamiento.
 */
...
/**
 * Subconjunto mínimo de MusicalContext que necesita la generación de movimiento.
 */
export interface MovementMusicalInput {
  energy: number
  bpm: number
}
 ...
 * @pure
 */
export function vmmCoordToProtocol(vmmCoord: number): number {
 ...
 */
export function calculateGearboxBudget(vibeId: string): number {
 ...
 * @pure
 */
export function buildVMMContext(
 ...
 * @pure
 */
export function assembleStereoMovementIntent(
  vmmIntentL: VMMMovementIntent,
  vmmIntentR: VMMMovementIntent,
  ...
  return {
    pattern: vmmIntentL.pattern as ProtocolMovementIntent['pattern'],
