...
/**
 * Punto objetivo en el espacio del escenario (metros).
 * El operador apunta aquí y el IK calcula pan/tilt para cada fixture.
 */
...
/**
 * Orientación de montaje completa de un fixture para el cálculo IK.
 * Combina la orientación de instalación (ceiling/floor/etc.) con la
 * rotación personalizada del fixture (Rotation3D del ShowFile).
 */
export interface FixtureOrientation {
  installation: InstallationOrientation
  rotation: Rotation3D  // rotación adicional personalizada del fixture
}

/**
 * Límites mecánicos reales del fixture.
 * Definen el rango angular que el motor puede alcanzar físicamente.
 */
export interface MechanicalLimits {
  panRangeDeg: number   // Rango total de pan en grados (típico: 540)
  tiltRangeDeg: number  // Rango total de tilt en grados (típico: 270)
  tiltLimits?: {        // Límites DMX de seguridad para tilt
    min: number         // 0-255 DMX (más bajo permitido)
    max: number         // 0-255 DMX (más alto permitido)
  }
}

/**
 * Calibración del fixture (offsets y ejes invertidos).
 * Exactamente como viene de ShowFileV2.FixtureV2.calibration.
 */
export interface FixtureCalibration {
  panOffset: number    // grados, compensación mecánica
  tiltOffset: number   // grados, compensación mecánica
  panInvert: boolean   // eje pan invertido por montaje
  tiltInvert: boolean  // eje tilt invertido por montaje
}

/**
 * Todo lo que el IK necesita saber de un fixture para resolver.
 * Se construye una vez desde PatchedFixture/FixtureV2 y se pasa al solver.
 */
...
// ═══════════════════════════════════════════════════════════════════════════
// WAVE 2621-2622: SPATIAL FAN TYPES
// Modos de dispersión geométrica para el fanning espacial.
// ═══════════════════════════════════════════════════════════════════════════

/** Modos de dispersión espacial del fan */
export type SpatialFanMode = 'converge' | 'line' | 'circle'

/**
 * Resultado de solveGroupWithFan incluye el sub-target calculado por fixture.
 * Necesario para que la UI dibuje los rayos al punto real, no al target central.
 */
export interface IKFanResult extends IKResult {
  /** Sub-target calculado para este fixture después de aplicar el fan offset */
  subTarget: Target3D
}
...
export function setIKDebug(enabled: boolean): void { DEBUG_IK = enabled }
export function isIKDebug(): boolean { return DEBUG_IK }
...
const _localFrameScratch: { x: number; y: number; z: number } = { x: 0, y: 0, z: 0 }

export function solveInto(
  out: IKResult,
  fixture: IKFixtureProfile,
  target: Target3D,
  currentPanDMX: number | null = null
...
export function solveGroup(
  ...
  target: Target3D,
  currentPanDMXMap: ReadonlyMap<string, number> | null = null
): Map<string, IKResult> {
 ...
 */
export function computeLineFanOffsets(
  fixturePositions: ReadonlyArray<Position3D>,
  target: Target3D,
  amplitude: number
): Array<{ dx: number; dz: number }> {
  const count = fixturePositions.length
  if (count === 0) return []
  if (count === 1 || amplitude === 0) return fixturePositions.map(() => ({ dx: 0, dz: 0 }))
 ...
 */
export function computeCircleFanOffsets(
  count: number,
  amplitude: number
): Array<{ dx: number; dz: number }> {
  if (count === 0) return []
  if (count === 1 || amplitude === 0) return [{ dx: 0, dz: 0 }]
...
export function solveGroupWithFan(
  ...
  const results = new Map<string, IKFanResult>()
...
export function buildProfile(
  ...
  installation?: InstallationOrientation,
  calibration?: Partial<FixtureCalibration>,
  panRangeDeg?: number,
  tiltRangeDeg?: number,
 ...
 */
function rotateToLocalFrameInto(
 ...
 */
function resolveShortestPanPath(
  rawDMX: number,
  currentDMX: number,
  panRange: number
): { dmx: number; flipped: boolean } {
 ...
 */
function dmxToDegrees(dmx: number, rangeDeg: number): number {
  // DMX 127.5 = centro = 0°
  // DMX 0     = -rangeDeg/2
  // DMX 255   = +rangeDeg/2
