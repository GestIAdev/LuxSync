...
/**
 * Transformación de montaje en radianes, en la convención de UN dominio
 * concreto (IK o visual — nunca ambos a la vez). Ver `getIKMountAngles` y
...
/** Hacia dónde encara el eje de emisión del fixture en reposo. */
export type MountFacing = 'down' | 'up'

/** Lado de pared donde está montado el fixture (o 'none' si no es de pared). */
export type MountWallSide = 'none' | 'left' | 'right'

/**
 * Semántica física pura de una orientación de montaje — SIN convención de
 * rotación de ningún dominio. Es la única fuente de verdad conceptual.
 */
export interface MountSemantics {
  /** Hacia dónde apunta el fixture en reposo (antes de pan/tilt). */
  readonly facing: MountFacing
  /** Si está montado de espaldas al escenario (invierte la referencia de pan). */
  readonly backFacing: boolean
  /** Lado de pared, si aplica. */
  readonly wallSide: MountWallSide
}
 ...
 */
export function getMountSemantics(o: InstallationOrientation): MountSemantics {
