...
/**
 * Nivel de exposición de un gen en la UI.
 *   - `safe` → visible en modo SHIELDED. Bajo riesgo, alto impacto visual.
 *   - `raw`  → sólo visible en modo RAW REACTOR. Requiere criterio técnico.
 */
export type GeneTier = 'safe' | 'raw'

/** Límites físicos y metadatos de UI de un gen numérico. */
export interface GeneRange {
  /** Valor mínimo aceptado. El resolver clampea por debajo. */
  readonly min: number
  /** Valor máximo aceptado. El resolver clampea por encima. */
  readonly max: number
  /** Incremento del slider / redondeo sugerido. */
  readonly step: number
  readonly tier: GeneTier
  /** Sub-rango de riesgo `[desde, hasta]`, pintado en rojo por la UI. */
  readonly danger?: readonly [number, number]
  /** Unidad para el label del slider: `'ms'`, `'K'`, `'°'`, `'DMX/s'`, `'beats'`. */
 ...
 */
export const ENVELOPE_GENE_RANGES: Readonly<Record<string, GeneRange>> = {
...
const PHYSICS_RANGES: Readonly<Record<string, GeneRange>> = {
...
const COLOR_RANGES: Readonly<Record<string, GeneRange>> = {
  // ── THE FORBIDDEN WHEEL ──
 ...
 */
export const SCHEDULER_GENE_RANGES: Readonly<Record<string, GeneRange>> = {
...
const MOVEMENT_RANGES: Readonly<Record<string, GeneRange>> = {
  // ── THE REACH (amplitud) ──
...
const PATTERN_IDS: readonly string[] = [
 ...
 */
function expandEnvelopeRanges(): Record<string, GeneRange> {
  const out: Record<string, GeneRange> = {}
 ...
 */
function expandSchedulerRanges(): Record<string, GeneRange> {
  const out: Record<string, GeneRange> = {}
 ...
 */
function expandOverrides41Ranges(): Record<string, GeneRange> {
  const out: Record<string, GeneRange> = {}
  const groups = ['transient', 'separation', 'sidechain'] as const
 ...
 */
export const GENE_RANGES: Readonly<Record<string, GeneRange>> = {
...
export function getGeneRange(path: string): GeneRange | undefined {
 ...
 */
export function clampGene(
  path: string,
  value: number,
): { value: number; clamped: boolean; range?: GeneRange } {
  const range = GENE_RANGES[path]
  if (!range) return { value, clamped: false }
  if (!Number.isFinite(value)) return { value: range.min, clamped: true, range }
...
export function isInDangerZone(path: string, value: number): boolean {
...
export function getGenePathsByTier(tier: GeneTier): string[] {
