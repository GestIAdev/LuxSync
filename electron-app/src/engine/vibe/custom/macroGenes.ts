...
import type { MacroGeneId } from '../../../types/CustomVibe'
...
/** Dirección de la curva del macro — informativa para la UI. */
export type MacroDirection = 'positive' | 'negative'

/** Función de curva: mapea `t ∈ [0,1]` al valor del gen en sus unidades. */
export type MacroCurve = (t: number) => number

/** Un gen subyacente afectado por un Macro Gene. */
export interface MacroGeneTarget {
  /** Ruta dot-notation del gen subyacente. */
  readonly path: string
  /** Curva de mapeo 0..1 → valor del gen. */
  readonly curve: MacroCurve
  /** Dirección: `'positive'` = sube con el macro, `'negative'` = baja. */
  readonly direction: MacroDirection
}

/** Definición completa de un Macro Gene. */
export interface MacroGeneDef {
  readonly id: MacroGeneId
  /** Etiqueta legible para el dial. */
  readonly label: string
  /** Emoji/icono para la UI. */
  readonly icon: string
  /** Color de acento del dial (hex). */
  readonly accentHex: string
  /** Descripción corta para el tooltip. */
  readonly description: string
  /** Genes subyacentes que mueve. */
  readonly targets: readonly MacroGeneTarget[]
}

/** Resultado de expandir un macro a mutaciones concretas. */
 ...
 */
function lerpRange(path: string): MacroCurve {
 ...
 */
function lerpRangeInverse(path: string): MacroCurve {
  const range = getGeneRange(path)
  if (!range) return (t) => 1 - t
 ...
 */
function easeInCubic(base: MacroCurve): MacroCurve {
  return (t) => base(t * t * t)
}
 ...
 */
function easeOutCubic(base: MacroCurve): MacroCurve {
 ...
 */
export const MACRO_GENES: Readonly<Record<MacroGeneId, MacroGeneDef>> = {
  // ── AGGRESSION ──────────────────────────────────────────────────────────
  ...
  aggression: {
    ...
    accentHex: '#ff3b30',
    description:
      'Golpe más duro, decay más corto, menos fricción. Sube el punch transitorio.',
    targets: [
  ...
  viscosity: {
    ...
    accentHex: '#0a84ff',
    description:
      'Ataque y release más largos, más fricción, más suavizado. Movimiento más líquido.',
    targets: [
  ...
  thermalBias: {
    ...
    accentHex: '#ff9500',
    description:
      'Temperatura atmosférica más cálida y mayor gravedad térmica. Paleta más dorada.',
    targets: [
  ...
  spatialReach: {
    ...
    accentHex: '#bf5af2',
    description:
      'Barrido pan y tilt más amplios, fan array más extendido. Los movers cubren más espacio.',
    targets: [
  ...
  nervousness: {
    ...
    accentHex: '#30d158',
    description:
      'Ciclos y frases más cortos, velocidad global más alta. Movimiento más frenético.',
    targets: [
...
export const MACRO_GENE_LIST: readonly MacroGeneDef[] = [
...
export function getMacroGene(id: MacroGeneId): MacroGeneDef {
 ...
 */
export function expandMacro(id: MacroGeneId, value: number): readonly MacroMutation[] {
 ...
 */
export function expandMacroClamped(id: MacroGeneId, value: number): readonly MacroMutation[] {
