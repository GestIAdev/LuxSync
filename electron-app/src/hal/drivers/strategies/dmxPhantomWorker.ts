...
interface DetectedPort { path: string; name: string; confidence: number }

async function detectDmxPort(): Promise<DetectedPort | null> {
  const ports = await SerialPort.list()
  const candidates: DetectedPort[] = []
  ...
  for (const p of ports) {
    ...
    for (const chip of Object.values(KNOWN_CHIPS)) {
      if (vid === chip.vid && chip.pids.includes(pid)) {
        ...
        if (mfr.includes('dmx') || mfr.includes('enttec')) confidence = Math.min(100, confidence + 20)
...
async function bootstrap(): Promise<void> {
  ...
  parentPort?.on('message', (msg) => {
    if (msg === 'STOP') port.close(() => process.exit(0))
  })
}
