...
import { CHANNELS_PER_UNI, DMX_HEADER_I32, DmxHdr } from '../../core/aether/glass/layout'
...
// ─────────────────────────────────────────────────────────────────────────────
const {
  sab,        // SharedArrayBuffer completo (header + datos, 25 664 bytes)
...
let port: any    = null
let isOpen       = false
let outputLoop:  ReturnType<typeof setImmediate> | null = null
...
let minFrameNs: bigint = BigInt(Math.floor((1000 / safeHz) * 1_000_000))
...
async function boot(): Promise<void> {
  ...
  try {
    ...
    SerialPort = (serialportModule as any).SerialPort ?? (serialportModule as any).default?.SerialPort
  ...
  port.open((err: Error | null) => {
    ...
    port.on('data', () => { /* descarte silencioso de ecos RS-485 */ })
