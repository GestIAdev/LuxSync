...
/**
 * Candidato a "presa" - una escena/patrón que Selene está observando
 */
export interface PreyCandidate {
  /** Patrón musical que representa esta presa */
  pattern: {
    note: string
    element: ElementType
    avgBeauty: number
  ...
  /** Info de stalking acumulada */
  stalkingInfo: {
    firstSpottedAt: number      // Timestamp cuando se detectó
    cyclesObserved: number      // Ciclos de observación
...
/**
 * Decisión de acecho - ¿Deberíamos atacar?
 * (Renombrado de HuntDecision para evitar conflicto con ConsciousnessToLightMapper)
 */
export interface StalkingDecision {
...
/**
 * Configuración del motor de acecho
 */
export interface StalkingConfig {
  minStalkingCycles: number   // Mínimo ciclos antes de considerar strike
  maxStalkingCycles: number   // Máximo ciclos de observación
  switchThreshold: number     // % mejora mínima para cambiar de objetivo
  maxCandidates: number       // Máximo candidatos a trackear
  targetTimeoutMs: number     // Timeout para abandonar presa
}
...
export class StalkingEngine {
  ...
  private targetAcquiredAt: number | null = null
  private cycleCount = 0
  
  // === Historial para aprendizaje ===
  private recentDecisions: StalkingDecision[] = []
  private maxDecisionHistory = 50
  
  constructor(config?: Partial<StalkingConfig>) {
    if (config) {
  ...
  identifyPreyCandidates(
    ...
    const topKeys = new Set(candidates.map(c => this.getPatternKey(c.pattern as unknown as MusicalPattern)))
    ...
    for (const candidate of candidates) {
      const key = this.getPatternKey(candidate.pattern as unknown as MusicalPattern)
   ...
   */
  decideHunt(clusterHealth: number = 0.8): StalkingDecision {
    this.cycleCount++
    ...
    const confidence = this.calculateConfidence(
      enoughObservation, goodTrend, worthyPrey, healthyCluster, mustDecide
    )
    ...
    if (shouldStrike) {
      console.log(`🐆 [STALKING] STRIKE RECOMMENDED: ${this.getPatternKey(pattern as unknown as MusicalPattern)}`)
      this.currentTarget = null
      this.targetAcquiredAt = null
    }
   ...
   */
  private getPatternKey(pattern: MusicalPattern | PreyCandidate['pattern']): string {
   ...
   */
  private createNewCandidate(pattern: MusicalPattern): PreyCandidate {
    return {
      pattern: {
        note: pattern.note,
   ...
   */
  private calculateStability(beautyHistory: number[]): number {
    if (beautyHistory.length < 2) return 0.5
   ...
   */
  private calculateImprovement(current: PreyCandidate, challenger: PreyCandidate): number {
  ...
  private calculateConfidence(
    ...
    goodTrend: boolean,
    worthyPrey: boolean,
    healthyCluster: boolean,
    mustDecide: boolean
  ): number {
    if (mustDecide) return 0.6  // Forzado, menos confianza
    
    let confidence = 0.5
    if (enoughObs) confidence += 0.15
    if (goodTrend) confidence += 0.15
    if (worthyPrey) confidence += 0.15
    if (healthyCluster) confidence += 0.05
  ...
  private generateReasoning(
    ...
    if (!enoughObs) factors.push('Need more observation cycles')
    if (!goodTrend) factors.push('Beauty trend not favorable')
    if (!worthyPrey) factors.push('Hunt worthiness too low')
    if (!healthyCluster) factors.push('Cluster health insufficient')
  ...
  getStats(): {
    ...
    return {
      ...
      targetKey: this.currentTarget 
        ? this.getPatternKey(this.currentTarget.pattern as unknown as MusicalPattern)
        : null,
