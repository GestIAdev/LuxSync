...
const ZODIAC_SIGNS = [
  { name: 'Aries', symbol: '♈', element: 'fire' as const, quality: 'cardinal' as const },
  { name: 'Taurus', symbol: '♉', element: 'earth' as const, quality: 'fixed' as const },
  { name: 'Gemini', symbol: '♊', element: 'air' as const, quality: 'mutable' as const },
  { name: 'Cancer', symbol: '♋', element: 'water' as const, quality: 'cardinal' as const },
  { name: 'Leo', symbol: '♌', element: 'fire' as const, quality: 'fixed' as const },
  { name: 'Virgo', symbol: '♍', element: 'earth' as const, quality: 'mutable' as const },
  { name: 'Libra', symbol: '♎', element: 'air' as const, quality: 'cardinal' as const },
  { name: 'Scorpio', symbol: '♏', element: 'water' as const, quality: 'fixed' as const },
  { name: 'Sagittarius', symbol: '♐', element: 'fire' as const, quality: 'mutable' as const },
  { name: 'Capricorn', symbol: '♑', element: 'earth' as const, quality: 'cardinal' as const },
  { name: 'Aquarius', symbol: '♒', element: 'air' as const, quality: 'fixed' as const },
  { name: 'Pisces', symbol: '♓', element: 'water' as const, quality: 'mutable' as const },
];
...
export class SeleneTelemetryCollector {
  ...
  private palettesFromMemory = 0;
  private palettesGenerated = 0;
  private patternsLearned = 0;
  
  // Throttling
  private lastEmitTime = 0;
  private readonly minEmitInterval: number; // ms between emissions
  
  // Log deduplication
  private lastLogMessage = '';
  private lastLogDuplicateCount = 0;
  private pendingLogs: TelemetryLogEntry[] = [];
  
  // Energy trend tracking
  private energyHistory: number[] = [];
  private readonly energyHistorySize = 30;
  
  // Last known values for stability
  private lastAudio: AudioAnalysis | null = null;
  ...
  private collectAudio(audio: AudioAnalysis, inputGain: number): AudioTelemetry {
    return {
      spectrum: {
  ...
  private calculateEnergyTrend(): 'rising' | 'falling' | 'stable' {
  ...
  private collectDNA(audio: AudioAnalysis, brainOutput: BrainOutput | null): MusicalDNATelemetry {
  ...
  private elementToZodiacPosition(element: 'fire' | 'water' | 'air' | 'earth'): number {
  ...
  private collectHunt(_brainOutput: BrainOutput | null): HuntTelemetry {
    ...
    const huntTelemetry: HuntTelemetry = {
      status: 'idle',
      cycleId: null,
      currentTarget: null,
      strikeConditions: {
  ...
  private collectCosmic(audio: AudioAnalysis, _brainOutput: BrainOutput | null): CosmicTelemetry {
    const zodiacElement = this.calculateZodiacElement(audio);
    const zodiacPosition = this.elementToZodiacPosition(zodiacElement);
    const zodiacSign = ZODIAC_SIGNS[zodiacPosition];
  ...
  private calculateFibonacciHarmony(sequence: number[]): number {
  ...
  private deriveKeyFromFibonacci(sequence: number[]): string {
  ...
  private collectPalette(brainOutput: BrainOutput | null): PaletteTelemetry {
    const defaultColor = { h: 280, s: 70, l: 50, hex: '#a855f7' };
    
    if (!brainOutput) {
      return {
        strategy: 'procedural' as 'analogous' | 'triadic' | 'complementary',
        source: 'fallback',
        colors: {
    ...
    return {
      strategy: (palette.strategy || 'procedural') as 'analogous' | 'triadic' | 'complementary',
  ...
  private hslToTelemetry(hsl: { h: number; s: number; l: number }): { h: number; s: number; l: number; hex: string } {
    return {
      h: hsl.h,
      s: hsl.s,
      l: hsl.l,
  ...
  private collectSession(brainOutput: BrainOutput | null): SessionTelemetry {
   ...
   */
  private generateContextualLogs(audio: AudioAnalysis, brainOutput: BrainOutput | null): void {
    ...
    if (this.frameCount % 30 === 0 && context?.genre) {
      if (context.genre.confidence > 0.6) {
        this.addLog(
          'GENRE',
          `🎸 Detectando ${context.genre.primary}... (${(context.genre.confidence * 100).toFixed(0)}% confianza)`,
          'info',
          { genre: context.genre.primary, confidence: context.genre.confidence }
        );
      }
    }
    ...
    if (this.frameCount % 20 === 0) {
      if (brainOutput.mode === 'intelligent') {
        if (beauty > 0.85) {
          this.addLog(
            'HUNT',
            `👁️ Acechando... Beauty Score: ${(beauty * 100).toFixed(0)}% ¡Objetivo a la vista!`,
            'warning',
            { beauty, mode: 'intelligent' }
          );
        } else if (beauty > 0.7) {
          this.addLog(
            'HUNT',
            `🔍 Evaluando patrón... (${(beauty * 100).toFixed(0)}%)`,
            'info',
            { beauty }
          );
        }
    ...
    if (context?.harmony?.key && context.harmony.key !== this.lastContext?.harmony?.key) {
      this.addLog(
        'INFO',
        `🎹 Nueva tonalidad detectada: ${context.harmony.key} ${context.harmony.mode?.scale || ''}`,
        'info',
        { key: context.harmony.key, mode: context.harmony.mode?.scale }
      );
    }
    ...
    if (beauty > 0.9 && this.frameCount % 10 === 0) {
      this.addLog(
        'STRIKE',
        `✨ ¡BELLEZA EXCEPCIONAL! Score: ${(beauty * 100).toFixed(0)}%`,
        'success',
        { beauty, key: context?.harmony?.key }
      );
    }
    ...
    if (audio.beat.detected && audio.beat.confidence > 0.8) {
      ...
      if (bpmChange > 10 && this.frameCount % 40 === 0) {
        this.addLog(
          'BPM',
          `🥁 BPM ajustado: ${audio.beat.bpm.toFixed(0)} (Δ${bpmChange.toFixed(0)})`,
          'info',
          { bpm: audio.beat.bpm, change: bpmChange }
        );
      }
    }
    ...
    if (syncChange > 0.15 && this.frameCount > 20) {
      ...
      this.addLog(
        'INFO',
        `${direction} Sincopación ${intensity}: ${(currentSync * 100).toFixed(0)}% (Δ${(syncChange * 100).toFixed(0)}%)`,
        currentSync > 0.4 ? 'warning' : 'info',
        { syncopation: currentSync, change: syncChange, intensity }
      );
    }
    ...
    if (this.frameCount % 60 === 0 && currentSync > 0) {
      ...
      this.addLog(
        'INFO',
        `🎯 Groove: ${grooveFeel} (${(currentSync * 100).toFixed(0)}% sync)`,
        'info',
        { syncopation: currentSync, feel: grooveFeel }
      );
    }
  }
   ...
   */
  addLog(type: TelemetryLogEntry['type'], message: string, severity: TelemetryLogEntry['severity'] = 'info', data?: Record<string, unknown>): void {
    // Deduplication check
  ...
  private flushPendingLogs(): TelemetryLogEntry[] {
...
let telemetryInstance: SeleneTelemetryCollector | null = null;

export function getTelemetryCollector(targetFps?: number): SeleneTelemetryCollector {
