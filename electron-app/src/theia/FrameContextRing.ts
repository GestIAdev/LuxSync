...
export interface FrameContextSnapshot {
  tickId: number
  timestamp: number
  generation: number
}
 ...
 */
export function createFrameContextSAB(): SharedArrayBuffer {
...
export class FrameContextWriter {
  ...
  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < FRAME_CONTEXT_BYTE_LENGTH) {
      throw new Error(
        `[FrameContextWriter] SAB too small: ${sab.byteLength} < ${FRAME_CONTEXT_BYTE_LENGTH}`,
      )
    }
   ...
   */
  advance(tickId: number, timestampMs: number): void {
...
export class FrameContextReader {
  ...
  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < FRAME_CONTEXT_BYTE_LENGTH) {
      throw new Error(
        `[FrameContextReader] SAB too small: ${sab.byteLength} < ${FRAME_CONTEXT_BYTE_LENGTH}`,
      )
    }
    this.view = new Int32Array(sab)
  }
   ...
   */
  readIfChanged(): FrameContextSnapshot | null {
    const gen = Atomics.load(this.view, SLOT_GENERATION)
    if (gen === this._lastGeneration) return null
...
function readSnapshot(view: Int32Array): FrameContextSnapshot {
