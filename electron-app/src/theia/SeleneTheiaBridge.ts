...
import type { TheiaAssetStateId } from './protocol'
...
/** Snapshot del FrameContext que TitanOrchestrator entrega al bridge por tick. */
export interface BrainFrameContext {
  /** Energía global del frame (0–1). Normalizada por AGC + inputGain. */
  energy:       number
  /** Tipo de sección musical del Brain ('drop', 'buildup', 'verse', 'intro', …). */
  sectionType:  string
  /** Señal de drop inminente derivada por el Brain (energy > 0.8 por convención actual). */
  dropImminent: boolean
  /** Índice de frame monotónico del TitanOrchestrator. Usado como clock de hysteresis. */
  frameIndex:   number
}
...
export class SeleneTheiaBridge {
  ...
  private _currentState:        TheiaAssetStateId = 'idle'
  private _pendingState:        TheiaAssetStateId | null = null
  private _pendingCount:        number = 0
  private _lastTransitionTick:  number = -MIN_DWELL_TICKS - 1
   ...
   */
  notify(ctx: BrainFrameContext): void {
    if (this._theta === null) return
    ...
    if (ctx.frameIndex - this._lastTransitionTick < MIN_DWELL_TICKS) return
    ...
    this._pendingCount++
    if (this._pendingCount < CONFIRMATION_TICKS) return
  ...
  getState(): TheiaAssetStateId { return this._currentState }
  isAttached():  boolean { return this._theta !== null }

  // ── Resolución de estado (determinista, sin side-effects) ────────────────

  private _resolveTarget(ctx: BrainFrameContext): TheiaAssetStateId {
    ...
    if (SECTION_TO_DROP.has(sectionType))    return 'drop'
    if (SECTION_TO_BUILDUP.has(sectionType)) return 'buildup'
    if (SECTION_TO_AMBIENT.has(sectionType)) return 'ambient'

    // 2. Señal cognitiva de drop inminente (energy > 0.8 según Brain).
    if (dropImminent) return 'drop'
    ...
    if (this._currentState === 'drop') {
      ...
      if (energy >= ENERGY_THRESHOLD.dropExit) return 'drop'
      if (energy >= ENERGY_THRESHOLD.buildupEnter) return 'buildup'
      return 'ambient'
    }

    if (this._currentState === 'buildup') {
      // En BUILDUP: sube a DROP o cae a AMBIENT
      if (energy >= ENERGY_THRESHOLD.dropEnter) return 'drop'
      if (energy >= ENERGY_THRESHOLD.buildupExit) return 'buildup'
      return 'ambient'
    }
...
let _bridgeInstance: SeleneTheiaBridge | null = null

export function getSeleneTheiaBridge(): SeleneTheiaBridge {
  if (!_bridgeInstance) _bridgeInstance = new SeleneTheiaBridge()
