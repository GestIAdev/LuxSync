 ...
 */
export function createSharedVideoFrameBuffer(): SharedArrayBuffer {
  return new SharedArrayBuffer(VIDEO_SAB_BYTE_LENGTH)
}

// ─────────────────────────────────────────────────────────────────────────
// Writer — vive en el ThetaWorker
// ─────────────────────────────────────────────────────────────────────────

export interface VideoFramePublishInfo {
  /** Bytes RGBA8 del frame (longitud = width*height*4). */
  rgba: Uint8ClampedArray | Uint8Array
  width: number
  height: number
  /** TickId del FrameContextRing en el momento de la publicación. */
  tickId: number
}

export class VideoFrameWriter {
  private readonly meta: Int32Array
  private readonly slots: [Uint8Array, Uint8Array]
  private _producerSeq = 0

  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < VIDEO_SAB_BYTE_LENGTH) {
      throw new Error(
        `[VideoFrameWriter] SAB too small: ${sab.byteLength} < ${VIDEO_SAB_BYTE_LENGTH}`,
      )
    }
   ...
   */
  publish(info: VideoFramePublishInfo): boolean {
    const { rgba, width, height, tickId } = info
    if (width <= 0 || height <= 0) return false
    if (width > VIDEO_MAX_WIDTH || height > VIDEO_MAX_HEIGHT) return false
    const expectedBytes = width * height * 4
    if (rgba.length < expectedBytes) return false
...
// ─────────────────────────────────────────────────────────────────────────
// Reader — vive en TheiaOutputWindow (rAF loop)
// ─────────────────────────────────────────────────────────────────────────

export interface VideoFrameSnapshot {
  /** Vista directa al SAB (no se modifica). Longitud = w*h*4. */
  view: Uint8Array
  width: number
  height: number
  tickId: number
...
export class VideoFrameReader {
  private readonly meta: Int32Array
  private readonly slots: [Uint8Array, Uint8Array]
  private _lastSeenSeq = -1

  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < VIDEO_SAB_BYTE_LENGTH) {
      throw new Error(
        `[VideoFrameReader] SAB too small: ${sab.byteLength} < ${VIDEO_SAB_BYTE_LENGTH}`,
      )
    }
    this.meta = new Int32Array(sab, 0, VIDEO_META_INT32_LENGTH)
    this.slots = [
   ...
   */
  readIfChanged(): VideoFrameSnapshot | null {
    const seq = Atomics.load(this.meta, META_PRODUCER_SEQ)
    if (seq === this._lastSeenSeq) return null
