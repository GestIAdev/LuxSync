...
const META_SEQ    = 0 as const
const META_ACTIVE = 1 as const
const META_W      = 2 as const
const META_H      = 3 as const
 ...
 */
export function createThumbSAB(): SharedArrayBuffer {
...
export class ThumbFrameWriter {
  ...
  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < THUMB_SAB_BYTE_LENGTH) {
      throw new RangeError(
        `ThumbFrameWriter: SAB too small (${sab.byteLength} < ${THUMB_SAB_BYTE_LENGTH})`,
      )
    }
   ...
   */
  publish(data: Uint8ClampedArray): void {
...
export class ThumbFrameReader {
  ...
  constructor(sab: SharedArrayBuffer) {
    if (sab.byteLength < THUMB_SAB_BYTE_LENGTH) {
      throw new RangeError(
        `ThumbFrameReader: SAB too small (${sab.byteLength} < ${THUMB_SAB_BYTE_LENGTH})`,
      )
    }
    this._meta   = new Int32Array(sab)
    this._pixels = new Uint8ClampedArray(sab, PIXEL_OFFSET, THUMB_RGBA_BYTES)
  ...
  get width():  number { return Atomics.load(this._meta, META_W) }
  get height(): number { return Atomics.load(this._meta, META_H) }

  /** True si el productor ha publicado al menos un frame. */
  get hasData(): boolean { return Atomics.load(this._meta, META_ACTIVE) === 1 }
   ...
   */
  readIfNew(): Uint8ClampedArray | null {
    const seq = Atomics.load(this._meta, META_SEQ)
    if (seq === this._lastSeq) return null
    if (Atomics.load(this._meta, META_ACTIVE) !== 1) return null
    this._lastSeq = seq
    return this._pixels
  }
   ...
   */
  readLatest(): Uint8ClampedArray | null {
    if (Atomics.load(this._meta, META_ACTIVE) !== 1) return null
