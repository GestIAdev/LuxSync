 ...
 */
function generateSplitRadixPermutation(N: number): Uint16Array {
  const perm = new Uint16Array(N);
  
  function fillPerm(output: Uint16Array, outOffset: number, start: number, stride: number, size: number): void {
    if (size === 1) {
 ...
 */
function iterativeSplitRadixDIT(
...
function test(signal: number[], label: string): boolean {
  ...
  for (let k = 0; k < N; k++) {
    ...
    if (N <= 8) {
      console.log(
        `  X[${k}]: SR=(${outRe[k].toFixed(4)}, ${outIm[k].toFixed(4)}) ` +
...
// Warmup
for (let i = 0; i < 20; i++) iterativeSplitRadixDIT(perfBuf, oR, oI, perm4k);
