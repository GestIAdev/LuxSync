...
interface WorkerNode {
  ...
  // 🛡️ WAVE 7554: Anti-fratricidio — bloquea reentrada en handleWorkerFailure
  // durante una resurrección en curso (exit event → handleWorkerDeath → re-entra).
...
export class TrinityOrchestrator extends EventEmitter {
  ...
  private zombieIpcCount = 0; // WAVE 3433-A: forensic counter (IPC packets outside legacy source)
    ...
    if (this.legacyBridge && this.audioMatrix) {
      ...
      if (activeSource && activeSource !== 'legacy-bridge') {
        ...
        if (this.zombieIpcCount % 60 === 1) {
          console.log(
            `[TitanOrchestrator] [ZOMBIE RADAR] IPC buffer while activeSource=${activeSource} ` +
  ...
  private sendToWorker<T>(
    ...
    if (payload instanceof Float32Array) {
      node.worker.postMessage(message, [payload.buffer as ArrayBuffer]);
    } else {
