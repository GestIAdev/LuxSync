...
function dft(re: number[], im: number[]): { outRe: number[], outIm: number[] } {
  const N = re.length;
...
function splitRadixDIF(inputRe: number[]): { outRe: number[], outIm: number[] } {
  ...
  for (let i = 0; i < n; i++) {
    const j = bitRev[i];
    if (j > i) {
      let tmp = outRe[i]; outRe[i] = outRe[j]; outRe[j] = tmp;
      tmp = outIm[i]; outIm[i] = outIm[j]; outIm[j] = tmp;
    }
  }
