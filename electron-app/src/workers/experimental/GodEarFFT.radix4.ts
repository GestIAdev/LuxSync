...
function testR4(signal: number[], label: string): boolean {
  ...
  for (let k = 0; k < N; k++) {
    ...
    if (N <= 8) {
      console.log(
        `  X[${k}]: R4=(${outRe[k].toFixed(4)}, ${outIm[k].toFixed(4)}) ` +
...
const imp256 = new Array(256).fill(0); imp256[0] = 1;
...
console.log('\n--- Chirp test ---');
const ch: number[] = [];
for (let n = 0; n < 4096; n++) ch.push(Math.sin(Math.PI * n * n / 4096));
...
const imp8 = new Array(8).fill(0); imp8[0] = 1;
allPassed = testR4(imp8, 'Impulse8') && allPassed;
const imp32 = new Array(32).fill(0); imp32[0] = 1;
allPassed = testR4(imp32, 'Impulse32') && allPassed;
const imp2048 = new Array(2048).fill(0); imp2048[0] = 1;
...
// Warmup
for (let i = 0; i < 50; i++) cooleyTukeyRadix4DIT(perfBuf, oR, oI, bitRev4k);
...
let timeEnergy = 0;
for (let n = 0; n < 4096; n++) timeEnergy += perfBuf[n] * perfBuf[n];
cooleyTukeyRadix4DIT(perfBuf, oR, oI, bitRev4k);
let freqEnergy = 0;
for (let k = 0; k < 4096; k++) freqEnergy += oR[k] * oR[k] + oI[k] * oI[k];
freqEnergy /= 4096;
...
for (let n = 0; n < N; n++) cosSignal[n] = Math.cos(2 * Math.PI * 100 * n / N);
